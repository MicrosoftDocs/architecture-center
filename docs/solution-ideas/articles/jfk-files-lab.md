...
1. The OCR technology is not perfect; the results will vary greatly by scan and image quality.
1. Most file formats and data sources are supported. However, some scanned and native PDF formats may not be parsed correctly.
...
1. Click the following link to deploy the template: [![Deploy to Azure](https://azuredeploy.net/deploybutton.png)](https://portal.azure.com/#create/Microsoft.Template/uri/https%3A%2F%2Fraw.githubusercontent.com%2FMicrosoft%2FAzureSearch_JFK_Files%2Fmaster%2Fazuredeploy.json)
1. You will be redirected to Azure, provide your credentials to sign in.
1. Provide the required information:
    * Subscription: select your subscription.
    * Create a new resource group with the name: `rg-jfk-<your initials>`.
...
1. Go to **Resource groups** in the left pane and **search** for your resource group: `rg-jfk-<your initials>`.
...
1. Click the **Search** button. You will see the seven documents that we initially uploaded to the storage and the content of the documents in the **text** field.
...
In the previous step, we leveraged a preexisting skill. In this step, you will learn how to create your own custom cognitive skill. You'll learn how to plug this custom cognitive skill into the Cognitive Search pipeline. Building a custom skill gives you a way to insert specific transformations to your content and apply whatever enrichment process you require. In this example we will create a custom skill that annotates documents that contain CIA "Cryptonym" code words, that is, the CIA assigned the cryptonym "GPFLOOR" to Lee Harvey Oswald, so any documents containing that Cryptonym will be linked with Oswald.
...
1. Add the following code snippet to the function method to get the input records.
...
1. Add the following code at the end of the method. This will process all the records and put the results in the output.

    ```csharp
    // Process each record and set the cryptonym to the output if found
    var cryptonymLinker = new CryptonymLinker(executionContext.FunctionAppDirectory);
    var response = WebApiSkillHelpers.ProcessRequestRecords(skillName, requestRecords,
        (inRecord, outRecord) => {
            var word = inRecord.Data["word"] as string;
...
1. Click **Manage query keys**.
1. Click **Add**.
1. Copy the auto-generated **KEY** value as you will need it later.
...
1. Add the following code snippet where indicated:
    ...
    try
    {
        var synonyms = new SynonymMap(SynonymMapName, SynonymMapFormat.Solr,
...
1. Click on **FBI**, you will see two results containing that entity.
...
In this lab, we've learned how to leverage Azure Cognitive Search to explore a complex dataset such as the JFK Files. Postman has served us to the Azure Search REST API. We've seen how to add to our pipelines skillsets that recognize characters and code words or search images for faces or categories. This learning will empower you to process and understand difficult datasets with little effort.