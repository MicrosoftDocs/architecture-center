...
This architecture outlines various Azure-native migration tools that you can use depending on the mainframe source data and the target database.
...
- [Azure Data Factory](https://azure.microsoft.com/services/data-factory) is a hybrid data integration service you can use to create, schedule, and orchestrate ETL and ELT workflows. In this architecture, it is used to send mainframe files to Azure Blob storage via FTP.
...
- [Azure Logic Apps](/azure/logic-apps/logic-apps-overview): Azure Logic Apps offers a native IBM Host File connector that interacts with mainframe systems to read, parse, and generate host file content.
