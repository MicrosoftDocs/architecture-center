...
- [MSIX Packaging Extension](/windows/msix/desktop/msix-packaging-extension?tabs=yaml) is a series of Azure DevOps extensions that make it easier to integrate MSIX-related tasks, like packaging and signing, in your CI/CD pipelines.
- [Nerdbank.GitVersioning](https://github.com/dotnet/Nerdbank.GitVersioning) is an open-source tool that makes it easier to generate a version number every time a CI/CD pipeline is executed. By using this tool, you can easily increment the version number of each MSIX package that's generated by your pipeline, ensuring that it gets recognized as a valid update.
...
- [Azure Sign Tool](https://github.com/vcsjones/AzureSignTool) is an open-source tool that you can use to sign any type of binary, including MSIX packages, with a certificate that's stored in Key Vault. Because it's a command line tool, you can easily integrate it into a CI/CD pipeline.
