...
1. The proposal is to send data generated by **IoT Devices & Edge** to **Field Services** in Dynamics 365. The data from the sensors are sent to the **IotHub**.
1. **IotHub** will dispatch data to different services. In this architecture it is sending to **Stream Analytics** to store this data in a **Database**. Another path is to send data to the **Stream Analytics** that will filter the **Event** or **Message** using the rules stored in the **Blob Storage**.
1. If the device **Message** values are over the threshold defined in the **JSON Blob** a **Fault** will be created in **Field Services**
1. This **Message** is forwarded to a **Logic App** that will parse the message and connect to **Field Services** in Dynamics 365
1. A **Case/Work Order** is then created and an e-mail notification is sent. A custom action (like a **Command** to the IoT Device) can also be triggered
1. If a **Command** is triggered then this message is parsed to a format the **IoT Device** is able to handle
1. The **Command** is sent back to the device via **IoTHub**.
