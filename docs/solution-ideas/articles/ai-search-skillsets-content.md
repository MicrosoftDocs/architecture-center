...
- [Azure AI Vision](https://azure.microsoft.com/products/ai-services/ai-vision) is a cognitive service that extracts text and visual information from images using OCR and image analysis. In this architecture, it uses [text recognition](/azure/ai-services/computer-vision/overview-ocr) to extract and recognize text information from images. The [Read API](/azure/ai-services/computer-vision/overview-ocr#ocr-read-editions) uses the latest OCR recognition models and is optimized for large, text-heavy documents and noisy images.
...
- [Blob Storage](/azure/well-architected/service-guides/azure-blob-storage) is REST-based object storage solution optimized for large volumes of unstructured data. You can use Blob Storage to expose data publicly or to store application data privately. In this architecture, Blob Storage holds the original JFK Files dataset, including scanned documents and images, which are ingested into the AI enrichment pipeline.
