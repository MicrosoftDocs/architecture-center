---
title: IoT Telemetry Analytics with Azure Data Explorer
...
As this pattern serves both operational and analytical use cases, data can be routed to Azure Data Explorer and Cosmos DB in parallel or from Cosmos DB to Azure Data Explorer. 
   * Integrate Cosmos DB with Azure Data Explorer using change feed. Every transaction in Cosmos DB triggers Azure Function via change feed. Data is then streamed to Event Hub and ingested into Azure Data Explorer.
   * From Azure Function, invoke Azure Digital Twin (ADT) APIs, ADT is used for storing digital models of physical environments. ADT streams data to Event Hub, which then gets ingested to Azure Data Explorer.
1. To gain insights from data stored in Azure Data Explorer:
   ...
   * Create and schedule alerts and notifications using [Azure Data Explorer connector for Azure Logic Apps](https://docs.microsoft.com/azure/data-explorer/kusto/tools/logicapps).
