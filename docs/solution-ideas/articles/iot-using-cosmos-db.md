...
Cosmos DB is ideally suited for IoT solutions. Cosmos DB can ingest device telemetry data at extremely high rates and can serve indexed queries back with extremely low latency and high availability.

Cosmos DB is multi-model with wire protocol–compatible API endpoints for Cassandra, MongoDB, SQL, Gremlin, Etcd, and Table along with built-in support for Jupyter notebooks.
...
4. The storage layer consists of:
    ...
    - Azure SQL Database - Utilize Azure SQL for storing your transactional and relational data (e.g. billing data, user roles).
...
8. Some device messages (e.g. a fault code) may require an action to be performed on the device. Using the Azure IoT Hub Service API, the Azure Function can connect to Azure IoT Hub and perform an action on the device (e.g. reboot) using either:
    - Device Twins
...
- [Azure Functions](https://azure.microsoft.com/services/functions) can be used to translate IoT message payloads (e.g. from binary to JSON) or trigger actions when connected to Cosmos DB Change Feed. Azure Functions is an event-driven serverless compute platform. Build and debug locally without additional setup, deploy and operate at scale in the cloud, and integrate services using triggers and bindings.
...
- Cosmos DB has a [20GB limit](https://docs.microsoft.com/azure/cosmos-db/partitioning-overview) (in the past it was 10GB) for a single logical partition. For most IoT solutions, this size is sufficient. If not, we recommend either:
    - Setting the partition key to an artificial field and assign a composite value (e.g. Device ID + Current Month and Year). This will ensure an extremely high cardinality of values.
    - Tier old Cosmos DB data out to cold storage (e.g. Azure Blob Storage) using a combination of TTL to automatically prune data from Cosmos DB and change feed to replicate data to cold storage.
