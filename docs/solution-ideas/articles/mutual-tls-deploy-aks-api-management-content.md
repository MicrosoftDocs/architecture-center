...
1. API Management is deployed in internal virtual network mode. This mode is only available in the Developer or Premium tier. In this mode, it's assigned a private IP address. API Management receives traffic over HTTPS, using custom domain PFX certificates.
...
- **[Defender for Cloud](https://azure.microsoft.com/products/defender-for-cloud):** Defender for Cloud is a solution for cloud security posture management and cloud workload protection. It finds weak spots across your cloud configuration, helps strengthen the security of your environment, and can protect workloads across multicloud and hybrid environments from evolving threats. In this scenario, container images deployed in Container Registry and AKS are scanned by [Microsoft Defender for containers](/azure/defender-for-cloud/defender-for-containers-introduction).
...
- **[Log Analytics](/azure/azure-monitor/logs/log-analytics-overview):** You can use Log Analytics to edit and run log queries with data in Azure Monitor logs. In this scenario, diagnostic logs from Application Gateway, AKS, API Management, SQL Database, Azure Cosmos DB and others can be sent to log analytics workspace so that the logs can be analyzed based on requirements.
