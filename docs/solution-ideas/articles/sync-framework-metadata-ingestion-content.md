...
By using the information that's passed in the pivot classes, the import step [can create or update an entity](/rest/api/purview/catalogdataplane/entity/create-or-update-entities), or a [a glossary term](/rest/api/purview/catalogdataplane/glossary/create-glossary-term) via, for example, the REST API.
