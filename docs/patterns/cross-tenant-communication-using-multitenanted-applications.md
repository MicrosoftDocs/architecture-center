...
Securing multitenant communications in Azure can be challenging due to limitations inherent to many services. Azure managed identities can be used to obtain tokens from Microsoft Entra ID, eliminating the need to manage credentials directly. However, Azure managed identities don't work across tenant boundaries, and the typical alternative is to use shared secrets (like SAS URLs). The issue with shared secrets is they introduce a need to securely distribute and rotate secrets across Entra tenant boundaries. 

One option that avoids this overhead is to create a multitenant application to represent your workload's identity. Through a consent process, this workload identity can be made known to an external tenant, ultimately allowing it to authenticate to services in the external tenant.
...
This pattern can be reused for any multitenant scenario with various services that need to communicate across Entra tenant boundaries.
...
1. Flow 3: The customer applies RBAC roles to this new service principal so it is authorized to access the service bus.
   
1. Flow 4: The provider's Azure Function leverages the clientId + clientSecret of the app registration to send an authenticated message to the customer’s Service Bus queue.

1. Flow 5: The customer's Azure Function leverages a managed identity to read the provider's message from the queue via a Service Bus trigger.
...
A Provider has multiple Customers. The Provider and each Customer have their own individual Entra ID tenant and Azure resources. The Provider and each Customer need a secure, bidirectional method of communication. Messages must be exchanged through Azure Service Bus queues from Provider to Customer and vice versa. The solution should have a compelling identity story that avoids introducing unnecessary credentials or secrets.
...
We first need to create a multitenant application in the Provider's tenant and provision that identity (i.e., a service principal) within the Customer's tenant. The architecture image provided showcases how to setup and provision a service principal from the Provider's tenant into the Customer's tenant. This architecture also demonstrates the process of provisioning with multiple Entra tenant's. 
