...
So, the Provider needs to authenticate using an identity the Customer recognizes. That is, the Provider needs to authenticate to the Customer's Entra as a service principal the Customer knows about. There are two ways this SP can be created:
...
1. After the Multi-Tenant application has been created, navigate to "Clients&Secrets."
...
*Provisioning the Service Principal into another Entra tenant can also be achieved with an admin Graph API call.*
...
1. Select "User, group, or Service Principal" and click "+Select members" in the "Select" field type in the Service Principal name and select.
...
1. Review the configuration, and then click "Review + assign."
