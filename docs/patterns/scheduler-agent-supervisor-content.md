...
- The steps performed by an Agent could be run more than once. The logic that implements these steps should be [idempotent](https://en.wikipedia.org/wiki/Idempotence) or the system should contain [infrastructure to maintain consistency](https://docs.particular.net/nservicebus/outbox/#how-it-works).
