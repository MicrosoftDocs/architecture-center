...
- **Deployment process.** When deploying multiple stamps, it is highly advisable to have automated and fully repeatable deployment processes. Consider using [Resource Manager templates](/azure/azure-resource-manager/templates/overview), [Bicep](/azure/azure-resource-manager/bicep/overview) templates, or [Terraform](/azure/developer/terraform/overview) modules to declaratively define the stamp.
