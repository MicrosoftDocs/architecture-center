...
The event storage selected needs to support the event load generated by your application. Any storage append-only is eventually enough.   
Be careful if an event process reaction sends another event to avoid an infinite loop. 
