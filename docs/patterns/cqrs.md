...
If separate read and write databases are used, they must be kept in sync. Typically this is accomplished by having the write model publish an event whenever it updates the database - see the guidance around [event-driven architecture](../guide/architecture-styles/event-driven.md) for more information about using events. Updating the database and publishing the event must occur in a single transaction.
