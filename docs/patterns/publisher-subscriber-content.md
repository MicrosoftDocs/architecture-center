...
- **Delivery guarantees and duplicate messages.** Messaging systems offer different delivery guarantees, and each one carries trade-offs.
  ...
  If your broker doesn't provide deduplication, design subscribers to [handle messages idempotently](/azure/architecture/reference-architectures/containers/aks-mission-critical/mission-critical-data-platform#idempotent-message-processing). Different subscribers in the same workload might need different guarantees.
