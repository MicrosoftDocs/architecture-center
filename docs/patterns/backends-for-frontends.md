...
:::image type="complex" border="false" source="./_images/backend-for-frontend-example.svg" alt-text="Diagram that shows Azure BFF architecture with API Management handling cross-cutting concerns. Both mobile and desktop fetch data by using client-specific BFF Azure Functions." lightbox="./_images/backend-for-frontend-example.svg":::
...
- [API Management](/azure/well-architected/service-guides/api-management/operational-excellence) serves as a proxy between the clients and their BFFs that add a perimeter. It's configured with policies to [validate the JSON Web Tokens](/azure/api-management/validate-jwt-policy) and rejects requests that arrive without a token or the claims aren't valid for the targeted BFF. It also streams all the activity logs to Azure Monitor.
