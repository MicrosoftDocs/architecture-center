...
- The workload must continue operating during network isolation or startup-time dependency failures, and you cannot reliably ship and maintain a local last-known-good configuration fallback. In that case, runtime dependency on an external store increases availability risk.
...
While building a custom configuration store might be necessary in some situations, many applications can instead use [Azure App Configuration](/azure/azure-app-configuration/overview). Azure App Configuration supports [key-value pairs](/azure/azure-app-configuration/concept-key-value) that can be namespaced. The keys are typed and are individually versioned. Azure App Configuration also supports [immutable snapshots](/azure/azure-app-configuration/concept-snapshots) of configuration so that you can inspect, roll back, or progressively deploy configuration changes without risk to running instances. Use [snapshot references](/azure/azure-app-configuration/concept-snapshot-references) to let applications switch between snapshots at runtime without code changes or redeployment. Configuration values can be exported so that a copy ships with your application as a startup fallback if the service is unreachable when the application starts.
