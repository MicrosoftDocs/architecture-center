...
- Virtual machines: Deploy a single set of virtual machines for all tenants to use, that way the management costs are shared accross the tenants. Virtual Machine Scale sets is a feature that automatically allows shared resource management, load-balancing, scaling, application's highly availability with no extra cost.
