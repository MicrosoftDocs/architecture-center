...
![Screenshot showing the use of the Cache-Aside pattern to read and store data in the cache.](./_images/cache-aside-diagram.svg)
...
**Semantic caching**. Some workloads can benefit from doing cache retrieval based on semantic meaning rather than exact keys. This reduces the number of requests and tokens sent to language models. Be sure the data being cached benefits from semantic equivalence and doesn't risk returning unrelated responses or contain private and sensitive data. For example, "What is my yearly take home salary?" is semantically similar to "What is my yearly take home pay?" but if asked by two different users so the answer shouldn't be the same, nor would you want to include this sensitive data in your cache.
