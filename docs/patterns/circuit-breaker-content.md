...
:::image type="complex" source="./_images/cb-example.png" alt-text="Diagram showing CosmosDB and a circuit breaker implementation in Azure App Service.":::
   In this architecture, a client sends requests an Azure App Services public endpoint where a hosted web application incorporates circuit breaker logic directly within its code. Under normal conditions—when the circuit breaker is in its closed state—the application process all the requests and query an Azure Cosmos DB instance, which handles non-critical data. However, should a 429 (Too Many Requests) response be encountered, the circuit breaker immediately transitions into its open state, causing subsequent requests to be short-circuited so that default or cached responses are returned to the client, thereby preventing further strain on Azure Cosmos DB by letting the instance to recover. After a predetermined timeout, the circuit breaker enters a half-open state, during which a limited number of trial requests are permitted; if these trials succeed without triggering additional 429 responses, the breaker resets to the closed state, restoring normal operations, whereas continued failures will revert to the open state. Throughout this process, Azure Monitor continuously collects logs and telemetry data from the Azure resources providing real-time insights in Azure Monitor alerts rules that enable proactive capacity management and swift intervention involving approvers from the workload team and the operation team upon approval to assess when to scale the database.
:::image-end:::
