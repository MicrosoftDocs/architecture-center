...
Consider a workload that has 100s of mobile clients frequently uploading large binaries. Without this pattern, the workload has essentially two options 1) to provide standing access and configuration to all the mobile clients to perform uploads directly to a storage account or 2) set up an API where clients authenticate and act as a bottle necked proxy to that storage, without adding additional value to the transaction. Both suffer problems addressed in the pattern context:

- Long lived, pre-shared secrets. Potentially without much way to provide different keys to different clients.
- Added cost for running a compute service that needs to have sufficient memory to deal with currently receiving large files.
- Potentially slowing up the mobile clients work by adding an extra layer of compute and network hop to the upload process.
...
1. The valet key pattern in this example allows clients to, just in time, authenticate to a light weight, scale-to-zero Azure Function hosted API.
1. The API then obtains and returns a time & scope limited SaS token.

   The token generated by the API restricts the mobile client to the following:
   ...
   - A short, window of operation, such as three minutes. This ensures that tokens have a TTL that doesn't extend past its utility.
   - Permissions to only _create_ a blob; not download, update, or delete.

1. That token is then used by the mobile client, within the narrow time window, to upload the file directly to the storage account.
...
This is the data transfer object (DTO) described by both the API and the client.
