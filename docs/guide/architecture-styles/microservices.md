...
:::image type="complex" border="false" source="./images/microservices-logical.svg" alt-text="Logical diagram of microservices architecture style." lightbox="./images/microservices-logical.svg":::
   The diagram illustrates a microservices architecture deployed on Microsoft Azure. It's organized into eight labeled sections that each represent a key architectural component. The layout flows from left to right and top to bottom. On the far left, icons labeled clients represent users or external systems that initiate requests to the application. An arrow points from the clients to API Gateway. Requests flow through API Gateway and on to the appropriate microservices. Arrows point from API Gateway to a box labeled microservices. This box contains two icons for domain services, one icon for composition services, and one icon for service. Arrows point from the microservices to another box that's labeled data persistence. This box contains icons that represent SQL SB, NoSQL DB, and SQL DB. Arrows also point from microservices to a box that represents event streaming and messaging services. The diagram also includes boxes that represent observability, management and orchestration, and DevOps.
:::image-end:::
...
**API Gateway.** The API gateway is the entry point for clients. Instead of calling services directly, clients call the API gateway, which forwards the call to the appropriate services on the back end. The API gateway handles common cross-cutting concerns like authentication, logging, and load balancing.

**Messaging infrastructure/platforms.** Messaging systems like Apache Kafka and Azure Service Bus enable asynchronous communication in microservices by promoting loose coupling and supporting high scalability. Messaging systems are foundational to event-driven architectures because they allow services to react to events in real-time and communicate via asynchronous messaging.

**Observability.** Effective observability strategies are crucial in microservices because they help teams maintain system reliability and quickly resolve problems. Centralized logging (Azure Monitor/Log Analytics) consolidates logs for easier diagnostics, while real-time monitoring (Azure Application Insights, Prometheus/Grafana) provides visibility into system health and performance. Distributed tracing like OpenTelemetry is also essential for tracking requests across service boundaries, helping identify bottlenecks and optimize performance.

**Database architecture.** A well-designed database architecture is critical for microservices, supporting autonomy and scalability. Microservices often adopt polyglot persistence by using different types of databases, like SQL and NoSQL document stores, based on each service's specific needs. This approach aligns with the principle of DDD/bounded context, where each service owns its data and schema, reducing cross-service dependencies and enabling independent evolution. This approach enhances flexibility, performance optimization, and resilience across the system.
...
- **Data integrity.** Each microservice responsible for its own data persistence. As a result, data consistency across multiple services can be a challenge. Different services persist data at different times, using different technology, and with potentially different levels of success. When more than one microservice is involved in persisting new or changed date, it's unlikely that the complete data change could be considered an ACID transaction. Instead, the technique is more aligned to BASE (Basically Available, Soft state, and Eventually consistent). Embrace eventual consistency where possible.
