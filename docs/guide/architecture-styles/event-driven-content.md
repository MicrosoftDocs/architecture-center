...
- **Include all required attributes in the payload:** Use this approach when you want consumers to have all available information without the need to query an external data source. However, it can lead to data consistency problems because of multiple [systems of record](https://wikipedia.org/wiki/System_of_record), specifically after updates. Contract management and versioning can also become complex.
...
- High volume and high velocity of data, such as IoT, is required.
...
- Event processing in order or exactly once.
