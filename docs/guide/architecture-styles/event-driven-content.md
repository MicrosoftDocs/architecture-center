...
- Implementing a traditional request-response pattern.
  ...
  - **Establish two permanent queues**; a request queue and a reply queue. The event producer sends an asynchronous request to the request queue, pauses other operation on that task, and waits for a response in the reply queue. Event consumers then process the request and send the reply back through the preestablished reply queue. The approach utilizes a session ID for tracking so the event producer knows which message in the response queue is related to the specific request. This technique is a common implementation.
  
  - **Create an ephemeral queue for each response.** This version behaves like the prior solution, but instead of having a preexisting response queue established, this approach uses an ephemeral queue that is exclusively recognized by the event producer for a particular request. The event producer can either create this ephemeral queue or it may be generated automatically by the message broker. The event producer sends the initial request to the queue, specifying the name of the ephemeral queue in a [reply-to header](/dotnet/api/azure.messaging.servicebus.servicebusmessage.replyto) or another mutually agreed-upon custom attribute. Any messages directed to this queue are expected to be solely for the event producer that initiated them. Once the event consumer receives the message, processes the request, and sends a response to the reply queue indicated in the message header. The original event producer retrieves this response and subsequently deletes the ephemeral queue. This approach can lack scalability because a dedicated queue must be created and deleted for each request. Also, this solution could run into service limits on queue creation, rate or quantity.
