...
- While a request is only visible to the receiving component, events are usually visible to all components in the system, even if they do not or are not meant to consume them. Be careful what information you include in events, since they are published to the entire system.
