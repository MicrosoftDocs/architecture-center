...
- Generating an excessive number of fine-grained events can saturate and overwhelm the system, making it difficult to analyze the overall workflow effectively. This issue is exacerbated when changes need to be rolled back. Conversely, overly consolidating events can also create problems, resulting in unnecessary processing and responses from event consumers. To address these challenges, it is essential to focus on the outcomes of events rather than individual state changes. Additionally, if event consumers must inspect event payloads to determine whether to respond and take action, this often indicates that more distinct events are needed.<br /> For example, consider a "food delivery service" as an event producer with three event consumers: the "kitchen management system," the "customer notification system," and the "delivery tracking system." In this scenario, it may be sufficient for the food delivery service to publish only two types of events: "order ready" and "order delivered." Each event would then be sent only to the relevant consumers, preventing unnecessary processing and minimizing the volume of generated events.
