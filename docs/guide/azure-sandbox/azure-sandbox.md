...
:::image type="complex" border="false" source="images/create-azure-sandbox.svg" alt-text="Diagram that shows the Azure Sandbox environment." lightbox="images/create-azure-sandbox.svg":::
   The diagram contains multiple sections arranged vertically. The top section shows an Azure subscription boundary. Inside that boundary is a rectangle that encloses most of the image elements that reads rg-sandbox-01. A Log Analytics workspace is included in this section. There are two rectangles within the subscription and three smaller rectangles near the bottom. The left rectangle labeled vnet-shared-01 (10.1.0.0/16) contains an Azure Bastion host, Azure Firewall, domain controller (DC) and Domain Name System (DNS), and vault icons. The right rectangle is labeled vnet-app-01 (10.2.0.0/16) and contains two jump boxes, a SQL VM, blob, file, sqlServer, and mysqlServer icons. A bidirectional arrow labeled peering connects the left and right rectangles. A line labeled public IP address connects the Azure Bastion host to the cloud. A line labeled public IP address connects Azure Firewall to the cloud. A line labeled point-to-site VPN connects vwan-xx (10.3.0.0/16) to the cloud. A bidirectional arrow labeled peering connects vwan-xx to vnet-shared-01. A bidirectional arrow connects key vault and the vault. At the bottom are three rectangles that contain icons labeled vhub-xx (10.4.0.0/16), testdb (SQL), and testdb (My). An arrow labeled peering connects vhub-xx to vnet-app-01. Arrows connect both testdb components to snet-privatelink-01. A bidirectional arrow connects snet-privatelink-01 (10.2.2.0/24) to an Azure Storage account and a container registry.
:::image-end:::
...
- Configure an explicit outbound path for sandbox resources, for example by routing egress through Azure Firewall or by using Network Address Translation (NAT) as appropriate for your scenario. If you use Azure Firewall for outbound traffic, enable [Azure Firewall threat intelligence-based filtering](/azure/firewall/threat-intel).
