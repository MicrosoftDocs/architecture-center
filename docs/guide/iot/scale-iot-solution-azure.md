...
# customer intent: As a solution architect, I want to understand how to scale an Azure IoT Hub-based solution so that I can support millions of devices while staying within Azure service and subscription limits.
---

# Scale out an Azure IoT Hub solution to support millions of devices

The scale-out pattern is an approach to scaling an Internet of Things (IoT) solution that adds instances to a deployment instead of increasing the size of each instance. On the Azure IoT Hub platform, you can scale out to support millions of devices while staying within Azure service and subscription limits.

This article shows you how to apply the scale-out pattern to an IoT Hub-based solution. It outlines the low-touch and zero-touch deployment models so that you can adopt the approach that fits your needs.
...
**Determine the number of devices that you need to deploy.** Some basic principles of implementing IoT solutions apply at all scales. Understand the scale to help avoid overengineering a solution. A solution for 1,000 devices differs fundamentally from a solution for 1 million devices. A proof-of-concept (PoC) solution for 10,000 devices might not scale appropriately to 10 million devices if you don't consider the target scale at the start of the solution design.
...
## Understand Azure IoT Hub and DPS concepts
...
- The [size](/azure/iot-hub/iot-hub-scaling#choose-your-size-editions-and-units) determines the message and data throughput base unit for D2C messages for IoT Hub. The maximum size for an instance of IoT Hub is size 3, which supports 300 million messages per day and 1,144.4 GB of data per day, per unit.
...
IoT Hub DPS is a helper service for IoT Hub that provides zero-touch, just-in-time provisioning to the right IoT hub without human intervention. Each Azure subscription supports a default of [10 DPS instances](/azure/iot-dps/about-iot-dps#quotas-and-limits).
Each service instance in turn supports a default of [1 million registrations](/azure/iot-dps/about-iot-dps#quotas-and-limits). Address service limits in your workload design to avoid future problems.

DPS instances reside in specific geographic regions but have a [global public endpoint](/azure/iot-dps/virtual-network-support#private-endpoint-limitations) by default. You access specific instances through an ID scope. Because instances are in specific regions and each instance has its own ID scope, you can configure the ID scope for your devices.

By default, DPS leverages geo-replication to enable region-level disaster recovery. For more information, see [DPS documentation around disaster recovery](/azure/iot-dps/iot-dps-ha-dr).
...
**Understand transient fault handling.** Any production distributed solution, whether it's on-premises or in the cloud, must recover from transient or temporary faults. Transient faults might occur more frequently in a cloud solution because of the following factors:
...
Transient fault handling requires you to build a retry capability into your device code. Several retry strategies exist, including exponential backoff with randomization, also known as *exponential backoff with jitter*. For more information, see [Transient fault handling](../../best-practices/transient-faults.md).
...
Beyond network and quota problems, you should also consider Azure service outages. These outages might affect individual services or entire regions. Some services, such as IoT Hub and IoT Hub DPS, support some form of geo-redundancy. You can link one IoT Hub to multiple DPS instances and one DPS instance supports multiple IoT Hub instances; both of these help with mitigating, but do not fully resolve, regional risks. If full regional high availability is a concern, use the [Geode pattern](/azure/architecture/patterns/geodes). This pattern hosts independent, grouped resources across different geographies. Similarly, a deployment stamp, also known as a *scale stamp*, applies this pattern to operate multiple workloads or tenants. For more information, see [Deployment Stamps pattern](/azure/architecture/patterns/deployment-stamp). These patterns also help resolve the concern that IoT Hub DPS does not do health monitoring on linked IoT Hubs, because a geographically dispersed deployment with an appropriate health monitoring solution and load-balancing solution allows routing to a different DPS instance and thus a different IoT Hub instance.
...
- **Data location:** Data location depends on device location and is subject to compliance concerns. For example, a solution that stores data for a state in the United States might require data storage in the US [geography](https://azure.microsoft.com/explore/global-infrastructure/geographies). Data locality requirements might also drive this need.
...
Although it's possible to track tenant usage at a higher level than the service level, it's often easier to segregate the cost of goods sold (COGS) based on customer data segregation for each SaaS provider's customer. This distinction is particularly important when the end user isn't the same as the SaaS customer. For example, for a smart TV platform, the platform vendor's customer might be the television vendor, while the end user is the television purchaser. The required customer tenancy model drives this segregation, which in turn often leads to separate DPS and IoT Hub instances for simplicity (at the risk of a potentially higher cost for the deployment). It should be noted that this segregation is specifically for tracking COGS; it's not necessary for just tenant isolation, as discussed at [IoT Hub-based multitenant solution architectural approaches](/azure/architecture/guide/multitenant/approaches/iot#tenant-isolation).

The provisioning service must also have a unique customer identity, which you can define through a unique endpoint or device authentication process. For more information, see [IoT multitenant guidance](../multitenant/approaches/iot.md).

## Scale out components and their supporting services

When you scale IoT solutions, evaluate each service and how they interrelate. Scale your IoT solution across multiple DPS instances and/or across multiple IoT Hub instances.

### Scale out across multiple DPS instances

Because of DPS service limits, you often need to expand to multiple DPS instances. Approach device provisioning across multiple DPS instances through either zero-touch or low-touch provisioning.

The following approaches apply the previously described *stamp* concept for resiliency and scaling out. This concept includes deploying Azure App Service in multiple regions and using a tool such as [Traffic Manager](/azure/traffic-manager/traffic-manager-overview) or [Azure Front Door](/azure/frontdoor/). For simplicity, the following diagrams don't show these components.
...
The diagram has two scale stamps that each include a DPS pool and an IoT Hub pool. The DPS pool includes three instances. The IoT Hub pool includes four instances. A two-sided arrow points from one instance in each pool to the device. A two-sided arrow also points from the device to a provisioning API.
:::image-end:::

1. The device requests a DPS ID scope from a provisioning API hosted in App Service. The provisioning API checks with its persistent database to determine the best instance for the device, based on existing device inventory, and returns the DPS ID scope.

   In this example, the database is an Azure Cosmos DB instance that has multi-primary write enabled for cross-region high availability. This database stores each device's assigned DPS. It supports tracking DPS instance usage for all appropriate metrics, such as provision requests per minute and total provisioned devices. This database also supports reprovisioning by using the same DPS ID scope when needed. Authenticate the provisioning API to prevent inappropriate provisioning requests.

1. The device requests provisioning from DPS by using the assigned ID scope. DPS responds with IoT hub assignment details.
...
The second design moves the DPS call to the provisioning API. In this model, the provisioning API contains the device authentication against DPS, along with most of the retry logic. This process supports more advanced queuing scenarios and potentially simpler provisioning code in the device itself. It also supports caching the assigned IoT hub to facilitate faster C2D messaging without needing to interrogate DPS for the assigned hub information.
...
1. The device sends a request to a provisioning API hosted in an instance of App Service. The provisioning API checks with its persistent database to determine the best instance for the device based on existing device inventory, and then it determines the DPS ID scope.

   In this example, the database is an Azure Cosmos DB instance that has multi-primary write enabled for cross-region high availability. This database stores each device's assigned DPS. It supports tracking DPS instance usage for all appropriate metrics. The database also supports reprovisioning by using the same DPS ID scope when needed.

   Authenticate the provisioning API to prevent inappropriate provisioning requests. You can likely use the same authentication that the provisioning service uses against DPS, such as a private key for an issued certificate. But other options exist. For example, a solution might use hardware unique identifiers as part of a service authentication process. The device manufacturing partner regularly supplies a list of unique identifiers to the device vendor to load into a database, which references the service behind the custom provisioning API.
...
This design avoids the need to reference the DPS SDK or the DPS service directly. It also avoids the need to manually maintain a DPS scope on the device as it comes from the provisioning API. This model supports transfer-of-ownership scenarios because the provisioning service can direct to the appropriate final customer DPS instance. However, this approach causes the provisioning API to duplicate some DPS functionality, which might not suit all scenarios.

#### Approach 3: Zero-touch provisioning with transfer of ownership

A third zero-touch provisioning design uses a factory-configured DPS instance as a starting point and redirects devices to other DPS instances as necessary. This design supports provisioning without a custom provisioning API, but it requires a management application to track DPS instances and supply redirection as necessary.

The management application requirements include tracking which DPS should be the active DPS for each specific device. Use this approach for transfer-of-ownership scenarios, where the device vendor transfers ownership of the device to the end device customer.
...
## Design device software for scale

Scalable device design requires following best practices and device-side considerations. Some of these practices come from anti-patterns that appear in the field. This section describes key concepts for a successfully scaled deployment.
...
**Check provisioning status if you must reprovision often, for example, at every device boot.** If you're unsure about the device provisioning status, [query the provisioning status](/rest/api/iot-dps/data-plane/runtime-registration/device-registration-status-lookup) first. The query operation uses a different quota than a provisioning operation and is faster. With this query, the device can validate the provisioning status before proceeding. This approach is especially useful when a device doesn't have available persistent storage to store the provisioning results.

**Ensure a good retry logic strategy.** The device must have appropriate retry algorithms built into the device code for both initial provisioning and later reprovisioning. Use techniques such as *exponential backoff with randomization*. Initial provisioning, by definition, might need to be more aggressive in the retry process than reprovisioning, depending on the use case. When throttled, DPS returns an [HTTP 429 Too Many Requests](https://http.cat/429) error code, like most Azure services. That response will sometimes include the `Retry-After` header to help guide the backoff timing. For more information, see [Retry](../../patterns/retry.yml) and [Anti-patterns to avoid](../../example-scenario/iot/iot-move-to-production.md#use-a-back-off-approach-when-a-transient-fault-occurs). The DPS documentation also explains how to interpret service retry recommendations and calculate jitter. The device location stability and connectivity access also influence the appropriate retry strategy. For example, if a device detects that it's offline for a period of time, it should avoid retrying online operations.

**Support over-the-air (OTA) updates.** Two simple update models include device twin properties with [automatic device management](/azure/iot-hub/iot-hub-automatic-device-management) and simple device commands. For more sophisticated update scenarios and reporting, see [Azure Device Update](/azure/iot-hub-device-update/). With OTA updates, you can fix defects in device code and reconfigure services, such as DPS ID scope, if necessary.

**Architect for certificate changes at all layers and certificate uses.** This recommendation aligns with the OTA update best practices. You must consider certificate rotation. The IoT Hub DPS documentation addresses this scenario [from a device identity certificate](/azure/iot-dps/how-to-roll-certificates) viewpoint. In a device solution, you use other certificates to access services like IoT Hub, App Service, and Azure Storage accounts. Azure sometimes changes certificate authority configurations, so you must anticipate changes at all layers. Also, use certificate pinning with caution, especially when certificates are outside the device manufacturer's control.

**Consider a reasonable default state.** To resolve initial provisioning failures, have a reasonable disconnected or unprovisioned configuration, depending on the circumstances. If the device has a heavy interaction component as part of initial provisioning, the provision process can occur in the background while the user performs other provisioning tasks. Always pair this approach with an appropriate retry pattern and the [Circuit Breaker pattern](../../patterns/circuit-breaker.md).
...
**Use the Azure IoT SDKs for provisioning.** Whether the DPS calls are on the device itself or in a custom provisioning API, use the Azure IoT SDKs to benefit from built-in best practices and simplify support. The SDKs are open source, so you can review how they work and suggest changes. Your choice of SDK depends on the device hardware and available runtimes on the device.

## Deploy devices

Device deployment is a key part of the device life cycle, but it's outside the scope of this article because it depends on the use case. The previously referenced discussion points about transfer of ownership might apply to the deployment and patterns that involve a provisioning application, such as a mobile application. But you must select the deployment approach based on the IoT device type in use.

## Monitor devices

An important part of your overall deployment is to monitor the solution from start to finish to ensure that the system performs appropriately. This article explicitly focuses on architecture and design and not the operational aspects of the solution, so it doesn't cover monitoring in depth. However, at a high level, Azure provides built-in monitoring tools through [Azure Monitor](/azure/azure-monitor/) to help an operator ensure that the solution doesn't reach limits. For more information, see the following articles:
...
Use these tools individually or as part of a more sophisticated security information and event management (SIEM) solution like [Microsoft Sentinel](/azure/sentinel/overview).
...
- Create an application that queries each enrollment group on a DPS instance, retrieves the total devices registered to that group, and then aggregates the numbers from across various enrollment groups. This method provides an exact count of devices currently registered through DPS and helps monitor the state of the service.

- Monitor device registrations over a specific period. For example, you can monitor registration rates for a DPS instance over the previous five days. This approach provides only an approximate figure and applies only to a set time period.

## Key takeaways for scaling an IoT solution

Scaling out an IoT solution to support millions, or even hundreds of millions, of devices requires careful planning. You must consider many factors and various ways to solve the problems that arise at those scales. This article summarizes the key concerns and provides approaches to address those concerns in a successful deployment.
