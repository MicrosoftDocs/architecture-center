...
Then, choose an encoding format. The factors that might influence your choice may depend the ability to create highly structured data if you need it, time taken to encode and transfer the message, and the ability to parse the payload. 
...
To transfer messages, the producer serializes the message to an encoding format. At the receiving end, the consumer deserializes the payload to use the data. This way both entities share the model and as long as the shape doesn’t change, messaging continues without issues. When the contract changes, the encoding format should be able to handle the change without breaking the consumer.
...
With text-based encoding, the message payload is in plain text and therefore easily parsed. Because the shape always resides with the payload, and the consumer doesn’t have to decipher the payload. That ability makes it suitable for archival data.Also, because you can read the payload, it’s easy to debug and send to logs for troubleshooting errors.

The downside is that the payload tends to be larger. A common text-based format is JSON.

Binary formats are compact and require serialization/deserialization libraries. The payload cannot be read unless its decoded.
...
The disadvantage is that the payload cannot be is not human readable. Most binary formats use complex systems that can be costly to maintain. Also, they need specialized libraries to decode, which may not be supported if you want to retrieve archival data.  
...
- Tagged metadata
    ...
    The producer writes the payload that contains metadata fields that describe the data. There is no schema at the producer’s end. 
...
Schema stored with the payload yields larger encoding size and is preferred for intermittent messages. Choose the schema approach if transferring smaller chunks of bytes is crucial or you expect a sequence of records. Be aware that the cost of maintaining an external schema store can be high.
...
- Changes musn’t affect or break the business logic of consumers.
...
-	Tabular data: Information is divided into rows and columns. Each column indicates a field, or the subject of the information and each row contains values for those fields. This layout is efficient for a repeating set of information, such as time series data. 
    ...
    For a text-based format, the simplest format is CSV. It presents data as a sequence of records with a common header. For binary encoding, Apache Avro has a preamble is similar to a CSV header but generate very compact encoding size. 
...
JSON can be used for tagging metadata and the payload can be parsed without a schema. JSON can also be used to define a schema with the option to specify optional fields which helps with forward and backward compatibility. 

The biggest advantage is that its universally available. It’s most interoperable and the default encoding format for many messaging services such as Azure Service Bus and Event Grid. 

Being a text-based format, it is not very efficient over the wire and not an ideal choice in cases where storage is a concern. If you are returning cached items directly to a client via HTTP, storing JSON could save the cost of deserializing from another format and then serializing to JSON.
...
The message contains a compressed binary small payload, which results is faster transfer. The downside is the payload is not human readable. Also, because the schema is external, it’s not recommended for cases where you have to retrieve archived data. 

### Apache Avro
[Apache Avro](https://avro.apache.org/) is a binary serialization format that uses definition file similar to protobuf but there is no compilation step. Instead, serialized data always includes a schema preamble.
...
### MessagePack
[MessagePack](https://msgpack.org/) is a binary serialization format that is designed to be compact for transmission over the wire. There are no message schemas or message type checking. This is not recommended for bulk storage. 

### CBOR
[Concise Binary Object Representation](http://cbor.io/) (CBOR) (Specification) is a binary format that offers very small encoding size. The advantage of CBOR over MessagePack is that its compliant with IETF in RFC7049.
