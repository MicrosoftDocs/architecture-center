...
- **Web application? (HTTP/HTTPS)** specifically refers to the capability to make making a routing decision for Layer 7 data (such as URL path), supporting the inspection of the communication payload (such as an HTTP request body), or handling TLS functionality.
