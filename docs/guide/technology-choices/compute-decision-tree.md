...
All of the application platforms that this decision guide addresses are regional. They require external routing to enable [multiregion topologies](/azure/architecture/high-availability/traffic-manager-application-gateway), such as active-active for resiliency or active-passive for recoverability. When you deploy multiple instances of the application platform, with at least one instance in each region, an external router or load balancer can direct traffic where needed across regions.
