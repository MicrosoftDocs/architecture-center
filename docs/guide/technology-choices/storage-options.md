...
- **Do your workloads require disk storage to support the deployment of infrastructure as a service (IaaS) virtual machines?** [Azure Managed Disks](/azure/virtual-machines/managed-disks-overview) provide virtual disk capabilities for IaaS virtual machines.
  ...
  **Do you need to consolidate block storage across multiple compute resources, or are you migrating an on-premises SAN?** [Azure Elastic SAN](/azure/storage/elastic-san/elastic-san-introduction) is an Azure-based storage area network that provides network-attached block storage over the iSCSI protocol. It connects to Azure Virtual Machines, Azure Kubernetes Service, and Azure VMware Solution. Elastic SAN pools performance at the SAN level and shares it across all volumes, which can be more cost effective than provisioning individual disks when you have many IO-intensive workloads. For single-VM workloads or scenarios that require locally attached storage, use Azure Managed Disks instead.
...
| I have an enterprise SAN or all-flash array. | [Azure Elastic SAN](/azure/storage/elastic-san/elastic-san-introduction), [Premium SSD or Azure Ultra Disk Storage](/azure/virtual-machines/disks-types), or [Azure NetApp Files](/azure/azure-netapp-files/azure-netapp-files-introduction) | Azure Elastic SAN provides a cloud-native SAN experience with pooled performance across volumes over iSCSI. Ultra Disk Storage is NVMe-based and offers submillisecond latency with high IOPS and bandwidth. Ultra Disk Storage is scalable up to 64 TiB. The choice depends on whether you need consolidated network-attached storage (Elastic SAN), locally attached high-performance disks (Premium SSD or Ultra Disk Storage), or file-based shares (Azure NetApp Files). |
| I have high-availability clustered servers, such as SQL Server FCI or Windows Server failover clustering. | [Azure Elastic SAN](/azure/storage/elastic-san/elastic-san-introduction), [Azure Files](/azure/storage/files/storage-files-planning#storage-tiers), or [Premium SSD or Ultra Disk Storage](/azure/virtual-machines/disks-types) | Clustered workloads require multiple nodes to mount the same underlying shared storage for failover or high availability. Azure Elastic SAN provides shared block storage over iSCSI that multiple nodes can access. Premium file shares offer shared storage that's mountable by using SMB. Shared block storage also can be configured on Premium SSD or Ultra Disk Storage by using partner solutions. See [SIOS DataKeeper Cluster Edition](https://marketplace.microsoft.com/product/sios_datakeeper.sios-datakeeper-8). |
| I have a relational database or data warehouse workload, such as SQL Server or Oracle. | [Premium SSD or Ultra Disk Storage](/azure/virtual-machines/disks-types) or [Azure Elastic SAN](/azure/storage/elastic-san/elastic-san-introduction) | The choice of Premium SSD or Ultra Disk Storage depends on peak latency, IOPS, and scalability requirements. Ultra Disk Storage also reduces complexity by removing the need for storage pool configuration for scalability. For multiple consolidated database workloads, Azure Elastic SAN can share pooled performance across volumes and reduce overall cost, but it's [not typically cost effective for a single database workload](/azure/azure-sql/virtual-machines/windows/performance-guidelines-best-practices-storage#azure-elastic-san). See [Mission critical performance](https://azure.microsoft.com/blog/mission-critical-performance-with-ultra-ssd-for-sql-server-on-azure-vm/). |
...
| I need to move an on-premises application that uses a local disk or iSCSI. | [Azure disk storage](/azure/virtual-machines/managed-disks-overview) or [Azure Elastic SAN](/azure/storage/elastic-san/elastic-san-introduction) | Use Azure disk storage for locally attached block storage. For iSCSI-based workloads or on-premises SAN migrations, Azure Elastic SAN provides a cloud-native SAN with iSCSI connectivity that can reduce the need to refactor your application architecture. |
...
| [Azure Elastic SAN](/azure/storage/elastic-san/elastic-san-introduction) | Azure Elastic SAN is an Azure storage area network that provides network-attached block storage over the iSCSI protocol. It enables you to provision a SAN, organize volumes into volume groups, and share pooled performance across all volumes. Elastic SAN connects to Azure Virtual Machines, Azure Kubernetes Service, and Azure VMware Solution. Elastic SAN supports locally redundant storage (LRS) and zone-redundant storage (ZRS), encryption at rest, and private endpoints. |
...
[Encryption at host](/azure/virtual-machines/disk-encryption#encryption-at-host---end-to-end-encryption-for-your-vm-data) provides end-to-end encryption for your virtual machine data. It ensures that temp disks, disk caches, and data flowing between compute and storage are all encrypted. Unlike Azure Disk Storage server-side encryption, which only encrypts data at rest on the storage clusters, encryption at host encrypts data from the VM through to the storage layer. You can use platform-managed keys or customer-managed keys with [Azure Key Vault](/azure/key-vault/). For more information, see [Overview of managed disk encryption options](/azure/virtual-machines/disk-encryption-overview).
...
ZRS availability for Elastic SAN is limited to a subset of those regions, and maximum scale targets (IOPS, throughput, and base storage capacity) [vary by region](/azure/storage/elastic-san/elastic-san-scale-targets).
