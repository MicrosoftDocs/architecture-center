...
**Partition state and use immutable states**. Leverage parallelism of multi-core servers by partitioning your application’s state into smaller chunks which are managed independently from each other. Having small and independent state chunks helps with fault-tolerance and scalability because while each node can only store and process a finite dataset, a network of them can handle larger computational problems. On the other hand, having small and independent state chunks can come at the cost of consistency and simplicity. Ideally, those state chunks should be immutable whenever possible but if you have to use mutable state, isolate and contain it using the [bulkhead pattern](../../patterns/bulkhead.yml) to minimize the chance of cascading failure. Ideally, your bulkhead should have a single-threaded execution (e.g. Node.js offers this functionality) to avoid concurrency-related problems such as deadlocks, race conditions, and corrupt data.
