...
**Abstract infrastructure away from domain logic**. Don't let domain logic get mixed <<RBC: This word is only on MSDN 3 times in library docs. Does this replacement work? If not something that will be more straightforward for ESL readers would be better. This does not mean I don't love that you used this word, because I do!>> with infrastructure-related functionality, such as messaging or persistence. Otherwise, changes in the domain logic will require updates to the infrastructure layers and vice versa. 
