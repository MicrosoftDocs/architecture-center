...
When you work with a multitenant system that uses Azure Managed Redis, you need to determine the level of isolation that you want to use. Azure Managed Redis supports several isolation models.
...
> [!NOTE]
> Azure Managed Redis doesn't support multiple databases per instance, and custom data access policies with key-prefix patterns are not currently available. As a result, there are fewer isolation models compared to Azure Cache for Redis. If you use Azure Cache for Redis and rely on those features, see [Multitenancy and Azure Cache for Redis](cache-redis.md).

### Shared cache instance

You might consider deploying a single cache and using it to store cached data for all of your tenants. This approach is commonly used when you have a single application instance that all of your tenants share.
...
Azure Managed Redis uses [private endpoints](/azure/redis/private-link) for network isolation. VNet injection isn't supported. If your solution requires per-tenant network isolation, each tenant needs a separate cache instance with its own private endpoint.
...
Many multitenant solutions need to be geo-distributed. You might share a globally distributed application tier. In this scenario, your application instances read from and write to a nearby cache to maintain acceptable performance. Azure Managed Redis supports [linking multiple caches together across regions in an active-active configuration](/azure/redis/how-to-active-geo-replication) on Memory Optimized, Balanced, and Compute Optimized tiers.

> [!NOTE]
> Active geo-replication isn't available on the Balanced B0 and B1 SKUs or on the Flash Optimized tier. You must enable active geo-replication when you create the cache instance. Caches without active geo-replication can't be added to a geo-replication group later. Only the RediSearch and RedisJSON modules are compatible with active geo-replication.

### Redis modules

Azure Managed Redis supports [Redis modules](/azure/redis/redis-modules), which extend the core Redis data structures with functionality that can benefit multitenant solutions:

- **RediSearch** provides full-text search, secondary indexing, and [vector similarity search](https://redis.io/solutions/vector-search/). In a multitenant context, you can create per-tenant search indexes within a shared cache instance. RediSearch requires the Enterprise cluster policy and the `NoEviction` eviction policy. RediSearch isn't available on the Flash Optimized tier.
- **RedisJSON** enables you to store and query JSON-formatted data. You can use RedisJSON to store structured tenant-specific documents. RedisJSON is compatible with active geo-replication.
...
> [!IMPORTANT]
> You must enable modules when you create your Azure Managed Redis instance. You can't add or remove modules after creation. Plan your module selection based on the needs of your multitenant solution at deployment time.

### Cluster policies

Azure Managed Redis offers three [cluster policies](/azure/redis/architecture#cluster-policies), which affect how tenant data is distributed and accessed:

- **OSS cluster policy**: Follows the Redis Cluster API protocol. It provides the best throughput and lowest latency because clients connect directly to individual shards. However, your client library must support the Redis Cluster API. Multi-key commands require all keys to map to the same hash slot. You can't use the RediSearch module with this policy.
- **Enterprise cluster policy**: Uses a single endpoint that routes requests internally. This approach is simpler to configure and is compatible with client libraries that don't support clustering. It's required for the RediSearch module. A single-node proxy can become a throughput bottleneck at scale.
- **Non-Clustered policy**: Stores data without sharding, and is available only for caches of 25 GB or less. This policy is useful when you migrate from a nonclustered Azure Cache for Redis instance or when your application requires cross-slot commands extensively.

You set the cluster policy when you create the cache instance. You can't change the cluster policy after creation, except for changing from Non-Clustered to a clustered configuration.

### Flash Optimized tier

> [!IMPORTANT]
> All Flash Optimized tier SKUs are in Public Preview.

The [Flash Optimized tier](/azure/redis/overview#choosing-the-right-tier) automatically moves less frequently accessed data from memory (RAM) to NVMe storage. This tier can be a cost-effective option for multitenant solutions that have large datasets with mixed access patterns, where some tenants' data is accessed more frequently than others.

The Flash Optimized tier doesn't support RediSearch, active geo-replication, or the Non-Clustered policy. Some other modules also have tier restrictions. For the latest module availability by tier, see [Scope of Redis modules](/azure/redis/redis-modules#scope-of-redis-modules). Consider these limitations when you plan your multitenant architecture.
...
- [Carl Dacosta](https://www.linkedin.com/in/carl-dacosta/) | Principal Software Engineering Manager, Azure Cache for Redis
...
- [Sharding pattern](../../../patterns/sharding-content.md)
