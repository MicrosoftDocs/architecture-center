...
Like the account-per-tenant model below, this approach gives the highest level of performance isolation, but the lowest tenant density. However, this is the best option when each tenant requires a more complicated data model than is feasible in the container-per-tenant model, and when it is a requirement for new tenant creation to be fast and/or free of any overhead to create tenant accounts up-front. It may also be the case that for the specific software development framework being used, database-per-tenant is the only level of performance isolation recognized in that framework - entity (container) level isolation is not typically supported natively in such frameworks.
