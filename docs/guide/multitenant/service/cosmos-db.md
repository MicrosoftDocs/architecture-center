...
For tenants that require guaranteed performance and security isolation, we recommend isolating tenants by database account, and allocating request units to the tenant. For tenants with less stringent requirements, we recommend isolating tenants by partition key, which enables sharing request units among your tenants and helps you to optimize for a low cost per tenant.
