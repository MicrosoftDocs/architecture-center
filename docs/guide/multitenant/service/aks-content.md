...
AKS uses Azure CNI powered by [Cilum](https://cilium.io/) as the recommended dataplane for all new clusters. Azure Network Policies are depreciated. Calico is still being supported for existing clusters, but not recommended for new clusters.
...
Azure Network Policiies and Calico implementations use Linux iptables to enforce the specified policies. Network policies are translated into sets of allowed and disallowed IP pairs. These pairs are then programmed as iptables filter rules. You can only use Azure network policies with AKS clusters that are configured with the [Azure CNI](/azure/aks/configure-azure-cni) network plugin. However, Calico network policies support both [Azure CNI](/azure/aks/configure-azure-cni) and [kubenet](/azure/aks/use-network-policies) - this has been depreciated for new clusters. However, the recommended approached is to use [Azure CNI powered by Cilium](https://learn.microsoft.com/en-us/azure/aks/azure-cni-powered-by-cilium) For more information, see [Secure traffic between pods using network policies in Azure Kubernetes Service](/azure/aks/use-network-policies).
...
## Monitoring
You can use [Azure Monitor](/azure/aks/monitor-aks) to observe the health and peformance of AKS clusters and tenant workloads. Azure Monitor provivdes provides collection of [logs, metrics](/azure/aks/monitor-aks-reference), telemetry analysis, and alerting to proactively detect issues.

For new AKS deployments or multitenant clusters, Microsoft recommends the Azure Monitor managed service for Kubernetes (AMK), which uses the Azure Monitor Agent (AMA) and Data Collection Rules (DCRs) to provide more flexible, cost‑efficient, and fine‑grained data ingestion—especially useful when isolating monitoring per tenant or namespace.

Container Insights remains fully supported, but it’s considered a legacy monitoring path and is no longer recommended for new clusters. AMK offers several advantages over Container Insights, including native Prometheus scraping, per‑namespace or per‑workload data control, and reduced cost through selective data ingestion.

You can also use open‑source tools such as Prometheus and Grafana, or other non‑Microsoft observability platforms, alongside Azure Monitor if you require extended monitoring or multitenancy‑aware reporting.


 and [container insights](/azure/azure-monitor/containers/container-insights-overview) to monitor tenant applications that run on a shared AKS cluster and to calculate cost breakdowns on individual namespaces. Use Azure Monitor to monitor the health and performance of AKS. It includes the collection of [logs and metrics](/azure/aks/monitor-aks-reference), telemetry analysis, and visualizations of collected data to identify trends and to configure alerting that proactively notifies you of critical problems. You can enable [container insights](/azure/azure-monitor/containers/container-insights-overview) to expand on this monitoring.
