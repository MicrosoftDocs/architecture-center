...
Each unique federated identity provider must be [configured](/azure/active-directory-b2c/user-flow-overview) by using either a user flow or in a custom policy. Typically, during sign-in, the user would select which identity provider they wish to authenticate against. If you are using a shared tenant isolation model and/or you have a large number of federated identity providers, you should consider using [home realm discovery](https://github.com/azure-ad-b2c/samples/tree/master/policies/default-home-realm-discovery) to auto-select an identity provider during sign-in.
