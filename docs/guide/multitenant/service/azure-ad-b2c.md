...
>[!IMPORTANT]
> Avoid basing your partitioning strategy on factors that are volatile or that could change over time because it's difficult to move users between Azure AD B2C tenants. For example, if you are building a SaaS offering that has multiple SKUs or product tiers, you shouldn't partition your users based on the SKU they selected, because their SKU might change if the customer upgrades their product.
...
Provisioning an Azure AD B2C tenant for each application tenant allows you to customize many factors for each tenant. However, this approach comes at the cost of significantly increased overhead. You must consider how you plan for and manage this type of deployment and upkeep long term. You will need to develop a deployment and [maintenance](#maintenance) strategy for a potentially large number of Azure AD B2C tenants.
