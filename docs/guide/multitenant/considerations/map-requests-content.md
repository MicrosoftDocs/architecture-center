...
If you use [tenant-specific domain or subdomain names](./domain-names-content.yml), it's likely that requests can be easily mapped to tenants by using the `Host` header, or another HTTP header that includes the original hostname for each request.
...
Learn about [considerations when you work with domain names in a multitenant application](domain-names-content.yml).
