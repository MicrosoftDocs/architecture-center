...
- Does your profitability depend on having customers use your solution in a limited way?
- If a customer overuses the solution, will that mean you're no longer profitable?
...
- **Tenant isolation.** The [tenancy model](tenancy-models.md) you use affects the level of isolation you have between your tenants. If you share your resources, do you need to consider how tenants might over-utilize or abuse the service, and how this will affect your COGS and performance for everyone? Some pricing models are not profitable without additional controls around resource allocation. For example, you may need to implement service throttling to make a flat-rate pricing model sustainable.
...
> [!NOTE]
> You can offer multiple models for a solution or combine models together. For example, you could provide a per-user model for your customers that have fairly stable user numbers, and also a consumption model for customers who have fluctuating usage patterns.
...
**Complexity and operational cost:** Consumption models rely on accurate measurements of usage and splitting this usage by tenant. This can be challenging, especially in a solution with many distributed components. You need to keep detailed consumption records for billing and auditing.
...
> [!NOTE]
> Implementing and supporting capacity reservations may increase complexity of the billing processes within your application. You may also need to consider how customers get refunds or exchange their capacity reservations, and these process can also add commercial and operational challenges.
...
**Complexity and operational cost:** Per-user models tend to be easy to implement. However, in some situations you need to measure per-user consumption, which can help you to ensure that the COGS for a single user remains profitable. Measuring consumption and associating it with a particular user can increase the operational complexity of your solution.

**Risks:** Different user consumption patterns may result in reduced profitability. For example, heavy users of the solution may cost more to serve than light users. Additionally, the actual return on value (ROV) for the solution is not reflected by the actual number of user licenses purchased.
...
**Complexity and operational cost:** Generally, per-unit pricing is easy to implement and has a fairly low operational cost. However, the operational cost can become higher if you need to differentiate and track usage by individual units like devices or retail stores. Measuring per-unit consumption helps you ensure your profitability is maintained, since you can determine the COGS for a single unit.
