...
Even if you use the volume of data stored by a tenant as an indicative consumption measure, it might not be a true representation of consumption for every tenant. If a tenant generates far more read activity than write activity, such as by using heavily using reporting capabilities, then that tenant might use much more compute from the solution than the storage requirements indicate.
