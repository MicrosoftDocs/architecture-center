...
Even if you use the volume of data stored by a tenant as an indicative consumption measure, it might not be a true representation of consumption for every tenant. If a tenant generates far more read activity or reporting than write activity, that tenant uses much more compute from the solution than the storage requirements indicate.
