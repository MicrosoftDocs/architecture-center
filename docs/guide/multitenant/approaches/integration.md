...
- **Request-response** is where a client initiates a request to a server and receives a response. Typically, request-response integrations are implemented by using APIs or webhooks. Requests might be *synchronous*, where they wait for acknowledgment and a response. Alternatively, requests can be *asynchronous* and use something like the [Asynchronous Request-Reply pattern](../../../patterns/async-request-reply.md) to wait for a response.
