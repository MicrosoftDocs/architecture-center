...
- **Import all the data from the underlying data store.** This approach requires that Power BI receives credentials for an identity that accesses the full data store. Power BI administrators then configure permissions for the imported data independently after it's brought into Power BI. Power BI enforces those permissions.

- **Import a subset of data from the underlying data store, based on a user's permissions.** When a user creates the data source, they use their credentials and associated permissions. The exact subset of data Power BI imports depends on the access level of the user who creates the data source.
...
Alternatively, if you work with user-scoped data, then you might need to use an approach like *delegation* to access the correct subset of data from the data set. Then the destination system effectively gets the same permission as a specific user. For more information about user delegation, see the [Delegated user access](#delegated-user-access) section. If you use delegation, consider how to handle scenarios where a user is deprovisioned or their permissions change.

### Real-time data or batch data

Consider whether you'll work with real-time data, or if the data will be sent in batches.
...
When data is exchanged between two parties, it's important that they both have a clear understanding of how the data will be formatted and structured. Consider the following parts of the data format:
...
When you work with a multitenant system, if possible, standardize and use the same data format for all of your tenants. This approach helps you avoid having to customize and retest your integration components for each tenant's requirements. However, in some scenarios, you might need to use different data formats for communicating with different tenants, and so you might need to implement multiple integrations. For an approach that can help to simplify this type of scenario, see the [Composable integration components](#composable-integration-components) section.
...
- Use the [Valet Key pattern](#valet-key-pattern), which uses security measures like shared access signatures (SASs) to grant restricted access to specific Azure resources.
...
When you start to interact directly with your tenants' data or transmit that data, it's crucial that you have a clear understanding of your tenants' [governance and compliance requirements](../approaches/governance-compliance.md).

## Approaches and patterns to consider

### Expose APIs

Real-time integrations often involve exposing APIs to your tenants or other parties to use. APIs require special considerations, especially when external parties use them. Consider the following factors:
...
For example, you can use this approach when batch exporting a large data file. After you generate the export file, you can save it to a blob container in Azure Storage and then generate a time-bound, read-only SAS. This signature can be provided to the tenant, along with the blob URL. The tenant can then download the file from Azure Storage until the signature's expiration date.

Similarly, you can generate a SAS with permissions to write to a specific blob. When you provide a SAS to a tenant, they can write their data to the blob. By using Event Grid integration for Azure Storage, your application can then be notified to process and import the data file.
...
When you use messaging as part of an integration with your tenants' systems, consider whether you should use [SASs for Azure Service Bus](/azure/service-bus-messaging/service-bus-sas) or [Event Hubs](/azure/event-hubs/authorize-access-shared-access-signature). SASs enable you to grant limited access to your messaging resources to third parties without enabling them to access the rest of your messaging subsystem.
...
- Transmit the data by using a specific network transport or to a known destination type.
...
- **Unnecessary tight coupling.** Loose coupling, such as by using [messaging](#messaging) approaches, can provide a range of benefits for security, performance isolation, and resiliency. When possible, you should loosely couple your integrations with third parties. If you need to tightly couple to a third party, ensure that you follow good practices like the [Retry pattern](../../../patterns/retry.yml), the [Circuit Breaker pattern](../../../patterns/circuit-breaker.md), and the [Bulkhead pattern](../../../patterns/bulkhead.yml).
