...
One way you might be able to reduce the cost of a multitenant solution is to move tenants to shared infrastructure. However, you need to carefully consider the impact of sharing resources, such as whether your tenants will begin to experience the [Noisy Neighbor problem](../../../antipatterns/noisy-neighbor/index.md).

You also need to consider how you measure and allocate the costs of shared components. For example, should you evenly divide the cost between each of the tenants that use the shared component? Or, can you meter each tenant's usage to get a more precise measurement of their consumption of shared components?

## Approaches and patterns to consider

### Allocate costs by using resource tags

Azure enables you to [apply tags to your resources](/azure/azure-resource-manager/management/tag-resources). A tag is a key-value pair. You use tags to add custom metadata. Tags are useful for many management operations, and also for analyzing the cost of your Azure consumption. After you apply tags, [you can determine costs associated with each tag](/azure/cost-management-billing/costs/cost-analysis-common-uses#view-costs-for-a-specific-tag).
...
In other situations, you might have sets of shared resources. For example, when you apply the [Sharding pattern](../../../patterns/sharding.md), you may deploy multiple databases and spread your tenants across them. Consider tagging the resources with an identifier for the *group* of tenants. You might not be able to easily allocate costs to a single tenant, but you can at least narrow down the cost to a set of tenants when you use this approach. You can also use the consumption information to help you rebalance tenants across shards if you notice that a specific shard is accruing higher costs than others.
...
Your application tier may already collect logs and metrics that are helpful to answer questions like:
...
If you need to track precise details about consumption or usage for billing purposes, you should instead build a custom pipeline to log the necessary data and aggregate it based on your requirements. Azure services that can be helpful for this purpose include [Event Hubs](https://azure.microsoft.com/services/event-hubs/), for capturing large volumes of telemetry, and [Stream Analytics](https://azure.microsoft.com/services/stream-analytics/) to process it in real-time.
...
Reservations can be used effectively in a multitenant solution. For example:

- When you deploy a multitenant solution that includes shared resources, consider the baseline level of consumption that you need for the workload. You might consider a reservation for that baseline consumption, and then paying standard rates for higher consumption during unpredictable peaks.
...
Reservation scopes can also be helpful when you have tenants that have unpredictable workloads. For example, consider a solution in which tenant A only needs one instance of a specific resource, but tenants B and C each need two. Then tenant B becomes less busy, so you reduce the instance count, and tenant A gets busier so you increase it. Your reservations are applied to the tenants that need them.
...
- **Making assumptions or guessing.** Ensure your cost measurement is based on real information. You might not need a high degree of precision, but even your estimates should be informed by real measurement.
- **Unnecessary precision.** You may not need to have a detailed accounting of every cost incurred for every tenant. Building unnecessarily precise cost measurement and optimization processes can be counterproductive, adding engineering complexity and creating brittle processes.
- **Real-time measurement.** Most solutions don't need up-to-the-minute cost measurement. Because metering and consumption data can be complex to process, it's best to log the necessary data and then asynchronously aggregate and process the data later.
