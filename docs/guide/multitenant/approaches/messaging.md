...
- If tenants need a high level of recoverability and resiliency, the designed solution should provide the ability to provision an Event Hubs Premium namespace, with geo-disaster recovery and [availability zones](/azure/reliability/availability-zones-overview) enabled. For more information, see [Azure Event Hubs - Geo-disaster recovery](/azure/event-hubs/event-hubs-geo-dr).
