...
- For highly regulated multitenant environments requiring strict network isolation between tenants, consider using [network security perimeter](/azure/event-grid/configure-network-security-perimeter) with Event Grid to implement additional network-level boundaries and access controls.
