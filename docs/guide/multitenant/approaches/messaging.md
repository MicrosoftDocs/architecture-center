...
- If tenants need a high level of resiliency (continued operation during zonal faults) and recoverability (failover/restoration for regional disruptions), provision an Event Hubs Premium namespace with both availability zones for resilience and geo-disaster recovery for recoverability. For more information, see [Azure Event Hubs - Geo-disaster recovery](/azure/event-hubs/event-hubs-geo-dr).
