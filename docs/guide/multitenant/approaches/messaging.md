...
This article distinguishes between messages and events, and provides guidelines that solution architects can follow when deciding which approach to use for messaging or eventing infrastructure in a multitenant solution.

## Messages, data points, and discrete events

All messaging systems have similar functionalities, transport protocols, and usage scenarios. For example, most modern messaging systems support asynchronous communications that use volatile or persistent queues, AMQP and HTTPS transport protocols, at-least-once delivery, and more. However, by examining the type of published information and how client applications consume and process data, we can distinguish between different kinds of messages and events. There's an essential distinction between services that deliver an event and systems that send a message. For more information, see the following resources:
...
An event is a lightweight notification of a condition or state change. Events can be discrete units or part of a series. Events are messages that don't generally convey a publisher's intent other than to inform. An event captures a fact and communicates it to other services or components. A consumer can process the event as needed and doesn't fulfill any specific expectations the publisher holds. You can classify events into two categories:
...
Messages contain raw data that's produced by a service to be consumed or stored elsewhere. Messages often carry information necessary for a receiving service to execute steps in a workflow or a processing chain. An example of this kind of communication is the [Command pattern](https://en.wikipedia.org/wiki/Command_pattern). The publisher might also expect the receiver(s) of a message to execute a series of actions and to report back the outcome of their processing with an asynchronous message. A contract exists between the message publisher and message receiver(s). For example, the publisher sends a message with some raw data and expects the consumer to create a file from that data and send back a response message when done. In other situations, a sender process could send an asynchronous, one-way message to ask another service to perform a given operation, but with no expectation of getting back an acknowledgment or response message from it.
...
- An event-driven application receives events from multiple tenants, via a single, shared [Azure Event Hubs](/azure/event-hubs/event-hubs-about) instance. This solution provides a high level of agility, because onboarding a new tenant doesn't require creating a dedicated event-ingestion resource, but it provides a low data protection level, as it inter-mingles messages from multiple tenants in the same data structure.
...
- If tenants need a high level of resilience (continued operation during zonal faults) and recoverability (failover/restoration for regional disruptions), provision an Event Hubs Premium namespace with both availability zones for resilience and geo-disaster recovery for recoverability. For more information, see [Azure Event Hubs - Geo-disaster recovery](/azure/event-hubs/event-hubs-geo-dr).
...
- Event Grid supports [cross-tenant delivery](/azure/event-grid/cross-tenant-delivery-using-managed-identity), which enables secure event delivery from Event Grid topics in one Microsoft Entra tenant to destinations (Event Hubs, Service Bus, Storage Queues, Webhooks) in another tenant using managed identities. This capability is particularly valuable for multitenant SaaS solutions where customer tenants operate in separate Microsoft Entra tenants but need to receive events from your application.
- You can [forward events between Event Grid namespace topics (Preview)](/azure/event-grid/forward-events-to-another-namespace-topic), either within the same namespace or across different namespaces. This enables sophisticated tenant-specific event routing patterns where events can be automatically forwarded to tenant-dedicated processing pipelines or external systems.
- For highly regulated multitenant environments requiring strict network isolation between tenants, consider using [network security perimeter in Event Grid (Preview)](/azure/event-grid/configure-network-security-perimeter) to implement additional network-level boundaries and access controls.

### Complexity of implementation

When designing a multitenant solution, it's essential to consider how the system will evolve in the medium to long term, in order to prevent its complexity from growing over time, until it's necessary to redesign part of or the entire solution. This redesign will help you cope with an increasing number of tenants. When designing a messaging system, you should consider the expected growth in message volumes and tenants, in the next few years, and then create a system that can scale out, in order to keep up with the predicted traffic and to reduce the complexity of operations, such as provisioning, monitoring, and maintenance.
...
[Azure Service Bus](/azure/service-bus-messaging/service-bus-geo-dr) and [Azure Event Hubs](/azure/event-hubs/event-hubs-geo-dr) support metadata disaster recovery across primary and secondary disaster recovery namespaces and across separate regions and availability zones to provide the best intra-region resiliency. Also, Azure Service Bus and Azure Event Hubs implement a set of federation patterns that actively replicate the same logical topic, queue, or event hub to be available in multiple namespaces, potentially located in different regions. For more information, see the following resources:
