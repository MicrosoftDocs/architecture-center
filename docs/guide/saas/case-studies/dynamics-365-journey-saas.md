...
There are some situations where user counts don't give a good indication of the resources that the customer consumes. For example, when a customer’s marketing team sends out a large number of messages for an email campaign, there might just be one user sending millions of email messages. Similarly, a background process, not the user, imports order details. It's important that customers understand the metrics they're charged for and can predict their bill. You might choose to use meters like the number of contacts they send email messages to or the number of order lines they process each month.
...
In the cloud, it's critical to move from *scaling up* to *scaling out* wherever possible. If more demand can be met by adding more nodes, or scaling out, instead of making existing nodes more powerful, scaling up, and that relationship is close to linear, then an approach based on scaling out provides the potential to drive even higher scale. Dynamics 365 uses a scale-out model at the application tier. Integrated monitoring detects increases in load for specific tenants and adds more nodes to meet the demand.
...
A core focus area for Dynamics 365 today is targeting redundancy across Azure availability zones to allow the business-critical Dynamics services to seamlessly continue operating, even if an outage impacts a datacenter or an entire availability zone.
...
## How we operated Dynamics 365 as SaaS
