...
Before you install WRF, you need to deploy and connect to a VM or HPC Cluster.
...
Files: gfs.0p25.2023021200.f000.grib2, gfs.0p25.2023021200.f003.grib2, gfs.0p25.2023021200.f006.grib2, gfs.0p25.2023021200.f009.grib2, gfs.0p25.2023021200.f384.grib2
...
7. Configuring option for WPS: For current performance test option 19 was selected as shown below
...
9. Edit namelist.input file for runtime options (at minimum, one must edit &time control for start, end and integration times, and &domains for grid dimensions).
...
- Expected mean time per step was achieved with all the nodes tested. However, scalability might vary depending on the dataset used and the node count. Be sure to test the affect of the tile size, process, and threads per process.
