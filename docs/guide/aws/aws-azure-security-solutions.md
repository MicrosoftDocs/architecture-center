...
- **Workload identities:** Use Microsoft Entra workload identity federation to support bidirectional multicloud trust without static credentials. Azure-based workloads can use short-lived tokens that AWS trusts to assume AWS IAM roles, which avoids static AWS access keys. AWS-based workloads, such as Amazon EC2 instances and AWS Lambda functions, can also exchange their AWS-issued OpenID Connect (OIDC) tokens for Microsoft Entra tokens to call Azure resources and APIs, such as Azure Functions that the Microsoft identity platform protects. This approach avoids stored Azure client secrets. In both directions, federation minimizes the risk of leaked credentials because it verifies every access request in real time and governs identities centrally. For configuration steps, see [Workload identity federation](/entra/workload-id/workload-identity-federation).
