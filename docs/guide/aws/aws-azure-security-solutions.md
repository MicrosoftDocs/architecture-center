...
- **Workload identities:** Use Microsoft Entra workload identity federation to support bidirectional multicloud trust without static credentials. Azure-based workloads can use short-lived tokens that AWS trusts to assume AWS IAM roles. This direction avoids static AWS access keys. AWS-based workloads, such as Amazon EC2 instances and AWS Lambda functions, can exchange OpenID Connect (OIDC) tokens issued by an external identity provider in AWS (for example, Amazon Cognito) for Microsoft identity platform access tokens. They use those access tokens to call Azure resources and APIs, such as Azure Functions protected by the Microsoft identity platform. This direction avoids stored Azure client secrets. In both directions, federation minimizes the risk of leaked credentials. It verifies every access request in real time and governs identities centrally. For configuration steps, see [Workload identity federation](/entra/workload-id/workload-identity-federation).
