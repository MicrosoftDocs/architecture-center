...
- Enable [dedicated data endpoints](/azure/container-registry/container-registry-dedicated-data-endpoints) on the registry so that Azure Firewall application rules can scope cluster egress to your registry. Without dedicated data endpoints, image-layer downloads a `blob.core.windows.net` endpoint, which forces you to allow that wildcard in firewall rules. This rule would allow node egress to *any* Azure Storage account which is an exfiltration risk. Dedicated data endpoints replace the wildcard with registry-specific FQDNs (`<registry>.<region>.data.azurecr.io`), one per replica region.
