...
Level 3 is dedicated to the Kubernetes objects and applications that are deployed at the cluster level in AKS, like `ama-logs`. For this check, you can use the Insights view of Monitor to check the status of the AKS deployments:
...
The switch reconfigures to the following state:
...
- There are alternative techniques for implementing the traffic switches that can provide more control. For example, it's possible to do a partial switch by using traffic rules that are based on one or more of the following criteria:
...
- You can have a single ingress for the cluster paired with a dedicated public load balancer. This approach is like the reference implementation of the architecture described in this guide. [Application Gateway for Containers](/azure/application-gateway/for-containers/overview) is an Azure-managed layer-7 load balancer and ingress solution for AKS. The in-cluster **application load balancer controller** is a component that watches Kubernetes Gateway API and Ingress resources and configures Application Gateway for Containers accordingly. It translates these resources into Application Gateway for Containers configuration. In certain scenarios, AKS clusters have admin endpoints in addition to application endpoints. Use the admin endpoints to perform operational tasks on the applications or for configuration tasks like updating configuration maps, secrets, network policies, and manifests.
- You can have a single external load balancer that serves multiple ingresses deployed on the same cluster or on multiple clusters. This approach isn't covered in the reference implementation. In this scenario, we recommend that you have separate Application Gateway for Containers deployments for public facing endpoints and for private ones.
- The resulting architecture that's proposed and depicted in this guide uses [Application Gateway for Containers](/azure/application-gateway/for-containers/overview). Traffic is processed at the Azure network layer outside the AKS node pool, so ingress capacity scales independently of cluster nodes. You can also adopt a standard in-cluster ingress controller, like [NGINX](https://kubernetes.github.io/ingress-nginx/) or [Envoy](https://gateway.envoyproxy.io/), without affecting the overall blue-green architecture.
