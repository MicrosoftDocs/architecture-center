...
This reference implementation is based on Application Gateway and [Application Gateway for Containers (AGC)](/azure/application-gateway/for-containers/overview). Each cluster has its own application gateway and the traffic switch is done via DNS, in particular via `CNAME` configuration.
