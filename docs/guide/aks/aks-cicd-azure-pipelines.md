...
1. The CD pipeline deploys a YAML template to the staging AKS environment that includes a [Defender agent](/azure/defender-for-cloud/defender-for-containers-enable-plan). This deployment uses a push model and runs via either kubectl or Helm. The template references the container image from the nonproduction registry.
