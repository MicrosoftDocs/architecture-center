...
:::image type="complex" source="media/sap-whole-landscape.svg" border="false" lightbox="media/sap-whole-landscape.svg" alt-text="Diagram that shows a sample overall SAP landscape in Azure.":::
This diagram presents an end-to-end SAP landscape across two stacked Azure regions, with the primary region in the upper half and the disaster recovery region in the lower half, and it uses numbered callouts to map key architecture elements and flow. On the far left, callout 1 marks the on-premises network, which connects through ExpressRoute or VPN to callout 2, the Azure subscription for regional hubs, where a hub virtual network in the primary region and a hub DR virtual network in the secondary region provide central connectivity. Inside the hub networks are gateway, Azure Firewall, shared services, and Application Gateway subnets, with shared services including Active Directory and DNS, and public IP connectivity for controlled inbound and outbound paths. To the right, callout 5 identifies the SAP non-production subscription, which contains separate spoke virtual networks for development, pre-production and QA, and sandbox environments, represented by repeated callout 6; each non-production spoke is segmented into application and database subnets, includes private endpoints, and connects to file services such as Azure Files or Azure NetApp Files and SMB/NFS shares. Callout 7 identifies the SAP production subscription, which contains the primary SAP production spoke and, as callout 9, a mirrored SAP production DR spoke; both are segmented into application, database, and perimeter-related subnets and show SAP workload tiers such as Web Dispatcher pool, ASCS and application servers, databases, SAProuter, and private endpoints. Hub-to-spoke connectivity is shown through labeled virtual network peering links, and the diagram emphasizes that traffic between spokes is mediated through the hub. Between primary and DR production environments, the drawing explicitly labels asynchronous database replication, asynchronous Azure Site Recovery replication, and NFS/SMB file replication, and it also shows Recovery Services Vaults and cloud witness components that support resilience and failover operations. On the right side, callout 10 groups connected Azure data and platform services, including Data Factory and Data Lake, and callout 11 shows SAP Business Technology Platform access through Private Link Service with DEV, QAS, and PRD subaccounts. The overall purpose of the architecture is to show how a large SAP estate is organized with subscription boundaries, hub-spoke networking, environment isolation, shared central services, and cross-region disaster recovery mechanisms while maintaining controlled connectivity to enterprise and platform services.
:::image-end:::
...
**Use a central firewall.** All the network traffic to the spoke virtual networks, including remote function call (RFC) connections, should pass through a central firewall in the Hub virtual network. All the communication between the spoke virtual networks passes through the Hub virtual network firewall. Network communication between the spoke virtual networks (spoke-to-spoke communication) passes through the hub virtual network firewall in the Azure Firewall subnet of the Hub virtual network. Similarly, network communication between the spoke virtual networks and on-premises network also pass through the hub virtual network firewall. We used virtual network peering to connect the various spoke virtual networks to the hub virtual network. You could also use a network virtual appliance instead of a firewall. For more information, see [network virtual appliance](https://azure.microsoft.com/solutions/network-appliances/).

Network traffic that stays within a virtual network shouldn't pass through a firewall. For example, don't put a firewall between the SAP application subnet and SAP database subnet. It isn't supported to place a firewall or network virtual appliances (NVAs) between the SAP application and the database management system (DBMS) layer of SAP systems running the SAP kernel. Doing so will negatively affect network latency for all database accesses and negatively affect SAP performance.
...
Ensure the subnets have sufficient network address space. If you use SAP virtual host names, you need more address space in your SAP subnets. Often each SAP instance requires 2, 3 or more IP addresses and includes one IP address for the virtual machine hostname. Other Azure services might require their own dedicated subnet when deployed in the SAP workload virtual networks.
