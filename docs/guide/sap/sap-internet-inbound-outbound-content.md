...
:::image type="complex" source="media/sap-internet-communication-architecture-main.svg" border="false" lightbox="media/sap-internet-communication-architecture-main.svg" alt-text="Architecture for inbound and outbound internet communication for SAP on Azure.":::
This architecture diagram shows an SAP on Azure network segmented across one Azure region and two subscriptions: on the left and center is the Azure subscription central hub, and on the right is the Azure subscription SAP production. At the far left is an on-premises network connected through ExpressRoute to a zone-redundant gateway in the hub virtual network. Inside the hub virtual network, four stacked subnets are shown: Gateway subnet, Azure Firewall subnet with a firewall, Application Gateway subnet with AppGw/WAF, and Shared services subnet with AD/DNS. To the right of the hub are two peered spoke virtual networks in the SAP production subscription: an upper SAP perimeter spoke virtual network with a single SAP perimeter network application subnet, and a lower SAP production spoke virtual network split into an Application subnet and a Database subnet. The SAP perimeter subnet contains SAProuter and Cloud Connector virtual machines, while the SAP production application subnet contains SAP Web Dispatcher virtual machines plus SAP ASCS and application server virtual machines, and the database subnet contains database virtual machines, private endpoint, and Azure Files NFS with a labeled database replication path. Two separate virtual network peering links connect the hub to each spoke, and there is no direct spoke-to-spoke link. At the top-left, an Internet cloud and external SAP endpoint connect to the architecture through public IP addresses; the HTTPS inbound path runs from Internet to a public IP in front of the firewall, then to AppGw/WAF, then to the SAP Web Dispatcher tier, and then to SAP application servers. A second labeled path for SNC communication SAProuter traffic runs from external SAP through the public IP and firewall to SAProuter in the perimeter subnet, then onward to SAP systems over peered virtual networks. Outbound traffic from SAP perimeter services, especially Cloud Connector, is shown as a BTP TLS tunnel that exits through firewall and public IP toward Internet or SAP BTP endpoints. The purpose of the design is to isolate internet-facing SAP components in a dedicated perimeter spoke, centralize ingress and egress controls in the hub, and allow controlled east-west traffic to SAP application and database tiers while preserving private service access through private endpoints.
:::image-end:::
...
**Zone-redundant gateway.** A network gateway connects distinct networks, extending your on-premises network to the Azure virtual network. We recommend that you use [ExpressRoute](/azure/well-architected/service-guides/azure-expressroute) to create private connections that don't use the public internet. You can also use a [site-to-site](../../reference-architectures/hybrid-networking/expressroute-vpn-failover.yml) connection. Use [zone-redundant Azure ExpressRoute](/azure/vpn-gateway/about-zone-redundant-vnet-gateways) or VPN gateways to guard against zone failures.
...
- Application Gateway recommends a /24 subnet for scaling. If choosing to limit the Application Gateway scale a smaller subnet could be used, at the [minimum /26 or larger](/azure/application-gateway/configuration-infrastructure#size-of-the-subnet).
...
SAProuter is a tunnel to SAP or to your partners. This architecture describes the use of SAProuter with SNC use to establish an encrypted application tunnel (network layer 7) to SAP/partners. The use of IPSEC based tunnel isn't covered in this architecture presently.  
...
Keep in mind that you need to secure the SAP Web Dispatcher in any situation, even if it's open only internally, accessed through Application Gateway via public IP, or accessible by other network means. For more information, see [Security Information for SAP Web Dispatcher](https://help.sap.com/docs/ABAP_PLATFORM/683d6a1797a34730a6e005d1e8de6f22/489ab29948c673e8e10000000a42189b.html?version=1709.008).
