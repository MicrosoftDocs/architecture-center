...
:::image type="complex" source="media/sap-internet-communication-architecture-main.svg" border="false" lightbox="media/sap-internet-communication-architecture-main.svg" alt-text="Diagram that shows an architecture for inbound and outbound internet communication for SAP on Azure.":::
This architecture diagram shows an SAP on Azure network that's segmented across two subscriptions in one region: a central hub subscription and an SAP production subscription. At the far left is an on-premises network connected through ExpressRoute to a zone-redundant gateway in the hub virtual network. Four subnets are located in the hub virtual network: a gateway subnet, an Azure Firewall subnet, an Application Gateway subnet, and a shared services subnet. The shared services subnet contains Microsoft Entra ID and DNS. To the right of the hub are two peered spoke virtual networks in the SAP production subscription: an SAP perimeter spoke virtual network and an SAP production spoke virtual network. The perimeter network contains an application subnet that in turn contains SAProuter and Cloud Connector VMs. The production network is split into an application subnet and a database subnet. The SAP production application subnet contains SAP Web Dispatcher, SAP ASCS, and application server VMs and a private endpoint. The database subnet contains database VMs that are connected with a database replication path. An NFS Azure file share is located outside of the production network and connects to the private endpoint. Two separate virtual network peering links connect the hub to each spoke, and there's no direct spoke-to-spoke link. At the top-left, an internet icon and external SAP endpoint connect to the architecture through public IP addresses. The HTTPS inbound path runs from the internet to a public IP in front of the firewall, then to the Application Gateway with Web Application Firewall, and then to the SAP Web Dispatcher tier. A second path for SNC communication SAProuter traffic runs from the external SAP endpoint through the public IP and firewall to SAProuter in the perimeter subnet. Outbound traffic from Cloud Connector is shown as a BTP TLS tunnel that exits through the firewall and public IP to the internet and SAP external endpoints.
:::image-end:::
...
- Two [Azure subscriptions](/azure/cloud-adoption-framework/ready/landing-zone/design-area/resource-org-subscriptions) are used in this architecture. The architecture implements the Azure [landing zone](/azure/cloud-adoption-framework/ready/landing-zone) approach. One subscription is used for each workload. One or more subscriptions are used for central IT services that contain the network hub and central, shared services like firewalls or Microsoft Entra ID and DNS. Another subscription is used for the SAP production workload. Use the [decision guide](/azure/cloud-adoption-framework/decision-guides/subscriptions) in the Cloud Adoption Framework for Azure to determine the best subscription strategy for your scenario.

- [Azure Virtual Network](/azure/well-architected/service-guides/virtual-network) connects Azure resources to each other with enhanced security. In this architecture, the virtual network connects to an on-premises environment via an ExpressRoute or virtual private network (VPN) gateway that's deployed in the hub of a [hub-spoke topology](../../networking/architecture/hub-spoke.yml). The SAP production landscape uses its own spoke virtual networks. Two distinct spoke virtual networks perform different tasks, and subnets provide network segregation.
...
- [Application security groups](/azure/virtual-network/security-overview) are used to define fine-grained network security policies in your NSGs based on workloads that are centered on applications. When you use application security groups instead of explicit IP addresses, you can group VMs by purpose, like by SAP SID. Application security groups help secure applications by filtering traffic from trusted segments of your network.

- [Private endpoints](/azure/private-link/private-endpoint-overview) are network interfaces in your virtual network. They effectively bring the service into your private network space. Many Azure services operate as public services, by design reachable via the internet. To allow private access via your private network range, you can use private endpoints for some services.

- [Application Gateway](/azure/well-architected/service-guides/azure-application-gateway) is a web-traffic load balancer. With its Web Application Firewall functionality, it's the ideal service to expose web applications to the internet with improved security. Application Gateway can service public (internet) clients, private clients, or both, depending on the configuration.

   In this architecture, Application Gateway, using a public IP address, allows inbound connections to the SAP landscape over HTTPS. Its back-end pool is two or more SAP Web Dispatcher VMs, accessed round-robin and providing high availability. The application gateway is a reverse proxy and web-traffic load balancer, but it doesn't replace the SAP Web Dispatcher. SAP Web Dispatcher provides application integration with your SAP systems and includes features that Application Gateway by itself doesn't provide. When client authentication reaches the SAP systems, the SAP application layer handles it natively or via single sign-on. When you enable Azure DDoS Protection, consider using [DDoS network protection SKU](/azure/ddos-protection/ddos-protection-sku-comparison) because you can receive discounts for Application Gateway Web Application Firewall.
...
- [Azure Load Balancer](/azure/well-architected/service-guides/azure-load-balancer) provides networking elements for the high-availability design of your SAP systems. For clustered systems, Load Balancer provides the virtual private IP address for the cluster service, like ASCS/SCS instances and databases running on VMs. You can also use Load Balancer to provide the IP address for the virtual SAP host name of nonclustered systems when [secondary IPs on Azure network cards](https://techcommunity.microsoft.com/t5/running-sap-applications-on-the/use-sap-virtual-host-names-with-linux-in-azure/ba-p/3251593) aren't an option. The use of Load Balancer instead of Application Gateway to address outbound internet access is covered later in this article.
...
An application-specific [perimeter network](/azure/cloud-adoption-framework/ready/azure-best-practices/perimeter-networks) (also known as a *DMZ*) contains the internet-facing applications, like SAProuter, SAP Cloud Connector, and SAP Analytics Cloud Agent. In the architecture diagram, the perimeter network is named *SAP perimeter - Spoke virtual network*. Because of dependencies on SAP systems, the SAP team typically deploys, configures, and manages these services. That's why these SAP perimeter services frequently aren't located in a central hub subscription and network. Central hub placement of workload-specific applications or services frequently creates organizational challenges.

Here are some of the benefits of using a separate SAP perimeter virtual network:

- Immediate isolation of compromised services if a breach is detected. Moving virtual network peering from the SAP perimeter to the hub immediately isolates the SAP perimeter workloads and SAP application virtual network applications from the internet. Changing or removing an NSG rule that permits access affects only new connections and doesn't cut existing connections.
...
:::image type="complex" source="media/sap-internet-communication-architecture-simplified.svg" border="false" lightbox="media/sap-internet-communication-architecture-simplified.svg" alt-text="Diagram that shows a simplified architecture for internet-facing communication for SAP on Azure.":::
This diagram shows a simplified SAP internet connectivity architecture in one Azure region with two subscriptions and one SAP production spoke virtual network. On the far left is an on-premises network with a gateway. It's connected by ExpressRoute to the hub virtual network in the central hub subscription. The hub virtual network contains a gateway subnet that contains a zone-redundant gateway, an Azure Firewall subnet, a shared services subnet that contains Microsoft Entra ID, DNS, and an Azure Bastion subnet. A virtual network peering link connects this hub to the SAP production spoke virtual network on the right. The SAP production spoke contains four subnets, as follows. The SAP perimeter network application subnet contains SAProuter, Azure NAT Gateway, and Cloud Connector. An Application Gateway subnet contains Application Gateway and Web Application Firewall. An application subnet contains SAP Web Dispatcher, SAP ASCS/SCS, and application server VMs, and a private endpoint. A database subnet contains database VMs and a database replication path. An NFS Azure file share is located outside of the production network and connects to the private endpoint. At the top left, the internet and an external SAP endpoint connect to the subscriptions, as follows. SNC communication SAProuter traffic flows from SAProuter to the firewall, and then to the internet and the external SAP endpoint via a public IP address. A separate BTP TLS tunnel path starts at Cloud Connector and flows to the external SAP and internet endpoints.
:::image-end:::
...
The simplified architecture uses Azure NAT Gateway in the SAP perimeter subnet. This gateway provides outbound connectivity for SAP Cloud Connector, SAP Analytics Cloud Agent, and other public endpoints. Because SAProuter requires both incoming and outbound connections, the SAProuter communication path goes through the firewall instead of using the NAT gateway. The simplified architecture also places the application gateway with its own designated subnet in the SAP perimeter virtual network, as an alternative approach to the hub virtual network.

[Azure NAT Gateway StandardV2](/azure/virtual-network/nat-gateway/nat-overview) is a service that provides static public IP addresses for outbound connectivity. The NAT gateway is assigned to a subnet. All outbound communications use the NAT gateway's IP addresses for internet access through source network address translation (SNAT). Inbound connections don't use the NAT gateway. Applications like SAP Cloud Connector, or VM OS update services that access repositories on the internet, can use the NAT gateway instead of routing all outbound traffic through the central firewall. Azure NAT Gateway also provides a much larger SNAT port pool than firewall-attached public IPs, which mitigates port exhaustion for chatty subnets. [User-defined rules](/azure/virtual-network/ip-services/default-outbound-access) are frequently implemented on all subnets to force internet-bound traffic from all virtual networks through the central firewall.

Depending on your requirements, you might be able to use the NAT gateway as an alternative to the central firewall for all or most outbound connections. This approach can reduce load on the central firewall while communicating with NSG-allowed public endpoints. You also get outbound IP control, because you can configure destination firewall rules on a set IP of the NAT gateway. Examples include reaching Azure public endpoints that are used by public services or non-Microsoft interfaces.

For a high-availability configuration, keep in mind that you need to deploy Azure NAT Gateway StandardV2 to achieve [cross-zone resiliency](/azure/nat-gateway/faq#how-does-a-nat-gateway-work-with-availability-zones).

### Use of network components across an SAP landscape

An architecture document typically depicts only one SAP system or landscape. This focused view makes them easier to understand. The result is that frequently the bigger picture, how the architecture fits into a larger SAP landscape that includes several system tracks and tiers, isn't addressed.
...
- **Load balancers** should be dedicated to individual services. Company policy dictates naming and grouping of resources. We recommend one load balancer for ASCS/SCS and ERS and another for database, separated for each SAP SID. Alternatively, a single load balancer for ASCS/SCS, ERS, and database clusters of one SAP system is also a good design. This configuration helps to ensure that troubleshooting doesn't get complex, with many front-end and back-end pools and load balancing rules all on a single load balancer. A single load balancer per SAP SID also ensures that placement in resource groups matches that of other infrastructure components.
- **Application Gateway**, like a load balancer, allows multiple back ends, front ends, HTTP settings, and rules. Using one application gateway for multiple uses is more common here because not all SAP systems in the environment require public access. Multiple uses in this context include different web dispatcher ports for same SAP S/4HANA systems or different SAP environments. We recommend at least one application gateway per tier (production, nonproduction, and sandbox) unless the complexity and number of connected systems becomes too high.
- **SAP services**, like SAProuter, Cloud Connector, and Analytics Cloud Agent, are deployed based on application requirements, either centrally or split up. Production and nonproduction separation is often desired.
...
- For Application Gateway, we recommend a /24 subnet for scaling. If you want to limit the Application Gateway scale, you could use a smaller subnet, at a [minimum /26 or larger](/azure/application-gateway/configuration-infrastructure#size-of-the-subnet).
...
You can use SAProuter to enable groups like SAP support or your partners to access your SAP system. SAProuter runs on one VM in Azure. Route permissions for using SAProuter are stored in a flat file called *saprouttab*. The *saprouttab* entries allow connection from any TCP/IP port to a network destination behind SAProuter, typically your SAP system VMs. Remote access by SAP support relies on SAProuter. The main architecture uses the design described earlier, with a SAProuter VM running within the designated SAP perimeter virtual network. Through virtual network peering, SAProuter then communicates with your SAP servers that run in their own spoke virtual network and subnets.

SAProuter is a tunnel to SAP or to your partners. This architecture describes the use of SAProuter with Secure Network Communications (SNC) to establish an encrypted application tunnel (network layer 7) to SAP or partners. This architecture doesn't cover the use of an IPSec-based tunnel.

The following features help protect the communication path over the internet:

- Azure Firewall or a non-Microsoft NVA provides the public IP entry point into your Azure networks. Firewall rules limit communication to only authorized IP addresses. For your connection to SAP support, [SAP note 48243 - Integrating the SAProuter software into a firewall environment](https://launchpad.support.sap.com/#/notes/48243) documents SAP router IP addresses.
...
- You maintain SAProuter allow and deny rules in the *saprouttab* file, specifying who can contact SAProuter and which SAP system can be accessed.
...
This architecture focuses on internet-facing applications. Various options are available for clients that access SAP Fiori, the web UI of an SAP NetWeaver system, or another SAP HTTPS interface through an internal, private IP address. One option is to treat all access to Fiori as public access, through the public IP. Another option is to use direct network access through the private network to the SAP Web Dispatchers, bypassing Application Gateway entirely. A third option is to use [both private and public IP addresses](/azure/application-gateway/configuration-front-end-ip#public-and-private-ip-address-support) on Application Gateway, providing access to both the internet and the private network.
...
SAP BTP is a large set of SAP applications, SaaS or PaaS, that are typically accessed through a public endpoint via the internet. [SAP Cloud Connector](https://help.sap.com/docs/CP_CONNECTIVITY/cca91383641e40ffbe03bdc78f00f681/e6c7616abb5710148cfcf3e75d96d596.html) is often used to provide communication for applications that run in private networks, like an SAP S/4HANA system running on Azure. SAP Cloud Connector runs as an application in a VM. It requires outbound internet access to establish a TLS-encrypted HTTPS tunnel with SAP BTP. It acts as a reverse invoke proxy between the private IP range in your virtual network and SAP BTP applications. Because of this reverse invoke support, you don't need open firewall ports or other access for inbound connections, because the connection from your virtual network is outbound.

By default, VMs have [outbound internet](/azure/virtual-network/ip-services/default-outbound-access) access natively on Azure. The public IP address that's used for outbound traffic, when there's no dedicated public IP address associated with the VM, is randomly selected from the pool of public IPs in the specific Azure region. You can't control this selection. To ensure that outbound connections are made through a controlled and identifiable service and IP address, you can use one of the following methods:
...
For some application scenarios, SAP Analytics Cloud Agent is an application that's installed in a VM. It uses SAP Cloud Connector for SAP BTP connectivity. In this architecture, the SAP Analytics Cloud Agent VM runs in the SAP perimeter application subnet alongside the SAP Cloud Connector VMs. For information about the traffic flow from private networks like an Azure virtual network to SAP BTP via SAP Analytics Cloud Agent, see the [SAP documentation](https://help.sap.com/docs/SAP_ANALYTICS_CLOUD/00f68c2e08b941f081002fd3691d86a7/5339a2395ccd4befb047c625a15f8481.html).
...
Because of changes to default outbound access in Azure, ensure that a scalable outbound access is defined. For VMs that are behind an internal load balancer, like those in clustered environments, keep in mind that Load Balancer modifies the behavior for public connectivity. For more information, see [Outbound connectivity for SAP VMs](/azure/virtual-machines/workloads/sap/high-availability-guide-standard-load-balancer-outbound-connections).
...
For Linux operating systems, you can access the following repositories if you obtain the OS license from Azure. If you purchase licenses directly and bring them to Azure (bring your own subscription), contact the OS vendor about ways to connect to OS repositories and their respective IP address ranges.
