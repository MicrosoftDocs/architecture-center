...
This architecture diagram shows an SAP on Azure network that's segmented across two subscriptions in one region: a central hub subscription and an SAP production subscription. At the far left is an on-premises network connected through ExpressRoute to a zone-redundant gateway in the hub virtual network. Four subnets are located in the hub virtual network: a gateway subnet, an Azure Firewall subnet, an Application Gateway subnet, and a shared services subnet. The shared services subnet contains Microsoft Entra ID and DNS. To the right of the hub are two peered spoke virtual networks in the SAP production subscription: an SAP perimeter spoke virtual network and an SAP production spoke virtual network.  split into an Application subnet and a Database subnet. The SAP perimeter subnet contains SAProuter and Cloud Connector virtual machines, while the SAP production application subnet contains SAP Web Dispatcher virtual machines plus SAP ASCS and application server virtual machines, and the database subnet contains database virtual machines, private endpoint, and Azure Files NFS with a labeled database replication path. Two separate virtual network peering links connect the hub to each spoke, and there is no direct spoke-to-spoke link. At the top-left, an Internet cloud and external SAP endpoint connect to the architecture through public IP addresses; the HTTPS inbound path runs from Internet to a public IP in front of the firewall, then to AppGw/WAF, then to the SAP Web Dispatcher tier, and then to SAP application servers. A second labeled path for SNC communication SAProuter traffic runs from external SAP through the public IP and firewall to SAProuter in the perimeter subnet, then onward to SAP systems over peered virtual networks. Outbound traffic from SAP perimeter services, especially Cloud Connector, is shown as a BTP TLS tunnel that exits through firewall and public IP toward Internet or SAP BTP endpoints. The purpose of the design is to isolate internet-facing SAP components in a dedicated perimeter spoke, centralize ingress and egress controls in the hub, and allow controlled east-west traffic to SAP application and database tiers while preserving private service access through private endpoints.
:::image-end:::
