...
This architecture diagram shows an SAP on Azure network that's segmented across two subscriptions in one region: a central hub subscription and an SAP production subscription. At the far left is an on-premises network connected through ExpressRoute to a zone-redundant gateway in the hub virtual network. Four subnets are located in the hub virtual network: a gateway subnet, an Azure Firewall subnet, an Application Gateway subnet, and a shared services subnet. The shared services subnet contains Microsoft Entra ID and DNS. To the right of the hub are two peered spoke virtual networks in the SAP production subscription: an SAP perimeter spoke virtual network and an SAP production spoke virtual network. The perimeter network contains an application subnet that in turn contains SAProuter and a Cloud Connector VMs. The production network is split into an application subnet and a database subnet. The SAP production application subnet contains SAP Web Dispatcher, SAP ASCS, and application server VMs and a private endpoint. The database subnet contains database VMs that are connected with a database replication path. An NFS Azure file share is located outside of the production network and connects to the private endpoint. Two separate virtual network peering links connect the hub to each spoke, and there is no direct spoke-to-spoke link. At the top-left, an internet icon and external SAP endpoint connect to the architecture through public IP addresses. The HTTPS inbound path runs from the internet to a public IP in front of the firewall, then to the Application Gateway with Web Application Firewall, and then to the SAP Web Dispatcher tier. A second path for SNC communication SAProuter traffic runs from the external SAP endpoint through the public IP and firewall to SAProuter in the perimeter subnet. Outbound traffic from Cloud Connector is shown as a BTP TLS tunnel that exits through the firewall and public IP to the internet and SAP external endpoints.
:::image-end:::
...
- [Azure Load Balancer](/azure/well-architected/service-guides/azure-load-balancer) provides networking elements for the high-availability design of your SAP systems. For clustered systems, Load Balancer provides the virtual private IP address for the cluster service, like ASCS/SCS instances and databases running on VMs. You can also use Load Balancer to provide the IP address for the virtual SAP host name of non-clustered systems when [secondary IPs on Azure network cards](https://techcommunity.microsoft.com/t5/running-sap-applications-on-the/use-sap-virtual-host-names-with-linux-in-azure/ba-p/3251593) aren't an option. The use of Load Balancer instead of Application Gateway to address outbound internet access is covered later in this article.
...
- **Application Gateway**, like a load balancer, allows multiple back ends, front ends, HTTP settings, and rules. Using one application gateway for multiple uses is more common here because not all SAP systems in the environment require public access. Multiple uses in this context include different web dispatcher ports for same SAP S/4HANA systems or different SAP environments. We recommend at least one application gateway per tier (production, non-production, and sandbox) unless the complexity and number of connected systems becomes too high.
