...
This diagram presents an end-to-end SAP landscape that crosses two Azure regions: a primary region and a disaster recovery (DR) region. It uses numbered callouts to indicate the workflow. Callout 1 marks the on-premises network, which connects to callout 2, the Azure subscription for regional hubs. The connection uses ExpressRoute for the primary region, and ExpressRoute or VPN for the DR region. Callout 3 is a hub virtual network in the primary region, and callout 4 is a hub virtual network in the DR region. Each hub network contains subnets for a gateway, Azure Firewall, shared services, and Application Gateway. Shared services include Microsoft Entra ID and DNS. Callout 5 identifies the SAP nonproduction subscription, which contains separate spoke virtual networks for development, pre-production/QA, and sandbox environments, each labelled with callout 6. Each nonproduction spoke contains application and database subnets and a private endpoint and connects to either Azure Files or Azure NetApp Files with SMB/NFS shares. In the pre-production spoke, Azure NetApp files is in its own subnet. Callout 7 identifies the SAP production subscription, which contains the primary SAP production spoke, labelled 8, and a mirrored SAP production DR spoke labelled 9. These production spokes contain application, database, perimeter, and Azure NetApp Files subnets. Each one contains tiers for the SAP Web Dispatcher pool, SAP ASCS and application servers, databases, and SAProuter. Each one also contains a private endpoint. Hub-to-spoke connectivity is provided by virtual network peering. The diagram shows asynchronous database replication, asynchronous Site Recovery replication, and NFS/SMB file replication between the primary and DR production environments. The nonproduction environment and both production environments all contain callouts labelled 10. Each of these callouts is associated with a cloud witness, Data Factory, Data Lake Storage, and Recovery Services vaults. The primary production environment also contains Site Recovery. The Recovery Services vaults icon in the DR environment is labelled G(Z)RS. Callout 11 depicts SAP BTP access through Private Link service via DEV, QAS, and PRD subaccounts.
:::image-end:::
...
1. *Azure subscription: SAP non-production*. An Azure subscription for all nonproduction SAP workloads. It includes pre-production, quality assurance, development, and sandbox environments.
...
1. *SAP Business Technology Platform (BTP)*. The SAP BPT. The SAP environment accesses the BPT through Azure Private Link.
...
- *SAP nonproduction*. An SAP nonproduction subscription where nonproduction systems, including sandbox, development, quality assurance, or pre-production systems, reside.
...
**Use ExpressRoute for on-premises connection.** For SAP workloads, we recommend using ExpressRoute to connect the on-premises network to the hub virtual network and hub DR virtual network. You can use Azure Virtual WAN topology if you have global locations. Consider setting up a site-to-site (S2S) VPN as a backup to ExpressRoute or any third-party route requirements.
...
**Use a central firewall.** All the network traffic to the spoke virtual networks, including remote function call (RFC) connections, should pass through a central firewall in the hub virtual network. All the communication between the spoke virtual networks passes through the hub virtual network firewall. Network communication between the spoke virtual networks (spoke-to-spoke communication) passes through the hub virtual network firewall in the Azure Firewall subnet of the hub virtual network. Similarly, network communication between the spoke virtual networks and the on-premises network also passes through the hub virtual network firewall. The architecture uses virtual network peering to connect the various spoke virtual networks to the hub virtual network. You could also use a network virtual appliance (NVA) instead of a firewall. For more information, see [Create an NVA in the hub](/azure/virtual-wan/how-to-nva-hub).
...
This diagram shows a secured perimeter variation of an SAP production landscape in one Azure region, with numbered callouts that emphasize traffic flow between spokes through a central hub. On the far left, callout 1 is an on-premises network with a gateway that connects through ExpressRoute to callout 2, an Azure subscription for regional hubs. Inside that subscription, callout 3 is the hub virtual network, which contains a gateway subnet with a zone-redundant gateway, an Azure Firewall subnet with a firewall, a shared services subnet with Active Directory and DNS, and an Azure Bastion subnet; the hub also exposes a public IP endpoint. On the right, callout 7 is the Azure subscription for SAP production, which is split into two separate spoke virtual networks: callout 8, the SAP perimeter spoke, and callout 9, the SAP production spoke. In the upper perimeter spoke, one subnet hosts Application Gateway with Web Application Firewall and another subnet hosts SAProuter and Cloud Connector, with a public IP at the perimeter edge; inbound internet traffic for web services and Fiori is shown entering through this perimeter path. In the lower production spoke, one subnet hosts SAP Web Dispatcher pool, SAP ASCS, and application servers, another subnet hosts databases, and a separate storage area includes Azure Files or Azure NetApp Files, an NFS/SMB service, and a private endpoint. A labeled virtual network peering connection links the hub and the production spoke, and red directional arrows labeled as spoke-hub-spoke traffic flow show that communication from the production spoke to the perimeter spoke is forced through the hub rather than direct spoke-to-spoke bypass. On the far right, callout 10 groups connected platform services, including Cloud Witness, Data Factory, Data Lake, Recovery Services vaults, and Site Recovery, and the top-right private-link label indicates private connectivity for SAP BTP. The purpose of the architecture is to isolate internet-facing SAP components in a dedicated perimeter spoke while enforcing central inspection and control of inter-spoke and outbound traffic through the hub firewall.
:::image-end:::
