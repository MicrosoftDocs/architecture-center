...
From an architecture perspective, choose services that optimize for speed, cost, and options. Use managed services and platforms as a service (PaaS) like Azure App Service to get started quickly without worrying about complex infrastructure. Manage costs by choosing smaller instance sizes and free tiers while you're exploring, with the potential for auto-scale rules to scale if needed. Containers support developing with whatever tools make sense for you and give you flexible deployment options for the future.
...
Consider using our suggested first technology stack as documented here: [Core startup stack architecture](/azure/architecture/example-scenario/startups/core-startup-stack).  This architecture uses PaaS services for ease of implementation, can be started with a minimal scale, and uses container and open source technologies that can be easily deployed on different technology stacks as you mature.
...
- Enhancing resiliency through a [highly available multi-region deployment](https://learn.microsoft.com/en-us/azure/architecture/reference-architectures/app-service-web-app/multi-region)
- Enhancing security through a [network hardened technology stack](https://learn.microsoft.com/en-us/azure/architecture/example-scenario/security/hardened-web-app)
