...
This architecture considers the use of Private Endpoints for secure communication within the deployment. If you use FQDN rules with Azure AI Foundry Hub, Azure Firewall is deployed as the mechanism for implementing those rules. Azure Firewall serves as a critical tool for inspecting and controlling traffic in your managed Azure Virtual Network. When using Azure Firewall, it’s important to evaluate communication paths involving the internet and on-premises connections. Here are some decision points to consider when selecting Azure Firewall Basic versus Standard in you AI Foundry Hub deployment.
