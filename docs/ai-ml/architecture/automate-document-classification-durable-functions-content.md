...
1. The user provides a document file that the web app uploads. The file contains multiple embedded documents of various types, such as a PDF or multipage TIFF file. The document file is stored in Azure Blob Storage (**1a**). The web app adds a command message to a storage queue to initiate pipeline processing (**1b**).
...
- To facilitate global distribution, this solution stores metadata in Azure Cosmos DB. [Azure SQL Database](https://azure.microsoft.com/products/azure-sql/database) is another option for persistent storage of document metadata and information.
...
Many companies need to manage and process documents that they scan in bulk and that can contain several different document types, such as PDFs or multi-page TIFF images. These documents might originate from outside the organization, and the receiving company doesn't control the format.
...
Many of these custom solutions are based on the state machine workflow pattern. They use database systems to persist workflow state and use polling services that check for the states that they need to process. To maintain and enhance such solutions can increase complexity and effort.
...
- **Report titles.** Many government agencies and municipalities manage paper records that aren't in digital form. An effective automated solution can generate a file that contains all the documents that you need to satisfy a document request.
...
- Use reserved capacity and lifecycle policies to [rightsize](/azure/well-architected/service-guides/storage-accounts/cost-optimization) storage accounts.

- Use proper [AI Search planning](/azure/search/search-sku-manage-costs) for regional deployments and operational scale-up scheduling.

- Use [commitment tier pricing](/azure/ai-services/commitment-tier) for Document Intelligence to manage [predictable costs](/azure/ai-studio/how-to/costs-plan-manage).
