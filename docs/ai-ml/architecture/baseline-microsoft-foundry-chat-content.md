...
1. When the web application receives a user query or instruction, it invokes the purpose-built agent. The web application communicates with the agent endpoints via the [Microsoft Agent Framework](/agent-framework/overview/). The framework invokes the server-side agent definition through the [OpenAI-compatible Responses API](/azure/foundry/agents/quickstarts/prompt-agent#chat-with-the-agent), so changing the agent to another supported model doesn't require changes to the client's HTTP call. The web application calls the agent over a private endpoint and authenticates to Foundry by using its managed identity.
