...
## Scope note: Classic ML MLOps vs. GenAIOps

This article focuses on traditional (predictive, tabular, classical ML) lifecycle capabilities. Generative AI operations (prompt lifecycle, retrieval augmentation, output safety, token cost governance) add extra layers and should be treated as an overlay rather than replacing these levels. Reference this [GenAIOps article](https://learn.microsoft.com/en-us/azure/architecture/ai-ml/guide/genaiops-for-mlops) for those concerns; do not conflate prompt iteration mechanics with the reproducible training–deployment loop described here.
