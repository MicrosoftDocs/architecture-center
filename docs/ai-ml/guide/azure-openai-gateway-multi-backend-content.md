...
- Ensure the gateway is using the `Retry-After` information available in the HTTP response from Azure OpenAI when supporting failover scenarios at the gateway. That authoritative information should be used to control your circuit-breaker implementation. Do not continuously hit an endpoint that returns a `429 Too Many Requests`, rather break the circuit for that model instance.
...
A topology that includes multiple Azure OpenAI instances in a single region across multiple subscription supports the following use cases:
...
- Enables you to obtain additional consumption-based quota, since subscription boundary is a factor in available for the consumption model. This additional quota can be used to support highly concurrent consumption.
...
- Enables a data residency & compliance design
...
For business-critical architectures that must survive a complete regional outage, a global, unified gateway helps eliminate failover logic from client code. This requires that the gateway itself remains unaffected by a regional outage.
...
Your gateway must honor throttling and availability signals from the Azure OpenAI instances and remove faulted backends from the pool until safe to re-add the faulted or throttled Azure OpenAI instance. The gateway should retry the current request against another backend instance in the pool upon fault, before falling back to returning a gateway error. The gateway's health check should signal unhealthy when no backend Azure OpenAI instances are available.
