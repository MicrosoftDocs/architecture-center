...
[SynapseML](https://aka.ms/spark/) (formerly known as SynapseML) is an open-source library that simplifies the creation of massively scalable machine learningRemove redundant parenthetical pipelines. SynapseML provides APIs for a variety of different machine learning tasks such as Text Analytics, vision, anomaly detection, and many others. SynapseML is built on the [Apache Spark](https://spark.apache.org/) distributed computing framework and shares the same API as the SparkML/MLLib library, allowing you to seamlessly embed SynapseML models into existing Apache Spark workflows.
