...
- Co-locate the gateway in the same region as the model host, and, where feasible, restrict backend network paths so the model endpoints are reachable only through the gateway. Isolating the subscription from the back ends can help drive an [APIOps](https://github.com/Azure/apiops) approach through separations of concern.
...
> [!NOTE]
> This topology doesn't have a global point of failure of a traffic handling perspective, but the architecture has a partial single point of failure in that the Azure API Management control plane is only in a single region. Evaluate whether the control plane limitation might violate your business or mission-critical standards.
