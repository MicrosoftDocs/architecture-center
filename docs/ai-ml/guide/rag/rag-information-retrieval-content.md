...
- What type of search you want to perform, such as vector, keyword, or hybrid
...
Your prompt orchestrator might take a static approach or a dynamic approach that combines approaches based on context clues from the prompt. The following sections address these options to help you experiment to find the right approach for your workload.

### Search types

Search platforms generally support full-text and vector searches. Some platforms, like AI Search, support hybrid searches. For more information about various vector search offerings, see [Choose an Azure service for vector search](../../../guide/technology-choices/vector-search.md).

#### Vector search

[Vector searches](/azure/search/vector-search-how-to-query) compare the similarity between the vectorized query (prompt) and vector fields.

> [!IMPORTANT]
> You should perform the same [cleaning operations](./rag-enrichment-phase.yml#cleaning) that you performed on chunks before you embed the query. For example, if you lowercased every word in your embedded chunk, you should lowercase every word in the query before embedding.

> [!NOTE]
> You can perform a vector search against multiple vector fields in the same query. In AI Search, this practice is considered a hybrid search. For more information, see that section.
...
The code that embeds the query preprocesses the query first. That preprocess should be the same code that preprocesses the chunks before embedding. The embedding model must be the same embedding model that embedded the chunks.

#### Full-text search

[Full-text searches](/azure/search/search-lucene-query-architecture) match plain text that's stored in an index. It's common practice to extract keywords from a query and use those extracted keywords in a full-text search against one or more indexed columns. You can configure full-text searches to return matches where any terms or all terms match.

You must experiment to determine which fields to run full-text searches against. As described in the [enrichment phase article](./rag-enrichment-phase.yml#augmenting-chunks), you should use keyword and entity metadata fields for full-text searches in scenarios where content has similar semantic meaning but entities or keywords differ. Other common fields to consider for full-text search include title, summary, and the chunk text.
...
The following sample code performs a full-text search against the title, content, and summary fields. It also performs vector searches against the contentVector and questionVector fields.
...
```
 ...
 The AI Search platform runs all the queries in parallel, reranks the results, and returns the top *retrieve_num_of_documents*.
...
You can use a language model to augment your query. But not all queries can be augmented. If you have context, you can pass it along to your language model to augment the query. If you don't have context, you have to determine if your language model has information that you can use to augment the query. For example, if you use a large language model, like a GPT model, you can determine if the internet has information readily about the query. If so, you can use the model to augment the query. Otherwise, you shouldn't augment the query.
...
Identify any other aspects of the topic not covered in the provided context that enrich the explanation.
...
You should determine whether the query requires multiple searches before you run any searches. If you require multiple subqueries, you can run [manual multiple queries](#manual-multiple) for all the queries. Use a language model to determine whether multiple subqueries are recommended.

The following prompt categorizes a query as simple or complex. Complex queries require multiple queries. For more information, see [RAG Experiment Accelerator GitHub repository](https://github.com/microsoft/rag-experiment-accelerator/blob/development/rag_experiment_accelerator/llm/prompt/prompt.py).
...
2. Complex, multipart question
  ...
  c. Breaking the question down into subquestions for each component allow for better results
  d. The question is open-ended and likely to have a complex or nuanced answer
  e. Answering it might require synthesizing information from multiple sources
...
- Include an additional subquery by using one more rule: Identify the main subject and object. Swap their positions in the sentence. Adjust the wording to make the new sentence grammatically correct and meaningful. Ensure that the new sentence asks about the original subject.
...
Provide your analysis in the following YAML format strictly adhering to the following structure, and don't output anything extra including the language itself:
...
You can use multiple query translations. You can even use all of these translations in conjunction. The following diagram shows an example of how you can combine these translations into a pipeline.
...
- Vector and keyword searches aren't always accurate. You can increase the count of documents that you return from your search, which can include valid results that might otherwise be ignored, and use reranking to evaluate the results.
...
The following example uses the [CrossEncoder provided by Hugging Face](https://huggingface.co/sentence-transformers) to load the Roberta model. It iterates over each chunk and uses the model to calculate similarity, which gives them a value. It sorts the results and returns the top *N* results. For more information, see [RAG Experiment Accelerator GitHub repository](https://github.com/microsoft/rag-experiment-accelerator/blob/development/rag_experiment_accelerator/reranking/reranker.py).
