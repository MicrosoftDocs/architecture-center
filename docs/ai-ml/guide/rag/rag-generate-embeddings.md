...
The vocabulary of an embedding model is important because the model handles words that aren't in its vocabulary in a unique manner. If a word isn't in the model's vocabulary, the model still calculates a vector for it. Many models break the words into subwords. They treat the subwords as distinct tokens, or they aggregate the vectors for the subwords to create a single embedding.
...
> [!IMPORTANT]
> For any model that you choose, verify that the license suits your needs and that the model provides the necessary language support.
...
### Preprocess multimodal content for embedding

Before you generate embeddings for multimodal content, convert non-text media into a text representation that an embedding model can process. [Azure Content Understanding](/azure/ai-services/content-understanding/overview) can serve as a preprocessing step for multimodal content before embedding. Content Understanding transforms documents, images, video, and audio into structured Markdown and JSON output that you can then embed by using standard text embedding models.

For example, the Content Understanding [video analyzers](/azure/ai-services/content-understanding/video/overview) produce Markdown output that includes scene descriptions, inline transcripts in WebVTT format, and key-frame references. Similarly, [audio analyzers](/azure/ai-services/content-understanding/audio/overview) generate transcripts with speaker diarization and summaries. You can embed this structured text output rather than attempting to process raw media files directly with embedding models.

The following example shows how to preprocess Content Understanding output and embed it into a vector store. Replace the placeholder values with your own resource settings, and use the actual response returned by your Content Understanding analyzer:
...
```python
    ...
    # analyzer_content holds the structured output that your Content Understanding
    # analyzer returns. Convert each content item to a string and wrap it in a
    # LangChain Document so that you can embed it.
    document_splits = [ str(content) for content in analyzer_content[0]["content"] ]
]
...
For the full implementation, see the [Content Understanding RAG Python code samples](https://github.com/Azure-Samples/azure-ai-search-with-content-understanding-python).
...
:::image type="complex" border="false" source="./_images/visualize-embeddings.png" alt-text="Graph that shows a visualization of an embedding. The image shows blue dots that are plotted on an X-Y scale." :::
   Two arrows point to plot points that are near one another. Another arrow shows a plot point far away from the other two.
:::image-end:::
