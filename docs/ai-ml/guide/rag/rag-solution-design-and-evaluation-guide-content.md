...
2. The data pipeline processes each document with the following steps:
    ...
    2.1 Chunks documents - Breaks down the documents into semantically relevant parts that ideally have a single idea or concept.
    2.2 Enrich chunks - Adds metadata fields created from the content in the chunks to the chunk field, such as title, summary, and keywords.
    2.3 Embed chunks - Uses an embedding model to vectorize the chunk and any other metadata fields that are used for vector searches.
    2.4 Persists chunks - Stores the chunks in the search index.
