...
Currently, many generative AI solutions use existing foundation language models that do not require fine-tuning before use. That said, there are use cases that can and do benefit from either fine-tuning a foundation model or training a new small generative AI model, such as a small language model (SLM).
...
:::image type="complex" source="_images/rag-architecture.svg" lightbox="_images/rag-architecture.png" alt-text="Diagram showing retrieval-augmented generation (RAG) architecture." border="false":::
   The diagram shows two flows. The first flow shows a user with an arrow pointing to a box called 'Intelligent application'. The 'Intelligent application' box points to an 'Orchestrator' box. The 'Orchestrator' box pointts to both an 'Azure OpenAI service' box and an 'Azure AI Search' box. The other flow shows 'Documents' pointint to a process flow with the following steps: 1. Chunk documents, 2. Enrich chunks, 3. Embed chunks, and 4. Persist chunks. That process flow points to the same 'Azure AI Search box' as the first flow.
:::image-end:::
...
Deployments of the new generative AI concerns, such as the orchestrator, should follow proper operational procedures like rigorous testing, including unit tests, integration tests, A/B tests, and end-to-end tests, and additionally roll out strategies such canary or blue/green deployments.
...
Operational monitoring is concerned with observing the ongoing operations of the system, including data operations (DataOps) and model training. You are looking for deviations including errors, changes to error rates, and changes to processing times.
