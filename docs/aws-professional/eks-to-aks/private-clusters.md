...
You can use [Azure Bastion](/azure/bastion/bastion-overview) in two ways to work with a private cluster. The first way is to connect to a jump box management VM in the same virtual network or a peered virtual network. Azure Bastion is a fully managed platform as a service (PaaS) that you can use to connect to a VM via your browser and the Azure portal. Azure Bastion provides Remote Desktop Protocol (RDP) or SSH VM connectivity over Transport Layer Security (TLS) directly from the Azure portal. When VMs connect via Azure Bastion, they don't need a public IP address, agent, or special client software. This approach resembles an Amazon EC2 bastion host that you sign in to and run `kubectl` from, but you don't manage the connectivity to the VM yourself.

The second way is to use Azure Bastion native client tunneling to reach the private API server endpoint directly from your local machine, without a jump box VM and without a VPN or ExpressRoute gateway. Azure Bastion establishes an encrypted TLS tunnel by using outbound connectivity over port 443 and maps the private API server to a local endpoint. Your local tools, such as `kubectl` and `helm`, then run against the private cluster through this tunnel as if the API server were directly reachable. Use this option when you want to keep your existing local workflow, in contrast to `az aks command invoke`, which runs your commands remotely through the Azure API rather than from your local machine.
