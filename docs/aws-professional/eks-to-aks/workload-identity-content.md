...
|----------------------------|-----------|----------|
| Control plane | AKS Cluster Name | Used by AKS control plane components to manage cluster resources including ingress load balancers and AKS managed public IPs, cluster autoscaler, and Azure Disk and Azure File CSI drivers | Contributor role for node resource group | Supported
| Kubelet | AKS Cluster Name-agentpool | Authentication with Azure Container Registry | NA (for kubernetes v1.15+) | Supported
...
| OSS project | aad-pod-identity | Lets applications access cloud resources securely with Azure AD | NA | See steps to grant permission at [AAD Pod Identity](https://github.com/Azure/aad-pod-identity#role-assignment).
