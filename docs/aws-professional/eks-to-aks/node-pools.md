...
In both Amazon EKS and AKS, the cloud platform provides and manages the control plane layer, and you manage the node layer. This division applies to standard AKS clusters. [AKS Automatic](/azure/aks/intro-aks-automatic) extends platform management to the system node pool, so Azure operates that node pool while you continue to manage your application node pools. The following diagram shows the relationship between the control plane and nodes in AKS Kubernetes architecture.
