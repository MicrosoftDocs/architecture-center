...
In both Amazon EKS and AKS, the cloud platform provides and manages the control plane layer, and the customer manages the node layer. This division applies to standard AKS clusters. [AKS Automatic](/azure/aks/intro-aks-automatic) extends platform management to the system node pool, so Azure operates that node pool while you continue to manage your application node pools. The following diagram shows the relationship between the control plane and nodes in AKS Kubernetes architecture.
...
The [nodes](/azure/aks/concepts-clusters-workloads#nodes), also called *agent nodes* or *worker nodes*, host the workloads and applications. In standard AKS clusters, you fully manage and pay for the agent nodes that are attached to the AKS cluster. In AKS Automatic clusters, Azure provisions and operates the system node pool for you, so you manage and pay only for the node pools that run your application workloads.
...
In AKS Automatic clusters, Azure provisions and operates a managed system node pool for the core Kubernetes system components and performs the scaling, patching, and upgrades for those system nodes. Your application workloads still run on separate user node pools, which maintains the separation between system and application workloads.
