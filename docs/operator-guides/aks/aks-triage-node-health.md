...
In private clusters, control plane communication may be broken due to DNS resolution issues as well. You can check if the Kubernetes API server DNS name resolves to the private IP of the API server. A common cause of DNS resolution failure is the misconfiguration of a custom DNS server. When using custom DNS servers, check if these servers are specified as DNS servers on the virtual network where nodes are provisioned. Also, check that the AKS private API server resolves through the custom DNS server. 
