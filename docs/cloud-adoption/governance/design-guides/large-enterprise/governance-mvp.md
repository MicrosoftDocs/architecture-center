...
Any governance foundation requires a few simple principles as the root of enabling rapid governance evolutions. These are the first of the three Cloud Governance Disciplines to approach in any governance process. Each will be expanded upon in this article.

To establish the starting point, this article will discuss the high-level strategies behind Identity Management, Security Management, and Configuration Management that are required to create a Governance minimally viable product (MVP), which will serve as the foundation for all adoption.
...
2. Determine a policy enforcement pattern.
...
Identity management is the fundamental starting point for all governance. Without a sound strategy for identity management, the users interacting with the cloud foundation can’t be verified or controlled. Before attempting to apply governance, identity must be established. That strategy will then be enforced by the governance solutions.

In this design guide, the Replication pattern has been implemented by the identity management team.

- RBAC will be provided by Azure Active Directory, leveraging the directory synchronization or "Same Sign-On" that was implemented during the Office 365 implementation.
  - See Reference Architecture for Azure AD Integration for implementation guidance.
- The Azure Active Directory tenant will also govern authentication and access for assets deployed to Azure.
...
Given the lack of requirements, IT security is playing it safe and has required a Demilitarized Pattern. This means that governance concerning the Azure deployments themselves will be very light.
...
The first decision to be made regarding configuration management is the pattern for enforcement. In this use case, the governance team decided to implement the Ongoing Enforcement pattern.
...
If any of the patterns selected in this design guide don't align with the reader's requirements, alternatives to each pattern is available in the list of links below.
