...
- Tailor consistency per component (check [using tactical DDD to design microservices](../model/tactical-ddd.yml) for further info) and use eventual consistency where possible. Understand the places in the system where you need strong consistency or ACID transactions, and the places where eventual consistency is acceptable.
