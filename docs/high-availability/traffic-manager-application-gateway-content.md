...
- **VMs:** VMs are the primary cost driver because every tier in both regions runs compute continuously. Use [Azure Reserved Virtual Machine Instances](/azure/virtual-machines/prepay-reserved-vm-instances) or [Azure savings plans for compute](https://docs.azure.cn/en-us/cost-management-billing/savings-plan/savings-plan-compute-overview). Reserved instances work well for the minimum always-on capacity, while savings plans provide flexibility if VM sizes change over time.
