...
AKS clusters should be created with the NetworkPolicy plugin. In AKS, you have the option between Azure or Calico as your Network Policy plugin. With Calico Network Policy you could either use Kubenet or Azure CNI, but for the Azure Network Policy it is only with Azure CNI (not Kubenet). Network Policies for Windows nodes are supported with Calico only. Both Azure and [Calico](https://www.tigera.io/project-calico/) Network Policy plugins are open source. Tigera, the owner and inventor of Project Calico, offers a [comprehensive PCI solution](https://www.tigera.io/lp/kubernetes-pci-compliance/?utm_campaign=calicocloud&utm_medium=digital&utm_source=microsoft_aks_pciwhitepaper) that addresses many of the firewall requirements below.
