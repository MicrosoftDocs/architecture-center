...
Another article, [Microservices architecture on AKS](./aks-microservices.md), and its companion [Microservices Reference Implementation](https://github.com/mspnp/microservices-reference-implementation) provide a starting configuration for basic AKS microservices deployments. That article focuses on the infrastructure and DevOps considerations of running a microservices architecture on AKS. That basic implementation doesn't cover some native Kubernetes features like the Horizontal Pod Autoscaler (HPA).
