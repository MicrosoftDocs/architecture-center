...
- **Backup**: To protect sensitive customer data and meet stringent compliance requirements, implement a robust backup and restore strategy for Azure Red Hat OpenShift (ARO).
    ...
    - For a backup and disaster recovery workflows—including scheduled backups, remote object store replication, and data mover support—consider the Red Hat guide [Backup and Restore for ARO using OADP](https://cloud.redhat.com/experts/aro/backup-restore/). This approach is recommended for production environments with strict RTO/RPO and compliance objectives.
