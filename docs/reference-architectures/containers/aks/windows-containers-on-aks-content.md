...
The network design used in this architecture is based off of the [design](/azure/architecture/reference-architectures/containers/aks/baseline-aks#network-topology) used in the AKS Baseline architecture and therefore shares all of the core components with that design except for the following changes.

- Due to limitations inherent to Azure App Gateway, a new ingress solution has been implemented for this architecture and is described in detail below. 
- A subnet dedicated to the Domain Controller and the Azure Application Proxy has been deployed.

The diagram below shows the hub and spoke network design used in this architecture.
...
All [guidance](/azure/architecture/reference-architectures/containers/aks/baseline-aks#egress-traffic-flow) found in the AKS Baseline article apply here with an additional recommendation for Windows workloads: In order to take advantage of automatic Windows Server updates, you can whitelist the [required FQDNs](/azure/aks/outbound-rules-control-egress#windows-server-based-node-pools-required-fqdn--application-rules) in your Azure Firewall ruleset.
...
Containers running Windows Server 2019 are currently supported by Azure Monitor and [Container Insights](/azure/azure-monitor/containers/container-insights-overview). Support for containers running Windows Server 2022 is currently in public preview. Log Monitor allows you to pull logs from Windows services and application events and port them to STDOUT for consumption by kubectl logs.
