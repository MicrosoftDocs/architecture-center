...
Enable network policy when you create the cluster or on an existing cluster. You have a few choices for technologies that implement `NetworkPolicy`. Azure network policy requires Azure CNI. Other options include Calico network policy, a well-known open-source option. Consider Calico if you need to manage cluster-wide network policies. Calico isn't covered under standard Azure support.
