...
This architecture focuses on day-0 cluster provisioning. When you evolve this baseline to support stateful workloads in day-2 operations, employ [AKS Backup](/azure/backup/azure-kubernetes-service-backup-overview) so that persistent volumes and cluster resource state are recoverable within your defined recovery-point and recovery-time objectives. From a reliability perspective, AKS Backup satisfies your data preservation requirements during both routine recovery and regional disaster scenarios. From an operational excellence perspective, Azure Policy enforces that backup is configured on your clusters, and Azure Monitor surfaces backup job health through the same observability stack.

If you plan to adopt AKS Backup in the future, account for the following architectural considerations in your day-0 design:
...
- **Trusted access.** AKS Backup requires [trusted access](/azure/aks/trusted-access-feature) between the Backup vault and the AKS cluster, regardless of whether the cluster is public, private, or IP-restricted. Ensure your IaC templates include a point where trusted access is enabled as part of the cluster provisioning or day-2 onboarding pipeline.
- **RBAC permissions.** The Backup vault's managed identity requires a set of permissions on the AKS cluster to configure and execute backups. The backup extension also creates a user identity with permissions on the storage account where backups are stored. Plan these role assignments into your identity governance from the start.
- **Network egress.** The backup extension communicates with Azure Backup services from within the cluster. Account for the required outbound endpoints in your Azure Firewall rules.
