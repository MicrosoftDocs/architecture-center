...
For additional information, see [Private Link deployment options](../../../guide/networking/private-link-hub-spoke-network.yml#decision-tree-for-private-link-deployment).
...
In this reference implementation, managed identities for pods is facilitated through [Azure AD Workload Identity for Kubernetes](/azure/aks/workload-identity-overview). This integrates with the Kubernetes native capabilities to federate with any external identity providers. For more information about Azure AD workload identity federation, see the following [overview](/azure/active-directory/develop/workload-identity-federation).
