...
Once the AKS cluster upate blueprint is determined, that can be easily mapped into the available update options for AKS nodes and AKS cluster version:
...
- AKS nodes:
  ...
  1. Automatic Unattennded updates: AKS execute native OS updates. This gives predictability by configuring maintenance winodws aligned with what is good for the business, it might be based on peak hours and what is best for operations. The level of control is low since it is not possible to know in advance what is going to be specifically installed within the AKS node.
  1. Automatic Node image updates: This automatic updates are desired. For security-labeled VHD updates, it is recommended to configure daily maintenance window giving the less predictability and less control. Regaular VHD updates can be configured with a weekly maitenance windows, every two weeks or even monthly. Depending on the maintenance window, predictability varies offering more or less flexiblity to be aligned with the business requirements. The level of control is low since it is not possible to know in advance what specific binaries were included into a new VHD.

- AKS cluster version:
  1. None/Manual updates: This is for immutable infrastructure or when manual updates are the preference. This achieves the greater level predictability and control over the AKS nodes updates. Opt-in for this is recommended, since this is leaving in full control ginving the chance to test a new AKS cluster version (i.e. 1.14.x to 1.15.x) in lower environments before hitting production.
  1. Automatic updates: A production cluster is not recommended to be automically patched or updated in any fashion (i.e. 1.16.x to 1.16.y)  to new AKS cluster version available without being properly tested in lower environments. While Kubernetes upstrem releases and AKS version are coordinateed providing with regular cadence, the recommendation is stil to be defensive with AKS clusters in production increasing the predictability and control over the update process. Consider this for lower environments and get prepared for updating your production AKS cluster later after testing over them.
