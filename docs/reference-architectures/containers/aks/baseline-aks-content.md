...
If Private Link or service endpoints aren't an option, you can reach other services through their public endpoints and control access through Azure Firewall rules and the firewall built into the target service. Egress flows are source network address translated (SNAT) to one of the firewall's attached public IP addresses per flow, and the selection isn't deterministic. Add the entire set of attached public IPs to the target service's IP allowlist, or use a [public IP address prefix](/azure/virtual-network/ip-services/public-ip-address-prefix) to express that set as a contiguous range.
