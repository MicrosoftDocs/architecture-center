This article describes the design and implementation of a blue-green deployment of Azure Kubernetes Service (AKS) that uses Azure managed services and native Kubernetes features. Adoption of this pattern improves operations and availability when deploying changes, including upgrades, to AKS clusters.
...
*Download a [Visio file](https://arch-center.azureedge.net/blue-green-deployment-for-aks.vsdx) of this architecture.*
...
This pattern is like a state machine, in which there are clear states and transitions. The blue and green clusters will be running at the same time, but only for a limited period of time, the main advantage of this is to optimize costs and operational efforts.

Here are the 5 stages of the pattern:

1. T0: The blue cluster is on.
2. T1: Deploy the green cluster.
3. T2: Sync the Kubernetes state between the blue and green clusters.
4. T3: Switch traffic to the green cluster.
5. T4: Destroy the blue cluster.

This pattern is recursive, and blue and green clusters swap roles in different iterations of the pattern. In one iteration, the blue cluster is the current active cluster. In the next, the green is the current active cluster.

The triggers to transition from stage to stage can be automated. Until that's achieved, some of them are manual. The triggers of the transitions are related to:

- Specific workload metrics, service-level objectives (SLOs), and service-level agreements (SLAs). They're mainly used in the T3 stage in order to validate the overall state of the AKS cluster before switching the traffic.
- Azure Platform metrics, in order to have data to evaluate the status and health of the workloads and the AKS cluster. They're used in all of the transitions.
...
- A DNS record dedicated to the blue and green clusters IP
  ...
  Then you can use these host names directly or as [backend pool](/azure/application-gateway/application-gateway-components) configuration in the application gateway that's on the top of each cluster.
...
#### T4: The blue cluster is destroyed
