...
If your workload has typical app hosting needs and you want to automate common operator tasks, [AKS Automatic](/azure/aks/intro-aks-automatic) can be a good starting option for production. AKS Automatic includes built-in configuration for node management, scaling, security policies, and upgrades. It also provisions and operates a managed system node pool for the core Kubernetes system components, so you don't create, scale, patch, or upgrade system node pools yourself. This approach reduces day-2 management overhead compared to clusters where you own the system node pool lifecycle.
