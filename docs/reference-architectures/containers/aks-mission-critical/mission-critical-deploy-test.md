...
Pushes to a geo-replicated registry land in the registry's home region first, then replicate asynchronously to each replica region. Until replication completes, pulls from a regional stamp can fail with `manifest unknown` even though the push succeeded. Build retry-with-backoff or a call to `az acr replication list` into the application deployment step so it tolerates this propagation window, and keep publish steps idempotent so a retry doesn't re-tag or rebuild artifacts with different digests.
