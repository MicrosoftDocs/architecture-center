...
Users interact directly with the `CatalogService`, so this part of the workload must respond under any load. The minimum number of instances for each cluster is a multiple of the number of availability zones in the region. This helps provide event distribution across all availability zones in that Azure region. The minimum instance count should be at least equal to the number of availability zones, with additional instances for redundancy within zones. The horizontal pod autoscaler (HPA) in AKS automatically adds more pods as needed. The Azure Cosmos DB autoscale feature can dynamically increase and reduce request units (RUs) available for the collection. The `CatalogService` and Azure Cosmos DB combine to form a scale unit within a stamp.
