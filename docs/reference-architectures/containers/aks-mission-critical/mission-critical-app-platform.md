...
To increase reliability, the cluster is configured to **use all three availability zones** in a given region. Additionally, to enable [AKS Uptime SLA](/azure/aks/free-standard-pricing-tiers) with guaranteed 99.95% SLA availability of the AKS control plane, the cluster must be on either **Standard**, or **Premium** tier.
