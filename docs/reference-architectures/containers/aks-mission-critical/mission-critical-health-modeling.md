...
- **Application Insights telemetry**: Telemetry data from the application is collected using Application Insights. Using the Application Insights SDK, the code has been instrumented to collect data on the performance of our application. Critical information, such as the resulting status code and duration of dependency calls and counters for unhandled exceptions is collected to be used in the Health Model and is available for alerting and troubleshooting.
...
Azure Mission-Critical uses different Kusto Query Language (KQL) queries to implement custom queries as functions to retrieve data from Log Analytics. These queries are stored as individual files in our code repository, separated for global and stamp deployments. They are imported and applied automatically via Terraform as part of each infrastructure pipeline run.

This approach separates the query logic from the visualization layer. This allows the Log Analytics queries to be called directly from code, for example from the HealthService API, or from a visualization tool such as Azure Dashboards, Monitor Workbooks, or Grafana.
...
These alerts form an extension of the health model, by alerting the operator to a change in health state, either to degraded/yellow state or to unhealthy/red state. By setting the alert to the root node of the Health Model, the operator is immediately aware of any business level impact to the state of the solution: After all, this root node will turn yellow or red if any of the underlying user flows or resources report yellow or red metrics. For troubleshooting, the operator can subsequently direct their attention to the Health Model visualization.

## Failure analysis

Composing the failure analysis is mostly a theoretical planning exercise. This theoretical exercise should be used as input for the automated failure injections that are part of the continuous validation process. By simulating the failure modes defined here, we can validate the resiliency of the solution against these failures to ensure they will not lead to outages.

The following table lists possible failure scenarios of the various components of the Azure Mission-Critical reference implementation. The table lists risks for the individual components and evaluates if their failure can cause an outage of the entire application. This list isn't a complete list, as there can be failure cases which haven't been thought of.
