...
| **Azure Kubernetes Service** | External container images / registry unavailable | Some components like cert-manager and ingress-nginx require downloading container images and helm charts from external container registries (outbound traffic). In case one or more of these repositories or images are unavailable, new instances on new nodes (where the image isn't already cached) might not be able to start. *Possible mitigation*: In some scenarios, it could make sense to **import external container images into the per-solution container registry**. This scenario adds extra complexity and should be planned and operationalized carefully. | Partially (during scale and update/upgrade operations) |
