...
- *Optional* **AKS Fleet hub cluster (Microsoft-managed):** Optionally, a single [AKS Fleet Hub cluster](/azure/kubernetes-fleet/concepts-fleet#what-are-hub-clusters) can be deployed to support specific features of AKS Fleets, such as fleet-based workload propagation. The hub cluster is a regionally scoped Azure resource that helps to manage multiple member clusters. It's best to deploy the hub cluster as a private AKS cluster, which must be reachable from member clusters to support heartbeat signals and to perform configuration reconciliation processes.
