...
For a customer-managed VM, all these rules apply. However, the current recommendation is to use [Azure Bastion](/azure/bastion/bastion-overview), a managed jumpbox solution that allows for HTML5 access to RDP or SSH behind Microsoft Entra ID Protection. This is a much simpler solution that ultimately has a lower total cost of ownership (TCO) for the customer.
