...
There are two articles in this series and a reference implementation (sample web app). The companion article describes how to [apply the pattern](apply-pattern.yml). A [reference implementation](https://github.com/Azure/reliable-web-app-pattern-java#reliable-web-app-pattern-for-java) of the pattern is available for you to deploy.
