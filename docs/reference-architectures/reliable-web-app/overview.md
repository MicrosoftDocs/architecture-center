---
title: Reliable web app pattern
description: Learn about the reliable web app pattern.
author: stephen-sumner    
ms.author: ssumner
ms.reviewer: ssumner
ms.date: 04/28/2022
ms.topic: conceptual
ms.service: architecture-center
ms.subservice: azure-guide
azureCategories:
    - web
    - developer-tools
    - databases
    - devops
products:
  - azure
  - azure-app-service
  - azure-cache-redis
categories:
  - web
---

# Reliable web app pattern

The reliable web app pattern provide implementation guidance to replatform web apps moving from on-premises to the cloud. The pattern focuses on minimal code changes, managed services, and reliability design patterns to you rapidly adopt the cloud.

[![Diagram showing the principles of the reliable web app pattern](images/reliable-web-app-overview.png)](images/reliable-web-app-overview.png)

The reliable web app pattern outlines you prescriptive architecture and code guidance. It builds on the pillars of the Azure Well-Architected Framework while focusing on key principles for cloud adoption. These principles include infrastructure provisioning, identity-centric security, secure ingress, cost-optimization, and observability.

## Next steps

>[!div class="nextstepaction"]
>[Reliable web app pattern for .NET](./dotnet/plan-implementation.yml)

>[!div class="nextstepaction"]
>[Reliable web app pattern for Java](./java/plan-implementation.yml)
