...
- **Azure Front Door**. Azure Front Door is configured with a custom domain name and TLS certificate named for the hostname of the application, for example `www.contoso.com`. Azure Front Door is configured with multiple origins, one per region where the application is deployed. Each origin is an Azure Application Gateway. Azure Front Door service can use multiple load balancing configurations to forward the request to one region or the other. The sample deployment available for download is configured with an equal-weight load balancing rule between two regions.

- **Application Gateway**. Each region has an Application Gateway deployed with a Web Application Firewall. The Web Application Firewall only allows incoming calls from your specific Azure Front Door profile. The Application Gateway is also configured with the same custom domain name and TLS certificate name as Azure Front Door. Application Gateway forwards allowed traffic to the Azure Spring Apps load balancer. The Application Gateway is optional in this solution. Take a look at the [alternatives](#alternatives) for this solution, where an architecture with only one reverse proxy is described.
