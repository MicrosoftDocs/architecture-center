...
> We are working on a reference implementation (RI) to accompany this article, which we expect to publish in early 2019. This article will be updated to include additional best practices from that RI. Using Service Fabric for deploying and managing containers is beyond the scope of this document. 
While most recommendations apply to both Windows and Linux environments, this reference implementation is for Windows.
