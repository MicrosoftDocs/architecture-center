...
1. Create an AzureML Workspace. This will also create an Azure Container Registry to host your Docker Images

1. Create an AzureML GPU Cluster.

1. Submit jobs. For each job with unique dependencies a new Docker Image will be built and pushed to your container registry. During execution the appropriate Docker image and execute your script inside it.
...
For production deployments, consider deploying the AzureML cluster into a subnet of a virtual network that you specify. This allows the compute nodes in the cluster to communicate securely with other virtual machines or with an on-premises network. You can also use [service endpoints][endpoints] with blob storage to grant access from a virtual network.
...
AzureML offers many ways to [instrument your experiments][azureml-logging], the stdout/stderr from your scripts are automatically logged. These logs are automatically synced to your workspace Blob storage. You can either view these files through the Azure portal, download or stream them using the Python SDK or AzureML CLI. If you log your experiments using Tensorboard these logs will also be automatically synced and you can either access them directly or use the AzureML SDK to stream them to a [Tensorboard session][azureml-tensorboard]. 
...
[azureml-logging]: https://docs.microsoft.com/en-us/azure/machine-learning/service/how-to-track-experiments
