...
Download a [Visio file][visio-download] for this architecture.
...
For extra considerations, see [Choose a solution for integrating on-premises Active Directory with Azure][considerations].
...
- **Active Directory subnet:** The AD DS servers are hosted in a separate subnet. Network security group (NSG) rules protect the Azure AD DS servers and provide a firewall against traffic from unexpected sources.

- **Azure gateway:** The Azure gateway provides a connection between the on-premises network and the Azure virtual network. This can be a [VPN connection][azure-vpn-gateway] or [Azure ExpressRoute][azure-expressroute]. For more information, see [Connect an on-premises network to Azure using a VPN gateway](/azure/expressroute/expressroute-howto-coexist-resource-manager).
...
Also, consider designating one or more servers in each domain as [standby operations masters][standby-operations-masters] in case connectivity to a server acting as a flexible single master operation role fails.
