...
The [COPY statement](#) is then used to load data from the staging environment into teh Synapse Dedicated pool.

TODO: Mechanics of Copy activity
...
1. Create a set of external tables for the data. An external table is a table definition that points to data stored outside of the warehouse &mdash; in this case, the flat *check staging file*#TODO type files in blob storage. This step does not move any data into the warehouse.
