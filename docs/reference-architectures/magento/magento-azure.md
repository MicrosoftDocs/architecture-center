...
- Add [Azure Application Gateway](https://azure.microsoft.com/services/application-gateway/) ingress to support secure socket layer (SSL) termination.
...
- Adequately provision Azure Files or another NAS system. Magento can store thousands of media files such as product images. Be sure to provision the Azure Files or other NAS product with sufficient *input/output operations per second (IOPS)* capacity.
...
To avoid exposing potentially sensitive data like client IP addresses, and avoid performance issues from emitting access logs, limit the following logs.

- Use the following Varnish daemon command to limit Varnish logging to error-level:
...
- In this architecture, MySQL doesn't expose a public endpoint. If the build server stores configuration settings to the backend MySQL database, you need to deploy that server into the same virtual network subnet that MySQL connects to via service endpoint.
