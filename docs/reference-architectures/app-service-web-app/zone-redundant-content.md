...
* [Azure Cache for Redis][redis] provides a high-performance distrubuted cache for output, session and general-purpose caching.
* [Azure Service Bus][service-bus] acts as a high-speed bus between front-end and back-end services for asynchonous messaging.
...
* [Azure Monitor - Application Insights][insights] collects application telemetry for observability.

### Alternatives

* Either Azure Active Directory (Azure AD) or Azure AD B2C can be used as an IDP in this scenario. Azure AD is generally used for business-to-business (B2B) scenarios, while Azure AD B2C is designed for business-to-consumer (B2C) scenarios.
* You can choose Azure-managed DNS, which is recommended, or you can choose to use your own DNS provider.
* [Azure Application Gateway][appgw] could be used instead of Azure Front Door if the majority of internet users were located in the same geographical region as the Azure region (i.e. latency was not a problem), and caching of origin requests was not an important requirement. [Azure Content Delivery Network][cdn] (Azure CDN) could be used alongside Application Gateway to cache static assets. However Azure Front Door was chosen for its higher availability provided by a global network presence, and improved performance through WAN accelleration and built-in content cache. Operational excellence is also improved with a single configuration point for custom domain names, SSL certs, Web Application Firewall and routing rules.
* [Static website hosting in Azure Storage][storage-spa] may be considered in place of Azure Static Web Apps, if already using Azure CDN for example. However static website hosting in Azure Storage does have limitations. For more information, see [Static website hosting in Azure Storage][storage-spa]. Azure Static Web Apps was chosen for its simplicity and ease of configuration, as well as its global high availability.
* Azure Static Web Apps also has the capability to host Functions, either fully managed or bring your own. See [API support in Azure Static Web Apps with Azure Functions][swa-apis] for more information. In this architecture, Functions are hosted in a zone-redundant Premium Functions plan.

### Solution details

Customers want the convenience of websites and apps that are available when they need them. Keeping hosting platforms highly available at scale has been problematic in the past, requiring complex and expensive multi-region deployments. These problems are resolved with [Availability Zones][azs], physically separate locations within each Azure region that are tolerant to local failures. With zone-redundant deployments a customer can spread workloads across multiple independent zones, improving availablity. 
...
Azure Static Web Apps is a global service resilient to zone and region failures. Deploy a Standard plan for production apps. API support and Enterprise-grade edge is not required for this architecture as Premium Functions and Azure Front Door provide these capabilities. 
...
See [Integrate Azure Functions with an Azure virtual network][func-vnet] for more information.
...
See [Enable zone redundancy for Azure Cache for Redis][enable-redis-zr] for more information.
...
See [Azure Cache for Redis with Azure Private Link][redis-pep] for more information.
...
* [Create a private endpoint for Azure Cognitive Search][cog-search-pep] which will disable the public endpoint.
* Integrate the private endpoint with an Azure Private DNS zone.

### Key Vault

Key Vault is automatically zone-redundant in any region where Availability zones are available. The Key Vault used in this architecture is deployed with a private endpoint enabled and public disabled for backend services to access secrets. See [Integrate Key Vault with Azure Private Link][akv-pep] for more information.

### Azure DNS Private Zones

Integrate Private Endpoints with Azure DNS Private Zones to simplify DNS management. See [Azure Private Endpoint DNS configuration][pep-dns] for more information.
...
Global services can also fail although the likelihood of this is increasingly unlikely. However customers can improve recovery times from global service failure by having a runbook prepared and rehearsed for these events. Some examples:

* The risk of Front Door service downtime can be mitigated with a runbook that changes DNS CNAME records to point to an alternative reverse HTTP Proxy like [Azure App Gateway][appgw].
* [Build resilience in your identity and access management infrastructure][aad-resilience].
...
There is an inverse relationship between Availability and cost. However zone-redundant architectures are less expensive than multi-region alternatives because services can be deployed in a single region. However there are several cost implications that customers should be aware of:

* Some zone-redundant services incur additional costs for inter-zone bandwidth. See [Bandwidth pricing][bandwidth-pricing] for more information.
* Some services require a minimum number of instances or replicas to be deployed to achieve zone-redundancy. See the [Recommendations](#recommendations) section above for details.
* Zone redundant storage (ZRS) is priced differently to Locally redundant storage (LRS). See [Storage pricing][storage-pricing] for more information.
* Private Endpoints incur hourly and bandwidth (data) charges. See [Private Link pricing][pep-pricing] for more information.

Some cost optimization considerations include:

* Save money when you reserve resources in advance. Several services in this architecture are eligible for Reserved capacity pricing. See [Reservations][reservations] for more information.
* Function Apps can be hosted in the same dedicated App Service Plan as the API Apps. This removes the segmentation of frontend and backend services and introduces risk of noisy neighbor effect; backend services could consume resouces needed by frontend services, and vice-versa.
* Private endpoint costs can be removed by operating backend services in the public address space and relying on service firewalls and Identity as security perimeters fro these services.
...
* Azure AD / Azure AD B2C and Azure DNS are not deployed by this sample. 
* Custom domain names and SSL certs are not created and configured. Default AFD frontend DNS names are used instead.
