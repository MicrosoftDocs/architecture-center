---
title: Trusted data center extension and DMZ in Microsoft Azure
description: How to implement a trusted extension and DMZ of your data center in Azure
author: telmosampaio
ms.service: guidance
ms.topic: article
ms.date: 04/14/2017
...
cardTitle: Tursted data center extension with DMZ
---
# Trusted data cener extension and DMZ in Azure
[!INCLUDE [header](../../_includes/header.md)]

This article describes proven practices for extending your data center to Azure, by creating a trusted data center extension in the cloud. This is a first step into moving your data center to a complete virtual data center (VDC).
...
* **On-premises network**. Your on-premises datacenter.
* **Gateway**. A gateway that provides connectivity between the routers in the on-premises datacenter and the trusted datacenter extension (Azure VNet - Trusted Hub) in an Azure region.
* **Azure VNet - Trusted Hub**. An Azure VNet in the **Trusted datacenter extension subscription** hosting a DMZ composed of:
  * **Gateway subnet**. Special subnet used to host the VPN or ExpressRoute gateways used to connect to your on-premises datacenter.
  * **NVA frontend subnet**. Subnet used to host network adapters (NICs) used for traffic coming from your on-premises network to Azure passing through network virtual applinces (NVAs).
  * **NVA backend subnet**. Subnet used to host NICs used for traffic coming from Azure workloads to yoru on-premises datacenter.
  * **Management subnet**. Subnet used to host management virtual machines (VMs) such as jumpboxes, and NICs used to manage NVAs.
  * **AD DS subnet**. Optional subnet used to host VMs acting as AD DS domain controllers.
...
* **User defined routes (UDR)**. [User defined routes][udr-overview] define the flow of IP traffic within Azure VNets. In this case, a UDR is used to forward all traffic destined to Azure workloads from your on-premises datacenter to the NVAs acting as firewalls. UDRs are also used at the workload VNets to force traffic destined outside the workkload VNet to flow through the same NVAs.
* **Azure Key Vault**. Key vault used to host secrets used by VMs and NVAs in the Trusted Hub. A separate key vault is also used for each workload VNet to host secrets used for individual workloads.

* **Web tier, business tier, and data tier subnets**. Subnets hosting the VMs and services that implement an example 3-tier workload running in the cloud. See [Running Windows VMs for an N-tier architecture on Azure][ra-n-tier] for more information. These workloads are created in their own subscriptions, separate from the subscription used to host the Trsuted Hub VNet.

## Overall VDC architecture

As a first step in the journey to extend your on-premises datacenter to Azure, the trusted datacenter extension reference architecture uses the following constraints:

* **Single tenant** environment that extends your company's namespace into Azure.
* **Multiple subscriptions** to separate workloads from the trusted hub in each region. Subscriptions can also be used to provide isolation among workloads in development, test, and production.
* **IaaS only approach** as a starting point for extending your datacenter to Azure.
* **Use of DevTestLabs feature in Azure** to stand up workloads in locked down VNets that do not allow the use of Internet endpoints, and only provide SSL connectivity in and out of the VNet, and HTTP connectivity for VM extensions.
* **Use of RBAC features in Azure** to separate concerns in the trusted hib VNet, and workload VNets as follows.
  * **Central SecOps**. Role used to manage all security resources in the trusted hub VNet.
  * **Central DevOps**. Role used to manage DNS and AD DS resources in the trusted hub VNet. Also able to add rules to the existing NVAs to expose workloads to the on-premises datacenter.
  * **Workload SecOps**. Role used to manage DevTestLabs environments and connect them to the trusted hub.
  * **Workload DevOps**. Role used to manage the workloads in DevTestLabs.
###### COntent below needs to be changed for VDC ######
## Recommendations
...
[0]: ../images/vdc-diagram.png "Trusted datacenter extension with DMZ in Azure"