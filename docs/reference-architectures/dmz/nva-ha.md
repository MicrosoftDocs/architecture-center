...
> [!NOTE]
> It is also possible to solve the asymmetric routing issue by ensuring the NVAs are performing inbound source network address translation (SNAT). This would replace the source IP of the original requestor to become the one of the NVA through with that session is originating. This ensures that you can use multiple NVAs at a time, yet preserving the route symmetry.
