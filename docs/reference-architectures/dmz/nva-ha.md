...
> [!NOTE]
> It is also possible to solve the asymmetric routing issue by ensuring the NVAs are performing inbound source network address translation (SNAT). This would replace the original source IP of the requestor to one of IP addresses of the NVA used on the inbound flow. This ensures that you can use multiple NVAs at a time, yet preserving the route symmetry.
