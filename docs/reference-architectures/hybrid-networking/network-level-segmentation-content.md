...
4. [Azure Virtual Network Manager (AVNM)](/azure/virtual-network-manager): Azure Virtual network Manager (AVNM) is a network management service that enables central IT management team to manage virtual networks globally across subscriptions at scale. With AVNM, you can group multiple virtual networks and enforce predefined security admin rules which should applied to the selected virtual networks at once. Similar to NSG, security admin rules created through AVNM also worked as a layer 4 Firewall but security admin rules will be evaluated first before NSG.
...
## Additional Pattern: Multiple Virtual Networks across multiple network segments

This pattern explains how to manage and protect multiple virtual networks which also across multiple subscriptions as a centralized way. Security admin rules which can be created on AVNM can enforce baseline security rule in a consistent manner, regardless of whether the virtual networks are connected to each other or belong to different subscriptions. For example, disable SSH port 22, RDP port 3389 and other high-risk ports from the internet to protect from security breaches on all virtual networks within the organization. See [this article](/azure/virtual-network-manager/concept-security-admins) for more details about the security admin rules and how it works.

![Azure Virtual Network MAnager](./images/avnm.png)

While AVNM is a great solution for protecting an organization's entire network with a minimum set of network rules, NSGs/ASGs still need to be used to apply granular network rules to each virtual network in addition to AVNM, which vary from system to system based on their security requirements.
