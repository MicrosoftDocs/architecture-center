...
An efficient approach eliminates waste within your SAP deployment on Azure and
...
SAP Central Services and database management system.
...
We recommend taking advantage of the following virtual machine accelerators for
your SAP application and database layers:

- Enable [Accelerated
    Networking](https://docs.microsoft.com/azure/virtual-network/create-vm-accelerated-networking-cli)
    on your virtual machines to accelerate network performance.

- If you plan to run your SAP database on the memory-optimized M-series
    virtual machines, consider enabling the [Write
    Accelerator](https://docs.microsoft.com/azure/virtual-machines/linux/how-to-enable-write-accelerator)
    durable write cache on your database log volumes to improve write I/O
    latency. Write Accelerator is mandatory for productive SAP HANA workloads to
    ensure a low write latency (sub-milliseconds) to the /hana/log volume.

For the SAP database layer, use Premium Azure Managed Disks to benefit from
...
to enable single sign-on (SSO) to your SAP S/4HANA Fiori Launchpad. Azure AD
...
Consider using Azure AD to authenticate and authorize users. Azure AD is a fully
