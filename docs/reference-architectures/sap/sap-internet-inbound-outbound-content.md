...
An application-specific [perimeter network](/azure/cloud-adoption-framework/ready/azure-best-practices/perimeter-networks) contains the internet-facing applications, like saprouter, SAP cloud connector, SAP analytics cloud agent, or application gateway. In the architecture diagram, the perimeter network is named SAP DMZ spoke vnet. Due to dependencies on the SAP systems the deployment, configuration and management of these services is typically performed by the SAP team. Because of it, these SAP DMZ services are often not located in a central hub subscription and network where they'd need to be managed by the central IT team, often causing organizational challenges. Application Gateway always requires own designated subnet, which is best placed in the SAP DMZ vnet. The Application Gateway uses a public IP addresses for frontend and https listener.
