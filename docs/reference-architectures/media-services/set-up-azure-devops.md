...
1. From your Azure DevOps project, in the left navigation, select **Repos** > **Files**.
...
1. On your computer, navigate to the location of the cloned Gridwich repo and run the following commands:
...
1. In [terraform/variables.tf](https://github.com/mspnp/gridwich/infrastructure/infrastructure/terraform/variables.tf), update the the default application name from `gridwich` to `cl1grw` or your application name, to avoid name collisions with `gridwich`.
...
```
   ...
   - For Apple FairPlay `amsDrmFairPlayAskHex` and `amsDrmFairPlayPfxPassword`, use the values from the *FairPlay-out-base64.txt* certificate file that you create and upload. For instructions, see [Apple FairPlay settings](gridwich-content-protection-drm.md#apple-fairplay-settings).
     ...
     The Apple FairPlay values in the preceding example are from the fake FairPlay certificate file [FairPlay-out-base64.txt](https://github.com/mspnp/gridwich/src/Gridwich.SagaParticipans.Publication.MediaServicesV3/tests/FakeFairPlayCert/FairPlay-out-base64.txt). For development purposes, you can use this fake file by selecting **Library** > **+ Secure file**, and uploading the file.
   
   - The `amsDrmOpenIdConnectDiscoveryDocumentEndpoint` should be the OpenID Connect Discovery Document endpoint URL that exposes the public signature keys in Azure Media Services.
...
1. Select **+ Variable group** and add a new variable group named `gridwich-cicd-variables.sb`, with variable `eventEndpointExtSys = https://cl1grw-grw-wa-viewer-sb.azurewebsites.net/api/eventgrid`.
...
1. Select **Run** to run the pipeline.
   ...
   - On first run, you must select **Permit** to approve the pipeline using the FairPlay Secure File you uploaded.
     ...
     ![permit](media/approve-secure-file.png)
...
1. Under **Pipelines**, select the pipeline and then select **Edit**. In the editor, add the following variables to the pipeline. For each variable, select **Let users override this value when running this pipeline**.
   ...
   - `environment`,
   - `RUN_FLAG_SUBSCRIPTIONS_FAIL_GRACEFULLY` (default: `false`),
   - `RUN_FLAG_SUBSCRIPTIONS_DISABLED` (default: `false`) and
   - `RUN_FLAG_TERRAFORM` (default: `true`)
...
For instructions on how to run this pipeline and set up a new environment, see
...
1. Under **Pipelines**, select the pipeline and then select **Edit**. In the editor, add the following variables to the pipeline. For each variable, select **Let users override this value when running this pipeline**.
   ...
   - `environment`
   - `RUN_FLAG_SUBSCRIPTIONS_FAIL_GRACEFULLY` with default: `false`
   - `RUN_FLAG_SUBSCRIPTIONS_DISABLED` with default: `false`
   - `serviceConnectionName` with default: `gridwich-connection` or your service connection name
   - `applicationName` with default: `cl1grw` or your app name
...
A user with elevated privileges must execute pipeline-generated bash scripts whenever a new environment has been deployed. For more information, see 
