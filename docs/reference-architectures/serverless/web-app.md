...
Both definitions have in common the idea that developers and DevOps personnel don't need to deploy, configure, or manage servers. The backend developed in this reference architecture is an example of FaaS using Azure Functions, while serving web content from Azure Blob Storage could be an example of BaaS.

The term *serverless* traditionally meant BaaS or rather MBaaS (Mobile backend as a service) services, which use third-party solutions for generic functionality such as authentication, CDN, data management, email and social integration, etc. With the advent of services such as AWS Lambda and Azure Functions, it became possible for developers to run their own code in a serverless backend, called FaaS or sometimes as *Compute as a service*. Some important characteristics of FaaS are:
...
Use [JAMstack]((https://jamstack.org)) principles to develop and deploy fast, modern web applications, without requiring web server development and maintenance. In such applications, the client-side **J**avaScript implements request-response cycles over HTTPS, utilizing third-party or custom-built serverless **A**PIs to fetch information from the backend or the database, and populates the pre-built static **M**arkup files with this information. The React-based JAMstack web application in this reference architecture uses the Azure Functions APIs developed on the backend, to populate the Markup files built before deploying to the Azure CMS.

The following list describes the [best practices recommended by the JAMstack community](https://jamstack.org/best-practices/) to improve the build and deploy processes for a JAMstack application:

- A JAMstack application should use a global-ready Content Delivery Network (CDN) to provide a fast and consistent user experience over vast geographical areas. Instead of a centralized web server, the CDN edge servers serve content to the website. This reference application uses [Azure CDN](https://docs.microsoft.com/azure/cdn/cdn-overview), and the content and JavaScript files are made available to the CDN via an Azure Blob Storage account.
- Keep your project in Git for version control, and help in automating the CI/CD process.
- Use modern build tools and static site generators to efficiently handle the complex build process, and perform other functions such as pre-configuration, minification, etc. There are many modern build tools you can choose from, based on the application or website. The React-based reference application uses [Gatsby](https://www.gatsbyjs.org/).
- Automate the build process to run every time a change is made to your Git repo. This will eliminate user errors and delays typically introduced by a manual build. The ease of downloading the Git project with automated build process encourages collaboration within the larger enterprise, and improves the developer velocity. The reference application uses [Azure Pipelines](https://docs.microsoft.com/azure/devops/pipelines/get-started/what-is-azure-pipelines?view=azure-devops) to automate build and deploy processes. Every change in the repo triggers the build pipeline. A successful build starts the deploy pipeline, which compresses the built markup and pushes it to the Blob Storage.
- Choose a CDN with the ability to [purge its cache](https://docs.microsoft.com/en-us/azure/cdn/cdn-purge-endpoint) to ensure all users are served the freshest content. A cache purge is required if the build and deploy processes are not atomic, for example, if they replace old files with newly built ones in the same origin folder. A more sophisticated build and cache strategy may not require cache purge by the CDN, for example:
    - The reference application manages the CDN cache by versioning. Every new build is packaged into a new folder for that version, and uploaded as a whole unit to the Blob storage. The CDN edge servers point to this new folder only after the deployment is complete.
- The reference application also increases the cache TTL by fingerprinting the resource filenames. If their content changes, the build process automatically updates the their filenames, and appends a new fingerprint GUID. The resource file reference in the *index.html* also changes. Since the *index.html* is more likely to change, it is validated against the origin for every new user or browser refresh. The new *index.html* file invalidates the resource file cached in the CDN or browser cache, causing the CDN to update the cached file. You can maximize cache efficiency for heavier resource files by keeping them in the cache for as long as a year, or until they change. See the [Manage expiration of web content in Azure CDN](https://docs.microsoft.com/azure/cdn/cdn-manage-expiration-of-cloud-service-content) for more information.
