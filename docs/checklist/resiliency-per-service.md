...
**Use a dead-letter queue**. If a message cannot be processed or delivered to any receiver in spite of multiple retries, it is moved to a dead letter queue. Implement a process to read messages from the dead letter queue, inspect them, and remediate the problem. Depending on the scenario, you might retry the message as-is, make changes and retry, or discard the message. For more information, see [Dead-letter queues](/azure/service-bus-messaging/service-bus-dead-letter-queues) 
