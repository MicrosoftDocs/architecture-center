...
**Use Azure role-based access control (RBAC).** Assigning user accounts and access to resources shouldn't be a manual process. Use [Azure role-based access control (Azure RBAC)][rbac] to grant access that's based on [Microsoft Entra ID][azure-ad] identities and groups.
