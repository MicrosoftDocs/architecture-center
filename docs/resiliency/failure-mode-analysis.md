...
- **Enable zone redundancy.** By enabling zone redundancy, Azure SQL Database automatically replicates your writes across multiple Azure availability zones within supported regions. For more information, see [Zone-redundant availability](/azure/azure-sql/database/high-availability-sla?view=azuresql#zone-redundant-availability).

- **Enable geo-replication.** If you use a multi-region architecture, consider enabling SQL Database active geo-replication.
...
- If you use a multi-region architecture, create two Service Bus namespaces in different regions, and replicate the messages. You can use either active replication or passive replication.
