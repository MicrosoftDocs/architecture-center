...
| [Field extraction](/azure/ai-services/content-understanding/document/overview#field-extraction) | Extracts structured key-value pairs from documents based on a schema that you define. Extract fields directly from the source, classify them from a predefined set of categories, or generate them by using a generative model. When you opt in, the Analyze API returns each extracted field value with a confidence score and source grounding so you can automate downstream workflows, such as routing low-confidence fields to human review. |
