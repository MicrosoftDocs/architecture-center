...
There are two main topologies that can be used in Azure multi-VNet designs: [traditional hub and spoke][hubnspoke], and [Virtual WAN][vwan]. The main difference across both design options is that in Virtual WAN the hub VNet and everything inside is managed by Microsoft, while in a traditional hub and spoke environment the hub VNet is managed by the Azure customer. That being said, both Virtual WAN and hub and spoke topologies are examples architectures where the workloads run in spoke VNets and connectivity to on-premises is centralized in a hub VNet, and as a consequence, many of the concepts explained in this document will be applicable to both traditional hub and spoke and Virtual WAN designs.

There are two main ways of connecting spoke VNets to each other:

1. Directly to each other: VNet peerings or VPN tunnels can be created between the spoke VNets to provide direct connectivity without traversing the hub VNet.
2. Over the hub VNet: each spoke VNet has a peering to the hub VNet. Using routing configurations you can send traffic between spokes through a virtual network appliance in the hub.
...
- ExpressRoute: this is a scenario that is not only not recommended by Microsoft, but also not supported. The reason is because in certain situations packets between spokes in the same region or in different regions connected to the same ExpressRoute circuit might be routed down to the Microsoft Enterprise Edge router, which will route the packet back to the destination spoke. While basic connectivity might work, this setup presents multiple problems due to the fact that this traffic puts extra pressure on the ExpressRoute infrastructure (gateway and physical routers) which can cause packet drops.

In hub and spoke designs, the NVA will typically be placed in the hub, and VNet peerings between hub and spoke VNets need to be created either manually or automatically with AVNM:
