...
>[!NOTE]
>This article has not yet been updated to reflect the Azure networking support for resyncing peering. Azure networking supports adding and removing address space on a virtual network that is peered without the need to remove and restablish peerings, and instead simply performing a Sync operation after the network space has changed. The sync can be performed via [Sync-AzVirtualNetworkPeering](/powershell/module/az.network/sync-azvirtualnetworkpeering) PowerShell command or via the Azure Portal.
