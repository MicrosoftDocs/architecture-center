...
[Azure Route Server][ars] is a service which allows an NVA to interact with Azure SDN via Border Gateway Protocol (BGP). Not only the NVAs learn which IP prefixes exist in the Azure VNets, but they're be able to inject routes in the effective route tables of the virtual machines in Azure. 
