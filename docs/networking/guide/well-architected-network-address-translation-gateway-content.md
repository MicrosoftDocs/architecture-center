...
- Graceful retry patterns should be considered to avoid aggressive retries/bursts during transient failure or failure recovery. An antipattern called *atomic connections* is when you create a new TCP connection for every HTTP operation. Atomic connections prevent your application from scaling well and wasting resources. Always pipeline multiple operations into the same connection. Your application will benefit in transaction speed and resource costs. When your application uses transport layer encryption (for example, TLS), there's a significant cost associated with the processing of new connections. See [Azure Cloud Design Patterns](/azure/architecture/patterns) for more best-practice patterns.
