...
For an example of data exploration and preprocessing done inside Azure Machine Learning Studio (classic), see the [Preprocessing data](https://azure.microsoft.com/documentation/videos/preprocessing-data-in-azure-ml-studio/) video.
...
**Azure Machine Learning consumes well-formed tabular data**.  If the data is already in tabular form, data preprocessing can be performed directly with Azure Machine Learning Studio (classic) in the Machine Learning.  If data isn't in tabular form, say it is in XML, parsing may be required in order to convert the data to tabular form.
...
* **Text cleaning**: Remove embedded characters that may cause data misalignment. For example, they might be embedded tabs in a tab-separated data file or embedded new lines that may break records.

The  following sections detail some of these data processing steps.

## How to deal with missing values?

To deal with missing values, it's best to first identify the reason for the missing values to better handle the problem. Typical missing value handling methods are:
...
**Data exploration** offers an early view into the data. Many data issues can be uncovered during this step and  corresponding methods can be applied to address those issues.  It's important to ask questions such as what is the source of the issue and how the issue was introduced. This process also helps you decide on the data processing steps that need to be taken to resolve them. Identifying the final use cases and personas can also be used to prioritize the data processing effort.
