...
This design covers an illustrative customer, Consoto, a mid-large organization on the journey to Azure cloud, enabled by automation.

Consoto have established the Azure cloud foundation with an [Enterprise Landing Zone](/azure/cloud-adoption-framework/ready/landing-zone/#azure-landing-zone-conceptual-architecture) and are now looking to take their first data workloads to the cloud, guided by Microsoft’s [Well-Architected Framework](/azure/well-architected/). 
...
- The Solution is an analytical/reporting system with a primary for the Finance department and other corporate functions.
- The on-premises source system is:
  ...
  - The solution MUST minimise it's impact on the source system.
...
- Consoto have an enterprise data model of which this finance data is a subset of. The key data elements MUST be cleansed, modelled and conformed to the various reporting hierarchies before being served for reporting.  
- Source data ingested which isn’t currently mapped to the enterprise model MUST be made retained and made available for future analysis and use cases. 
...
- The Solution MUST support the forecast usage of:
   ...
   - Only Consoto employees can access the solution. This explicitly excludes any direct access by 3rd parties or externals. 
...
- The Solution SHOULD strongly prefer:
  ...
  - Higher cloud service tiers i.e. PaaS over IaaS, reducing toil, risk and below the line cost.
  - Use of components which are mature in the market, easy to find and upskill resources across the SDLC.
- The Solution SHOULD be optimised for the NFR’s (in order):
...
- The key changes for this solution:
  - Azure SQL has been selected for the data modelling capability due to expected data volumes, reduction in new components introduced and reuse of existing skills. 
...
![Diagram showing Medlallion architecture and data flow.](_images/ADF-ALZ-Medallion-Initial.png)
...
3.	Azure SQL Server is used to support the Enterprise data modelling requirements, including hierarchical conformance.
...
![Diagram showing Medlallion architecture Network design.](_images/ADF-ALZ-Medallion-Initial-Network.png)
...
Alternatives for the storage modelling layer:
...
- [Azure PostgresSQL](/azure/postgresql/): Discounted due to Consoto’s existing skillsets and desire to introduce as few new technologies as required, reducing cost and complexity. 
...
- Access to data across the solution will be controlled via a set of domain specific Entra ID groups, reflecting Consoto’s data classification framework. Individual solution components will then use these groups to apply data level controls. i.e. SQL Server [dynamic data masking](https://learn.microsoft.com/sql/relational-databases/security/dynamic-data-masking?view=sql-server-ver16), Power BI [RLS](https://learn.microsoft.com/power-bi/enterprise/service-admin-rls).
...
- For development:
  ...
  - Utilise testing frameworks like [PSRule](https://azure.github.io/PSRule.Rules.Azure/) to ensure deployments align with WAF guidance.
  - Utilise [Azure Policy](/azure/governance/policy/overview) to enforce organizational standards and to assess compliance at-scale, supported by [Azure Governance Visualizer](https://github.com/Azure/Azure-Governance-Visualizer) for configurable, granular insights on the technical implementation.
...
- Based upon the solution financial modelling having cost and budget alerting in place, particularly targeting the Databricks computer cluster, ADF SHIR’s and SQL server processing.
...
- Based upon the requirements, the standard service tiers of the various component SKUs will be acceptable, acknowledging these can be scaled-up, on-demand with no interruption in service levels.
  - This should be [rigorous tested](/azure/well-architected/performance-efficiency/performance-test) before production release.
