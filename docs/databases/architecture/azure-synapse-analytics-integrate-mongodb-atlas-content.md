...
- [Event Grid](/azure/event-grid/overview) is a highly scalable, serverless event broker that routes events to subscriber endpoints. In this architecture, Event Grid receives events from the Event Grid version of the App Service web app and triggers Azure Synapse Analytics pipelines via custom event-based triggers.

- [MongoDB Atlas](https://www.mongodb.com/atlas/database) is a fully managed, multicloud database as a service (DaaS) platform that supports transactional processing, relevance-based search, real-time analytics, and mobile-to-cloud data synchronization. MongoDB also provides MongoDB Enterprise Advanced, which is an on-premises solution. In this architecture, MongoDB Atlas serves as the operational data source that generates change events through change streams and transfers data to Azure Synapse Analytics to enable real-time insights.
