...
- [Azure DNS](/azure/dns/dns-overview) is a high-availability hosting service for Domain Name System (DNS) domains. Azure DNS management is similar to managing other Azure services, and it uses the same credentials, APIs, tools, and billing. In this architecture, Azure DNS ensures fast DNS resolution and quick updates for web apps and APIs.
- [Azure Front Door](/azure/well-architected/service-guides/azure-front-door) is a secure content delivery network and load balancer. In this architecture, it accelerates content delivery, provides instant failover, and protects apps from cyber threats.
- [App Service](/azure/well-architected/service-guides/app-service-web-apps) is a fully managed service for building, deploying, hosting, and scaling web apps. You can build apps by using .NET, .NET Core, Node.js, Java, Python, or PHP. Apps can run in containers or on Windows or Linux. In a mainframe migration, you can code the front-end screens or web interface as HTTP-based REST APIs. You can segragate them and make them stateless to orchestrate a microservices-based system. For more information about web APIs, see [RESTful web API design](../../best-practices/api-design.md). In this architecture, App Service hosts the web interface and REST APIs for the application.
...
- [Azure Storage](/azure/storage/common/storage-introduction) is a set of massively scalable and secure cloud services for data, apps, and workloads. It includes [Azure Files](/azure/well-architected/service-guides/azure-files), which serves as an effective tool to migrate mainframe workloads.
  ...
  - [Table Storage](/azure/storage/tables/table-storage-overview) is a NoSQL key-value store for rapid development that uses massive semi-structured datasets. The tables are schemaless and adapt readily as needs change. Access is fast and cost-effective for many types of applications, and typically costs less than other types of keyed storage. This architecture uses Table Storage for low-cost, long-term data retention.
