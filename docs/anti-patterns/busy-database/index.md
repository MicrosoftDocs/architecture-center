...
Examining the performance of the SQL Database using the database monitor in the
Azure Management console and capturing the CPU utilization together with the number of
DTUs provides a measure of how much processing the database
was performing. In the graph below, CPU and DTU utilization both quickly reached 100%
during the test (the test ran from 10:20 to 10:50.) <<RBC: Does the time the test ran matter, or should this just be that it ran for 30 minutes? Especially since the image doesn't show a time scale.>>
...
**Note:** This code is available in the `LessProcSql` controller in the [solution code][fullDemonstrationOfSolution] provided with this antipattern.
...
processing, such as calculating aggregate values (`SUM`, `MAX`, `MIN`, `AVG`,
...
server has to transfer far more data over the network (see the [Extraneous Fetching antipattern][ExtraneousFetching].)
...
the updated code (the test started just before 11:00 and ran for 30 minutes). <<RBC: Again does the hour matter, or is just saying 30 minutes sufficient?>> Note
...
## Related resources <<RBC: Is the Extraneous Fetching antipattern not a related resource? You already referenced MVVM in the text, so it's not because you haven't mentioned that.>>
