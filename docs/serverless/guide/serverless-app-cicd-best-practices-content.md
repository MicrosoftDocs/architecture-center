...
The sample is a React application, built using [Gatsby.js](https://www.gatsbyjs.com/docs/), which is a static site generator and front-end development framework. These tools can be run locally during development and testing phases, and then integrated with [Azure Pipelines](/azure/devops/pipelines/get-started/what-is-azure-pipelines) for the final deployment.

The sample uses the Gatsby file [gatsby-ssr.js](https://www.gatsbyjs.com/docs/reference/config-files/gatsby-ssr/) for rendering, and [gatsby-config.js](https://www.gatsbyjs.com/docs/reference/config-files/gatsby-config/) for site configuration. Gatsby converts all JavaScript files under the `pages` subdirectory of the `src` folder to HTML files. Additional components go in the `components` subdirectory. The sample also uses the [gatsby-plugin-typescript](https://www.gatsbyjs.com/plugins/gatsby-plugin-typescript/) plugin that allows using [TypeScript](https://www.typescriptlang.org/) for type safety, instead of JavaScript.

For more information about setting up a Gatsby project, see the official [Gatsby documentation](https://www.gatsbyjs.com/docs/how-to/#creating-pages-automatically).
