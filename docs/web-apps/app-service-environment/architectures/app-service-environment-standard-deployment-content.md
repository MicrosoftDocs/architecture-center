...
> [!NOTE]
> The `pickHostNameFromBackendAddress` setting overrides the HTTP `Host` header with the back-end address. This approach can cause issues with cookies, redirect URLs, and session affinity. For more information about the implications and alternative configurations, see [Preserve the original HTTP host name](../../../best-practices/host-name-preservation.yml).
