...
- [Azure Firewall](/azure/well-architected/service-guides/azure-firewall) is a cloud-native, stateful firewall service built into Azure. It provides high availability and unrestricted cloud scalability, and it supports both inbound and outbound filtering rules. In this architecture, Azure Firewall restricts outbound traffic from the web application. A route table is configured to route outgoing traffic that doesn't go through the private endpoint channels to the firewall. The firewall applies source network address translation (SNAT) to those flows, so traffic reaches the internet from the firewall's public IP addresses rather than the shared App Service Environment outbound pool, which gives downstream services a stable egress identity to allowlist. For simplicity, this architecture configures all private endpoints on the services subnet.
