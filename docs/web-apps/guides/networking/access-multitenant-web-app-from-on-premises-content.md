...
- [SQL Database](/azure/well-architected/service-guides/azure-sql-database) is a managed relational database service that supports various data types, including relational, spatial, JSON, and XML data. In this architecture, it serves as the back-end database that App Service accesses through a private endpoint. Firewall rules are configured to allow traffic only from *VNet Integration Subnet*.
