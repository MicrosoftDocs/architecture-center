...
- Configure a [Health Model](/azure/well-architected/design-guides/health-modeling) to capture resource state, drive operational decisions and alerting across your entire architecture.

  For this architecture, a health model should aggregate health signals from App Service and SQL Database into an overall system health state. For example, the model reports on App Service instance availability and response latency, SQL Database connectivity and query performance, and authentication success rates. Each component emits a health state (healthy, degraded, or unhealthy), and the model combines these states into a single top-level indicator.
