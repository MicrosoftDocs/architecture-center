...
Traffic from Application Gateway to the Spring Apps instance is re-encrypted to make sure only secure traffic reaches the application. The Spring Apps runtime receives that traffic and this is the TLS termination point. From here on, inter-service communication within the application microservices isn't encrypted. However, communication with other Azure PaaS services and the runtime is over TLS.
