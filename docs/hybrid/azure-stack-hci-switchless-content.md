...
### Scenario details and solution benefits
...
To help design and plan an Azure Stack HCI deployment correctly, it recommended to use the [Azure Stack HCI - Sizer Tool](https://aka.ms/hci-catalog#sizer), once open in a browser, select the option to create a "New Project" for sizing your HCI cluster(s). This requires an understanding of your workload requirements, in terms of number of workload VMs and their size, such as the amount of vCPUs, memory and storage capacity of the VMs. The Sizer Tool will ask questions in relation to your Resiliency requirements for the cluster, which will be a minimum of N+1 nodes, with the option of N+2 nodes should you desire the ability to withstand a node failure during an update, or other unplanned event that impacts two nodes simultaneously.

The output from the Azure Stack HCI Sizer Tool will be a list of recommended hardware solution SKUs that will be able to deliver the workload capacity and platform resiliency requirements that were entered into the Project as the input. If you wish to simply browse the full list of OEM hardware partner solutions available, see to the [Azure Stack HCI Solutions Catalog](https://aka.ms/hci-catalog#catalog)
