...
1. **There is a project or use case requirement to deploy a hybrid cloud solution in an on-premises or edge location** to provide local compute or data processing capabilities with an Azure consistent management and billing experience. Additional details are outlined in the [Potential use cases](#potential-use-cases) section of this article. The remaining steps assume Azure Stack HCI has been selected as infrastructure platform solution for the project.
...
1. **Review the Azure Stack HCI Sizer output for the recommended OEM hardware partner solution**, this will include details of the physical server make and model specifications, including  
