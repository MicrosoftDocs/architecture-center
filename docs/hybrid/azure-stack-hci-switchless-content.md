...
1. **Networking integration**: Although no switches are required for the node NICs used for the storage intent, single or dual (_recommended_) top of rack (ToR) switches are required for the outbound (north/south) compute and management intents. If using existing network switches, ensure the make and model is on the [approved list of network switches supported by Azure Stack HCI](/azure-stack/hci/concepts/physical-network-requirements#network-switches-for-azure-stack-hci).
...
- **The data backup and recovery process are tested** for each business system every six months, this provides assurance their business continuity and disaster recovery (BCDR) processes are valid and the business would be protected in the event of a datacenter disaster or cyber incident.
