...
1. **Network integration**: Although no switches are required for the node NICs used for the storage intent, the solution still requires either single or dual (_recommended_) top of rack (ToR) switches for the outbound (_north/south_) compute and management traffic. If you plan to use existing network switches, ensure the make and model is on the [approved list of network switches supported by Azure Stack HCI](/azure-stack/hci/concepts/physical-network-requirements#network-switches-for-azure-stack-hci).
