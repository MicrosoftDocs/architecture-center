...
To help design and plan an Azure Stack HCI deployment correctly, it is recommended to use the [Azure Stack HCI - Sizer Tool](https://aka.ms/hci-catalog#sizer) and create a "New Project" for sizing your HCI cluster(s).To use the Sizer requires that you understand your workload requirements, in terms of number and size of workload VMs that will run on the cluster, this includes number of vCPUs, and amount of Memory and Storage required for the VMs. The Sizer Tool also asks questions that relate to the Resiliency requirements for the cluster, such as reserving a minimum of "N+1 nodes" worth of capacity (_one node_) across the cluster. Or the option to reserve "N+2 nodes worth of capacity" across the cluster, if you desire the ability to withstand a node failure during an update, or a similar unplanned event that impacts two nodes simultaneously, yet allows for sufficient capacity to be available in the cluster for the workload to run on the remaining online nodes, which requires use of a three-way mirror for storage volumes.
