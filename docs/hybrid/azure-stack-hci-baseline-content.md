...
| Azure Stack HCI cluster outage | Power, network, hardware or software failure | Medium | To prevent a prolonged application outage caused by the failure of an Azure Stack HCI cluster, your workload should be architected using high-availability and disaster recovery principles, such as using workload data replication technology to maintain multiple copies of persistent state data using Arc VMs or AKS instances deployed on separate Azure Stack HCI clusters, and in separate physical locations. | Potential outage |
