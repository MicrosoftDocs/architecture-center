...
- Storage traffic:
  ...
  - The "SMB1" and "SMB2" storage ports connect to **two separate non-routable (_layer 2_) networks**, each has a specific VLAN configured on the switch ports of both ToR switches.
...
| Connectivity to Azure | Network outage | Medium | The cluster needs to reach the Azure control plane regularly for management and monitoring capabilities. If your cluster loses connectivity to Azure, it will operate in a degraded state, for example it would not be possible to deploy **new** Arc VMs or AKS clusters. Any existing workloads that are already running on the HCI cluster will continue to run, but you should restore the connection within 48 to 72 hours to ensure uninterrupted operation.| None |
