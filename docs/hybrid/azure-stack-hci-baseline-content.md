...
- External communication:
  ...
  - The [NetworkATC service](/azure-stack/hci/deploy/network-atc) is designed to ensure optimal networking configuration and traffic flow using network "_Intents_". Network intents define which physical network interface ports that are used for the different traffic types, such as the management, compute, and storage intents.
...
- Storage traffic:
  ...
  - Each node can access shared storage spaces, virtual disks, and volumes using SMB-Direct over these two dedicated storage network adapter ports which use SMB Multichannel for resiliency.
