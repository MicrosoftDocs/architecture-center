...
To estimate the cost of Azure services used in this architecture, use this [preconfigured estimate in the Azure pricing calculator](https://azure.com/e/bce97361d5004aa6bd8cdb3f7c0eb6a3). The estimate models a three-node Azure Local cluster running 10 multi-session hosts for approximately 50 to 100 medium-workload users. Adjust the values to match your workload profile. The estimate doesn't include Azure Virtual Desktop per-user access fees because those fees are typically covered by existing enterprise licenses. If your organization doesn't have an eligible license, add the per-user access fee to your cost model. The estimate also doesn't include on-premises hardware, physical networking equipment, or Windows Server licensing.
