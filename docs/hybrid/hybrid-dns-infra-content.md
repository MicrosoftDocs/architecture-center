...
- You can use Software Defined Wide Area Network (SD-WAN) technologies instead of VPN or ExpressRoute, but the fundamental architecture for DNS does not change.
...
1. The Azure Firewall forwards the DNS query to the Azure DNS Private Resolver inbound endpoint's IP address (`10.1.0.68`).
...
However, this approach also poses some inconvenience:
...
- Some Azure PaaS services that support multiregional access recommend having multiple regional private endpoints associated to the same domain, which in practice means that you would also need as many private DNS zones. Examples of these services include:
   ...
   - Azure Storage (geo-redundant): [Failover considerations for storage accounts with private endpoints](/azure/storage/common/storage-failover-private-endpoints).
...
In spite of these inconveniences, the overall recommendation is to use global private DNS zone whenever possible.
...
:::image type="complex" border="false" source="./images/hybrid-dns-infrastructure-multiregion-multizone.svg" alt-text="Diagram that shows hybrid DNS infrastructure with two on-premises locations, two Azure regions and one a private DNS zone per region." lightbox="./images/hybrid-dns-infrastructure-multiregion-multizone.svg":::
...
- If you need different DNS zones to return different records, for example as described in [Failover considerations for Azure Cosmos DB accounts with private endpoints](/azure/cosmos-db/failover-considerations-for-private-endpoints) and [Failover considerations for storage accounts with private endpoints](/azure/storage/common/storage-failover-private-endpoints), resolution from on-premises might not be consistent. The reason is that not all DNS servers behave in the same way when dealing with forwarding rules that have more than one target DNS server:
