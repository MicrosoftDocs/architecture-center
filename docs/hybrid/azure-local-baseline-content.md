...
- [Azure Policy][azure-policy] is a service that evaluates Azure and on-premises resources. Azure Policy evaluates resources through integration with Azure Arc by using the properties of those resources to business rules, known as *policy definitions*, to determine compliance or capabilities that you can use to apply VM guest configuration via policy settings. In this architecture, Azure Policy enforces compliance and configuration standards for resources managed by Azure Arc on Azure Local.
