...
SQL Database provides an extensive set of security features for data access, monitoring, and auditing, as well as encrypting data at rest. For details, see [Microsoft Defender for SQL Server Database Engine and Azure SQL Database](/sql/relational-databases/security/security-center-for-sql-server-database-engine-and-azure-sql-database?WT.mc_id=iotinsightssoln-docs-ercenk).
