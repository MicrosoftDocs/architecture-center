...
[![Lambda architecture: analytics clients layer highlighted.](./images/extract-insights-iot-data/lambda-analytics-clients.png)](./images/extract-insights-iot-data/lambda-4.png#lightbox)
...
As for the systems providing the source data, we need to be careful about not to overwhelm the system, and basically causing a denial of service (DoS) attack on the system, by querying it too frequently. If you poll the system, you should keep in mind that adjusting the polling frequency has two effects, the granularity of the data (the more often you query, the closer it gets to real-time) and the load created on the remote system.
