...
1. Create a **Signals Last Known Values** materialized view by using the aggregation function `arg_max()` on the timestamp to keep an up-to-date status of the vehicles.

1. Create a **Signals Downsampled** materialized view to aggregate signals by using the [summarize operator](/azure/data-explorer/kusto/query/summarize-operator) with time bins such as *hourly* and *daily* to simplify reporting across the fleet.
