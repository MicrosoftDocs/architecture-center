...
The reference architecture supports composability, innovation, and supportability by:
...
- Being based on generally available Microsoft Azure services. The architecture will evolve over time as those services introduce new features.

The architecture is composed of the following elements:

- **Vehicle Edge** is responsible for in-vehicle logic and connection to the cloud backend.
...
- **Business Data** encompasses the data model and links between fleet common data model and existing Dynamics 365 modules.
- **Analytics** integrates and generates insights from diverse & large data sources.
- **Business Operations** provides capabilities for the management and operation of vehicle fleets.
- **Business Automation** provides low-code/no-code extensibility to implement use cases based on the business data.
- **Visualization** provides reporting and business intelligence capabilities.
- **Operations and Security** provides monitoring and observability on all services and devices, secures network connectivity, and provides authentication/authorization to devices, applications, and users.

The following sections expand on architecture and workflow details for four of the key modules.
...
It's also possible to use [Azure IoT Hub](/azure/iot-hub) as an MQTT broker. It offers limited support for MQTT 3.1.1 and 5.0 with predefined topics and tight coupling between devices and cloud apps. For more information, see [Compare MQTT support in IoT Hub and Event Grid](/azure/iot/iot-mqtt-connect-to-iot-hub#compare-mqtt-support-in-iot-hub-and-event-grid).

The connection between devices and cloud can be configured over a private link for enhanced network security.
...
| ------------ |------------ |
| Vehicle status updates | The vehicle status update message is sent periodically during vehicle operations, usually between seconds and minutes. It contains the position and the operational data for the vehicle. |
| Vehicle alerts & notifications | Vehicle alerts & notifications is a specialized status update. This update is triggered by the edge device or calculated and generated in the telematics layer when specific conditions are reached. Common events include crash, geofence violation, harsh driving, and unauthorized movement. |
| Vehicle Health | The vehicle health contains information from the On-Board diagnostics system. It contains a list of installed hardware and Diagnostic Trouble Codes. This message type is sent with low frequency, usually a few times per day, on demand, or as part of a priority message if there's an imminent or actual breakdown. |
...
The following architecture diagram explains the dataflow for these messages:
...
Line of business integration is achieved by using a Power Platform data connector. The connector offers the possibility to create workflows in Power Automate or Azure Logic Apps, enabling Low-Code/No-Code integration for vehicle functions.
...
1. **Triggers**
    1. *Incoming event messages*: Start a workflow on Power Apps / Power Platform based on an event message type. The payload of the message can be parsed and accessed in Power Platform.
    1. *Life Cycle Management provisioning*: Notification of status changes on the provisioning status of vehicles.
1. **Actions**
...
1. Data is stored in the data lake, including:
    ...
    - Line of business information pushed from the Dataverse, via Azure Synapse Link
    - Exports pushed from Third Party System

1. Data is processed with Synapse pipelines in several steps:
    - Clean up, decode, and deduplicated data from raw *bronze* tables
...
1. Visualization through accessing the data from the Lake House. Power BI provides visualization capabilities to the lake house using Parquet Connectors, and Azure Data Explorer clusters by using DirectQuery.

### Components

The following components are referenced in this architecture:
...
- [Azure Event Grid](/azure/event-grid) is a highly scalable, fully managed Pub Sub message distribution service that uses the MQTT and HTTP protocols. This service enables telematic devices to communicate with the cloud.
- [Azure IoT Hub](/azure/iot-hub) is a managed service that acts as a central message hub between the telematics devices and the cloud.
- [Azure IoT Hub Device Provisioning Service](/azure/iot-dps/) is a helper service that enables zero-touch, just-in-time provisioning of the telematic devices.
- [Azure Event Hubs](/azure/event-hubs) is a scalable event processing service that ingests and processes large volumes of events and data. It processes the high-volume of events generated by the telematics devices.
...
- [Azure Cosmos DB](/azure/cosmos-db) is a fully managed NoSQL and relational database for modern app development. It's stores information about vehicles, devices, and users.

#### Integration services

The following services allow you to publish at scale, create and manage gateways, provide updated infrastructure and resources, create web and mobile apps, and provide geospatial capabilities.
...
- [Azure Maps](/azure/azure-maps) is a collection of geospatial services and SDKs to provide geographic context to web and mobile applications.

#### Data and analytics services

The following services allow you to query and analyze high folumes of data.
...
- [Azure Private Link](/azure/private-link) enables access to Azure PaaS services over a private endpoint in your virtual network. Private Links avoids exposing services to the internet.
- [Microsoft Entra ID](/azure/active-directory) is a cloud-based identity and access management service. It provides a common experience across all application, services and users.

#### Business integration

The following services allow you to manage data, apps, workflows, build low-code apps, increase insights and more.

- [Microsoft Dataverse](/power-apps/maker/data-platform) is a cloud scale database used to securely store data for business applications built on Power Apps.
- [Microsoft Power Automate](/power-automate) is a cloud-based service that allows users to automate repetitive tasks and streamline business processes with a low-code platform.
- [Microsoft Power Apps](/power-apps) is a cloud-based service that enables users to rapidly build and share low-code apps.
- [Microsoft Power BI](/power-bi) is a business analytics service for data visualization and insights.
- [Microsoft Dynamics 365](/dynamics365) is a set of intelligent business applications that helps you run your entire business and deliver greater results through predictive, AI-driven insights.
- [Microsoft Dynamics 365 Field Service](/dynamics365/field-service/overview) helps organizations deliver onsite service to customer locations.
...
*Download a [PowerPoint file](https://arch-center.azureedge.net/automotive-connected-fleets-diagrams.pptx) of the diagrams in the architecture.*

Independent Software Vendors (ISVs) can use the Connected Fleets reference architecture to build scenario-independent functionality that's critical to overall fleet management activities. Capabilities can be categorized as pertaining to the management of the vehicles or the business functions in a fleet. The Capabilities layer in the previous diagram depicts capabilities within each of these two categories.

- The organization of capabilities into categories is for descriptive convenience.
- An ISV might develop more than one capability in more than one capability category.
- Multiple ISVs offers different versions of the same capability.
...
- Extra design is required to process messages related to health and safety. For example, tying a crash signal to a 911 emergency call.
- The telematics hardware provider must guarantee functional safety for commands execution.

### Security

Security provides assurances against deliberate attacks and the abuse of your valuable data and systems. For more information, see [Overview of the security pillar](/azure/architecture/framework/security/overview).

- Use Microsoft Defender and Microsoft Sentinel to identify and address device vulnerabilities and threats. Consider integrating the lightweight security agent in your device. For more information, see [What is Microsoft Defender for IoT for device builders?](/azure/defender-for-iot/device-builders/overview).
...
- The cost of connected fleet operations is directly related to the volume of messages per vehicle.
  - Consider the required update frequency per vehicle. Consider dynamically adjusting the update speed based on use case.
  ...
  - Consider delayed processing of large files such as videos and log files by using Azure Spot instances.
  - Use topic aliases on frequent MQTT messages from the vehicles to save network bandwidth.
- The execution time for decoding and enrichment must be maintained as low as possible to reduce the size and scale of the function apps.
- Vehicle operations typically have periods of high and low demand during the day. Consider the use of autoscale for services that experience a demand to reduce costs.
...
- *Dead-lettering* messages in data lake for analytics allows you to monitor the system for problems and configure alerts to detect problems with vehicle communication.
...
- The current structure of the Automotive Common Data model requires multiple entries per vehicle status update. Each value requires updates in Device Measure and Device Meter. The information about the sensors should be surfaced from the fleet integration layer on demand.
- Spamming alert and notification messages creates problems in the Dataverse. The update frequency to the Dataverse must be configurable and subject to throttle.
...
You can follow the step-by-step tutorial for the [Microsoft Connected Fleet Reference Architecture](https://github.com/microsoft/connected-fleet-refarch) to deploy the solution in your subscription.
...
- [Autonomous Vehicle Operations (AVOps) design guide](/azure/architecture/guide/machine-learning/avops-design-guide) contains the approach for the development and model training of autonomous vehicle fleets.
- [Automated guided vehicles fleet control](/azure/architecture/example-scenario/iot/automated-guided-vehicles-fleet-control) shows an end-to-end approach to control automated guided vehicles (AGVs) for just-in-time manufacturing and automated show-floor logistics.

The following patterns are relevant when implementing this architecture:

- [Publisher-Subscriber](/azure/architecture/patterns/publisher-subscriber) describes how a device announces events to multiple interested applications.
- [Event sourcing](/azure/architecture/patterns/event-sourcing) describes the usage of an append-only store to record the full series of actions taken on entities such as vehicles, devices and users instead of just the last known values.
- [Throttling](/azure/architecture/patterns/throttling) is a pattern to control the consumption of resources to allow a system to continue to function and meet service level agreements.
