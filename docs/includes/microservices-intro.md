...
- **Data integrity**. With each microservice responsible for its own data persistence. As a result, data consistency can be a challenge especially during rollbacks. Microservices don't fully support ACID compliance but instead use BASE technique which stands for Basically Available, Soft State, Eventual Consistency. Embrace eventual consistency where possible.
