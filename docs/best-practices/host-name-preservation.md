...
:::image type="complex" source="images/host-name-preservation/platform-appservice.png" alt-text="Diagram that illustrates host-based routing in App Service.":::
    Diagram that shows how host-based routing works in Azure App Service. It shows a user’s browser making an HTTP request to a custom domain, like `contoso.com`. The DNS system resolves `contoso.com` to the public IP address of Azure App Service. The request is received by the App Service front end, which uses the host name in the HTTP request (contoso.com) to determine which web app instance should handle the request. The diagram highlights that the host name is preserved throughout the process, so that the App Service platform routes the request to the correct back-end web application based on the incoming host name. This ensures that the application receives the original domain name used by the client, which is important for scenarios like authentication, redirects, and cookie handling.
:::image-end:::
...
- Container Apps can [redirect HTTP requests to HTTPS](/azure/container-apps/ingress-how-to) and use the incoming host to generate the HTTPS URL.
...
:::image type="complex" source="images/host-name-preservation/issue-cookies.png" alt-text="Diagram that illustrates an incorrect cookie domain.":::
    Diagram shows the problem that occurs when a reverse proxy does not preserve the original HTTP host name, specifically in the context of cookie domains. The browser sends a request to `contoso.com`, which is received by the reverse proxy. The reverse proxy rewrites the host header to `contoso.azurewebsites.net` before forwarding the request to the back-end web application. The application generates a cookie with a domain attribute set to `contoso.azurewebsites.net`, based on the rewritten host name. The browser stores the cookie for `contoso.azurewebsites.net`, not for `contoso.com`. On subsequent requests to `contoso.com`, the browser does not send the cookie, because the domain does not match. As a result, the application does not receive the cookie it previously issued, which can cause loss of session state or break features like affinity routing. The diagram traces the flow of requests and cookie handling, highlighting where the host name changes and the resulting issues with cookie domains.:::image-end:::
