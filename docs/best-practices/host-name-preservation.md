...
In case you require end-to-end TLS/SSL (meaning that the connection between the reverse proxy and the backend service should use HTTPS), this does mean the backend service will also need a matching TLS certificate for the original host name. This adds operational complexity in deploying and renewing certificates, but most PaaS services offer free TLS certificates that are fully managed.
