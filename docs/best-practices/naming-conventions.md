...
|Tag |Associated Entity |512 (name), 256 (value) |Case insensitive |Alphanumeric, Special Characters except `<` `>` `%` `&` `\` `?` `/` See the limitations [here](https://docs.microsoft.com/en-us/azure/azure-resource-manager/resource-group-using-tags) |`"key" : "value"` |`"department" : "Central IT"` |
