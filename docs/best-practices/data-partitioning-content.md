...
1. Monitor the system to verify that data is distributed as expected and that the partitions can handle the load. Actual usage does not always match what an analysis predicts. If so, it might be possible to rebalance the partitions, or else redesign some parts of the system to gain the required balance.
...
In some cases, partitioning isn't considered important because the initial dataset is small and easily handled by a single server. While some workloads might not need partitioning, many commercial systems do. They need to expand as the number of users increases.
