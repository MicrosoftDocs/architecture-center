...
Caching is less useful for dynamic data, although there are some exceptions to this consideration. For more information, see [Cache highly dynamic data](cache-highly-dynamic-data). When the original data changes regularly, either the cached information becomes stale quickly or the overhead of synchronizing the cache with the original data store reduces the effectiveness of caching.
