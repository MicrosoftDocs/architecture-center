...
> [!NOTE]
> Currently there are no general-purpose standards that define how to model the HATEOAS principle. While [The Atom Publishing Protocol](https://tools.ietf.org/html/rfc5023) is an established standard for providing hypermedia controls, it is (as the name suggests) primarily designed for content publishing and thus best fit for [CRUD](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete)-style APIs. While more general-purpose alternatives such as [HAL](http://stateless.co/hal_specification.html), [Siren](https://github.com/kevinswiber/siren), [Verbose](https://verbose.readthedocs.io/en/latest/index.html), [Mason](https://github.com/JornWildt/Mason), [Hyper](http://hyperjson.io/) and [Hydra](https://www.hydra-cg.com/) exist, none are ratified standards yet. The examples shown in this section illustrate one possible, proprietary solution.
