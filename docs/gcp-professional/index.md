...
Google Cloud's `Project` is conceptually similar to the Azure subscription, in terms of billing and quotas/limits. However, from a functional perspective, it is closer to the Azure Resource Group - a logical unit where cloud resources are deployed to. 
Note that unlike in GCP, there is no maximum number of Azure subscriptions. Each Azure subscription is linked to a single Azure AD tenant (account in Google Cloud's terms), in turn the tenant can contain unlimited number of subscriptions, whereas GCP has a default limit of 30 projects per account. 
