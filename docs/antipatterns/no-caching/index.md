...
- Repeatedly fetching the same information from a resource that's expensive to use, in terms of I/O overhead or latency.
...
This approach is suitable for data that changes frequently. Here's the previous example, updated to use the [Cache-Aside][cache-aside-pattern] pattern.
...
Notice that the `GetAsync` method now calls the `CacheService` class, rather than calling the database directly. The `CacheService` class first tries to get the item from Azure Managed Redis. If the value isn't found in the cache, the `CacheService` invokes a lambda function that the caller passed to it. The lambda function is responsible for fetching the data from the database. This implementation decouples the repository from the particular caching solution and decouples the `CacheService` from the database.
...
- Besides caching data from an external data source, you can use caching to save the results of complex computations. Before you do that, instrument the application to determine whether the application is really CPU bound.
...
Data access statistics and other information provided by a data store can give useful information, like which queries are repeated most frequently. For example, in Microsoft SQL Server, the `sys.dm_exec_query_stats` management view has statistical information for recently executed queries. The text for each query is available in the `sys.dm_exec-query_plan` view. You can use a tool like SQL Server Management Studio to run the following SQL query and determine how frequently queries are performed.
