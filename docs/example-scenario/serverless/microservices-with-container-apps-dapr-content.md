...
- [Azure Cache for Redis](/azure/azure-cache-for-redis/managed-redis/managed-redis-overview) is a distributed, in-memory, scalable managed Redis cache. It's used as a Dapr state store component for the Makeline Service to store data on the orders that are being processed.
