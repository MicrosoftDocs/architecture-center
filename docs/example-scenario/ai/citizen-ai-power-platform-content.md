...
1. **Ingest:** Use [Azure Synapse pipelines](/azure/data-factory/concepts-pipelines-activities) to pull batch data from a wide variety of sources, both on-premises and in the cloud. Pipelines can be triggered based on a pre-defined schedule or in response to an event. They can also be invoked by calling REST APIs. You can use [Azure Event Hubs or Azure IoT Hub](/azure/iot-hub/iot-hub-compare-event-hubs) to ingest data streams generated by client applications or IoT devices. Event Hubs or IoT Hub ingests and stores streaming data, preserving the sequence of events received. Consumers can connect to hub endpoints to retrieve messages for processing.
1. **Store:** Ingested data can be landed directly in raw format and then transformed on the [Azure Data Lake](/azure/storage/blobs/data-lake-storage-introduction). Data once curated and transformed to relational structures can be presented for consumption in [Azure Synapse Analytics](https://azure.microsoft.com/services/synapse-analytics).
