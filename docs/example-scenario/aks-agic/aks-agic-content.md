...
An Azure Container Registry (ACR) is used to build, store, and manage container images and artifacts (such as Helm charts).
...
- Consider deploying the cluster configuration using [GitOps](https://docs.microsoft.com/en-us/azure/architecture/example-scenario/gitops-aks/gitops-blueprint-aks). Using GitOps provides uniformity between primary and DR clusters and a quick way to rebuild new cluster in case of cluster loss. 
...
* [Paolo Salvatori](https://it.linkedin.com/in/paolo-salvatori) | Principal Service Engineer
