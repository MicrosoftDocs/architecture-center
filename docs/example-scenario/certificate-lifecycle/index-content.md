...
To monitor the process and status of expired and expiring certificates, Log Analytics stores the data, and the workspace presents it in the form of tabular and graphical dashboard.
...
The solution also incorporates Log Analytics Workspace and Azure Workbook to enhance monitoring and visualization of certificate statuses stored in Key Vault. These components play a crucial role in maintaining visibility into certificate health:
  ...
  - **Azure Workbook:** Retrieves data from the Log Analytics Workspace and presents it in a user-friendly dashboard with visual representations, such as pie charts and detailed tables, categorizing certificates into "Not Expired" (green), "Expiring Soon" (yellow), and "Expired" (red).

Here is how certificate information are retrieved an presented in the workbook:
