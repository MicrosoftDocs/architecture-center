...
:::image type="complex" border="false" source="./media/split-brain-dns-host-public.svg" alt-text="Diagram of the public internet workflow." lightbox="./media/split-brain-dns-host-public.svg":::
   The diagram shows a six-step public internet data flow. In step 1, a double‑sided arrow connects app.contoso.com and the internet. In step 2, a double‑sided arrow connects the Azure DNS zone for contoso.com and the internet. In step 3, an arrow labeled 207.x.x (pip‑appgw) points from Azure Front Door to the Azure Application Gateway public IP endpoint. In step 4, a network security group (NSG) on the Application Gateway subnet allows inbound access from the AzureFrontDoor.Backend service tag. In step 5, an arrow points from the Application Gateway public IP endpoint to the app subnet. In step 6, the app subnet hosts the application resources.
:::image-end:::
...
1. External customers access the web application via Azure Front Door Standard or Premium, which functions as a global load balancer and a web application firewall (WAF).
   ...
   - The origin group is configured to point to the Azure Application Gateway instance via the Application Gateway public IP address.
...
1. The Application Gateway instance has a [listener](/azure/application-gateway/configuration-listeners) on port 443. Traffic is routed to the back end based on the hostname that's specified within the listener.
   ...
   - Azure generates a unique identifier for each Azure Front Door profile. The unique identifier is the *Azure Front Door ID* value located on the overview page of the Azure portal.
...
:::image type="complex" border="false" source="./media/split-brain-dns-host-private.svg" alt-text="Diagram of the private enterprise workflow." lightbox="./media/split-brain-dns-host-private.svg":::
   In step 1, a user labeled app.contoso.com initiates a request from an on-premises environment. In step 2, the request reaches the on-premises DNS server, which resolves the application name to the private IP address of Application Gateway. In step 3, dotted lines extend from the DNS server to the ExpressRoute circuit and the site‑to‑site VPN connection, which provide the path from on‑premises to Azure. In step 4, an NSG on the AppGW subnet controls the private traffic that reaches the Application Gateway. In step 5, two lines from the ExpressRoute circuit and the site-to-site VPN connection merge into a single arrow labeled AppGW subnet. This arrow points to the Application Gateway endpoint that shows the values 207.x.x.x (pip) and 192.168.200.4. In step 6, an arrow points from the Application Gateway endpoint to the app in the app subnet.
:::image-end:::
...
1. Application FQDNs are configured on the on-premises DNS provider. This DNS provider can be on-premises Active Directory Domain Services (AD DS) DNS servers or other partner solutions. The DNS entries for each of the application FQDNs are configured to point to the private IP address of the Application Gateway instance.
...
- [DNS](/azure/dns/dns-overview) is a service that resolves application FQDNs to the correct endpoints. For a public internet workflow, you must configure a public Azure DNS zone with the proper CNAME of the Azure Front Door endpoint FQDN. On the private (enterprise) side, configure the local DNS provider (AD DS DNS or a partner solution) to point each application FQDN to the private IP address of Application Gateway. In this architecture, DNS provides both public and private name resolution to support the split-brain DNS pattern.

- [Azure DNS Private Resolver](/azure/dns/dns-private-resolver-overview) is a fully managed service that enables DNS resolution between on-premises environments and Azure without deploying custom DNS servers. DNS Private Resolver enables resolution of on-premises customers, so enterprise users can use this split-brain DNS solution to access applications without traversing the public internet. In this architecture, DNS Private Resolver enables private name resolution for on-premises users.

- [Azure Front Door](/azure/well-architected/service-guides/azure-front-door) is a global load balancer and WAF that provides fast and secure web application delivery to global customers. In this architecture, Azure Front Door Standard or Premium tier routes external customers to the Application Gateway instance and provides caching and optimization options to enhance customer experience.
...
- [ExpressRoute](/azure/well-architected/service-guides/azure-expressroute) is a service that extends on-premises networks into the cloud through a private connection established by a connectivity provider. In this architecture, ExpressRoute facilitates private connectivity to Application Gateway for on-premises customers.
  
### Alternatives

As an alternative solution, you can remove Azure Front Door Standard or Premium and instead point the public Azure DNS record to the public IP address of Application Gateway. Based on this architecture's requirements, you must [cache and optimize](/azure/frontdoor/front-door-caching) at the entry point into Azure. As a result, you can't use the alternative solution for this scenario. For more information, see [Cost Optimization](#cost-optimization).
...
- Provides fast and reliable access over the internet to a web application for global nonenterprise customers.
...
- **Implement identity and access controls effectively.** Implement Microsoft Entra ID for robust identity management. Use Microsoft Entra Conditional Access policies to enforce strict access controls based on customer context, device health, and location.
  ...
  - **Assess your security measures.** Evaluate the effectiveness of the security measures for your dual-access workload by implementing:

    - **Evaluate your defensive investments regularly.** Regularly assess the effectiveness of Azure Front Door and Application Gateway. Ensure that they provide meaningful protection against threats.

    - **Restrict the impact scope of potential breaches.** Ensure that you contain security breaches within a limited scope. For example, effectively isolate external and internal traffic flows.

- **Assume that a breach is always possible.** Acknowledge that attackers can breach security controls. Prepare for such scenarios.

- **Implement security measures comprehensively.** Implement network segmentation, micro-segmentation, and NSGs. Assume that an attacker might gain access, and design compensating controls accordingly.
...
- [Firewall and Application Gateway for virtual networks](../../example-scenario/gateway/firewall-application-gateway.md#architecture-2)
