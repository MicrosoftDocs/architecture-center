...
:::image type="complex" border="false" source="./media/split-brain-dns-host-private.svg" alt-text="Diagram of the private enterprise workflow." lightbox="./media/split-brain-dns-host-private.svg":::
   In step 1, a user labeled app.contoso.com initiates a request from an on-premises environment. In step 2, the request reaches the on-premises AD DS DNS server. In step 3, dotted lines extend from the DNS server to the ExpressRoute circuit and the site‑to‑site VPN connection, which provide the path from on‑premises to Azure. In step 4, an NSG on the AppGW subnet controls the private traffic that reaches Application Gateway. In step 5, the two lines from the ExpressRoute circuit and the site-to-site VPN connection merge into a single arrow that points to the endpoint in the AppGW subnet. This endpoint is labeled 207.x.x.x (pip) and 192.168.200.4. In step 6, an arrow points from the Application Gateway endpoint to the app in the app subnet.
:::image-end:::
