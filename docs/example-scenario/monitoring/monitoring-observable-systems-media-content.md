...
2. **Ingestion.** This raw telemetry can come directly from end clients via HTTP calls. Alternatively, you can upload it via third-party systems to persistent storage and data lakes like Blob Storage. Fabric Eventstream provides a no-code experience for routing data to Fabric entities such as Eventhouse and Activator.
3. **Transformation and persistence.** Fabric enables transformation using table update policies and materialized views. Transformed data is stored in Eventhouse, which supports high-throughput analytics on large time-series datasets. This replaces the previous architecture of Azure Functions + Data Explorer pipeline with a more integrated and scalable Fabric-native solution.
4. **Monitoring.** The Real-Time Hub within Fabric centralizes access to streaming events and monitoring data. Users can visualize metrics, set alerts, and monitor performance across the Fabric tenant.
5. **Anomaly detection.** Fabric’s Real-Time Intelligence includes built-in anomaly detection via Activator, which can automatically trigger actions when patterns or thresholds are met. Eventhouse also supports advanced anomaly detection functions that account for seasonality and trends. 
...
- [Fabric Eventstream](/fabric/real-time-intelligence/event-streams/overview?tabs=enhancedcapabilities) is a feature that provides you with various source connectors to fetch event data from the various sources. The drag and drop experience gives you an intuitive and easy way to create your event data processing, transforming, and routing logic without writing any code. An end-to-end data flow diagram in an eventstream can provide you with a comprehensive understanding of the data flow and organization. The event processor editor is a no-code experience that allows you to drag and drop to design the event data processing logic.
