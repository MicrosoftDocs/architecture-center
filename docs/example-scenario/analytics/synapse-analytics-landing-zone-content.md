...
- Azure Synapse resources, the Azure integration runtime, and Spark pools that are located in the Managed workspace Virtual Network can connect to Azure Data Lake Storage, Azure Key Vault, and other Azure data stores with heightened security by using [Managed private endpoints](/azure/synapse-analytics/security/synapse-workspace-managed-private-endpoints). Azure Synapse SQL pools that are hosted outside the Managed workspace Virtual Network can connect to Azure services via private endpoint in the customer virtual network. 
