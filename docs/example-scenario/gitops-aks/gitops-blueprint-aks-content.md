GitOps is a set of principles for operating and managing a software system that uses source control as the single source of truth for the system. GitOps applies development practices like version control, collaboration, compliance, and continuous integration/continuous deployment (CI/CD) to infrastructure automation. GitOps can be applied to any software system.

GitOps is often used for Kubernetes cluster management and application delivery. This article describes some common options for using GitOps together with an Azure Kubernetes Services (AKS) cluster.

## Architecture

Two of the widely used GitOps operators are [Flux](https://fluxcd.io/) and [Argo CD](https://argo-cd.readthedocs.io/). Both are Cloud Native Computing Foundation ([CNCF](https://www.cncf.io/)) projects and can be used with AKS. The following scenarios show how they can be used.

### Scenario 1: GitOps with Flux and AKS

![Diagram of GitOps with Flux v2, GitHub and AKS](media/gitops-flux.png)

Flux is provided as a native [cluster extension](/azure/aks/cluster-extensions) to AKS. Cluster extensions provide a platform for solutions to be installed and managed on an AKS cluster. You can use the Azure portal, the Azure CLI, or IaC scripts (such as Terraform or Bicep) to enabled Flux as an extension to AKS. You can also use Azure Policy to apply Flux v2 configurations at scale on AKS clusters. For more information, see [Deploy applications consistently at scale using Flux v2 configurations and Azure Policy](/azure/azure-arc/kubernetes/use-azure-policy-flux-2).
...
1. Flux applies the desired state to the Kubernetes cluster.

#### Alternatives for scenario 1

- Flux can be used with other Git repositories such as Azure DevOps, GitLab, and BitBucket.
...
- Flux can also be configured against an Azure Blob Storage container as a source to produce artifacts. For more information, see [GitOps Flux v2 configurations with AKS and Azure Arc-enabled Kubernetes](/azure/azure-arc/kubernetes/conceptual-gitops-flux2).
...
This scenario is a pull-based DevOps pipeline for a typical web application. The pipeline uses GitHub Actions for build. For deployment, it uses Flux as the GitOps operator to pull/sync the app. The data flows through the scenario as follows:
...
1. GitHub Actions updates a Kubernetes manifest deployment file with the current image version that's based on the version number of the container image in the Azure Container Registry.
1. The Flux operator detects configuration drift in the Git repository and pulls configuration changes.
...
By default, the Argo CD user interface and the API server aren't exposed. To access them, we recommend that you [create an ingress controller with internal IP](/azure/aks/ingress-basic?tabs=azure-cli#create-an-ingress-controller-using-an-internal-ip-address). Or, you can [use an internal load balancer](/azure/aks/internal-lb) to expose them.

#### Alternatives for scenario 3

The configuration source repository can be any repository, including Azure DevOps, that's compatible with Git.
...
This scenario is a pull-based DevOps pipeline for a typical web application. The pipeline uses GitHub Actions for build. For deployment, it uses Argo CD as the GitOps operator to pull/sync the app. The data flows through the scenario as follows:
...
1. **Flux** is the GitOps operator and controller, and is the only component that can make changes to the cluster. Flux pulls cluster desired state changes from GitHub, and syncs them into AKS. Flux:
   ...
   - Applies the desired state to the Kubernetes cluster.
...
1. [Azure Kubernetes Service (AKS)](https://azure.microsoft.com/en-us/products/kubernetes-service) is a highly available, secure, and fully managed Kubernetes service in Azure. In AKS, Azure manages the Kubernetes API server. Cluster owners and operators access and manage the Kubernetes nodes and node pools.
...
1. [Syncier Security Tower](https://securitytower.syncier.com/) is a tool that Syncier developed and makes publicly available to help overcome GitOps security and compliance challenges. To help ensure that only trusted images run in the cluster, Syncier Security Tower comes with a set of best-practice policies that are grouped according to well-known security standards.
...
1. **Versioned and immutable**: Desired state is stored in a way that enforces immutability and versioning, and retains a complete version history.
...
Kubernetes describes everything from cluster state to application deployments declaratively with manifests. GitOps for Kubernetes places the cluster infrastructure desired state under version control. A component in the cluster, typically called an operator, continuously syncs the declarative state. Rather than having direct access to the cluster, most operations happen through code changes that can be reviewed and audited. This approach enhances security by supporting the principle of least privilege.

GitOps agents continuously monitor the system state and attempt to apply the desired state that's stored in your code repository. Some GitOps agents can revert operations that are performed outside of the cluster, such as manual creation of Kubernetes objects. [Admission Controllers](https://www.openpolicyagent.org/docs/latest/kubernetes-introduction/), for instance, enforce policies on objects during create, update, and delete operations. You can use them to ensure that the deployments can only change by by means of changing the deployment code in the source repository.

Policy management and enforcement tools can be combined with GitOps to enforce policies and provide feedback for proposed policy changes. You can configure notifications for various teams to update them on GitOps status. For example, you can notify when a deployment succeeds or when a reconciliation fails.

### GitOps as the single source of truth

GitOps provides consistency and standardization of the cluster state, and can help to enhance security. GitOps can also be used to ensure consistent state across multiple clusters. For example, GitOps can apply the same configuration across primary and DR clusters, or across a farm of clusters.
...
### Bootstrap initial configuration by using GitOps

There can be situations where you need want to deploy AKS clusters as part of your baseline configuration. An example is that you have to deploy a set of shared services or a configuration before you deploy workloads. These shared-services can configure AKS add-ons such as:
...
Because Flux can be enabled as an extension that's applied at the time of cluster creation, it can bootstrap the baseline configuration to the AKS cluster. The [Baseline architecture for an Azure Kubernetes Service (AKS) cluster](/azure/architecture/reference-architectures/containers/aks/baseline-aks) suggests using GitOps for bootstrapping. If you use the Flux extension, you can bootstrap clusters nearly at the time of deployment.

### Other GitOps tools and add-ons

The described scenarios can be extended to other GitOps tools. Jenkins X is another GitOps tool that provides instructions to [integrate to Azure](https://jenkins-x.io/v3/admin/platforms/azure/). You can use progressive delivery tools such as [Flagger](https://fluxcd.io/flagger/) for gradual shifting of production workloads that are deployed through GitOps.
...
GitOps shields the developer from the complexities of managing a container environment. Developers can continue to work with familiar tools such as Git to manage updates and new features. Hence GitOps enhances developer productivity.
...
- Azure DevOps offers a free tier that can be used for some scenarios. Use the [Azure pricing calculator](https://azure.microsoft.com/pricing/calculator) to estimate costs.

## Deploy a scenario

### Scenario 1

[Tutorial: Deploy applications using GitOps with Flux v2](/azure/azure-arc/kubernetes/tutorial-use-gitops-flux2?tabs=azure-cli)  provides steps for deploying applications to AKS by using GitOps with Flux v2.
...
### Scenario 2
 ...
 [Tutorial: Implement CI/CD with GitOps (Flux v2)](/azure/azure-arc/kubernetes/tutorial-gitops-flux2-ci-cd) provides steps for deploying applications to AKS by using GitOps with Flux v2, and for implementing CI/CD with GitOps and Flux v2 to AKS.
...
Follow the [Syncier - getting started guide](https://securitytower.syncier.com/docs/guide/get-started/) for Syncier Security Tower to deploy scenario 5 to AKS.
