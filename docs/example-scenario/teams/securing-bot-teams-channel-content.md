...
1.  Using the following CLI commands, deploy an Azure Firewall into the firewall subnet created in step 1.
...
1.  Run the following Azure CLI script to [deploy and configure the private endpoint](/azure/app-service/scripts/cli-deploy-privateendpoint). This step also implements VNet integration for the bot's App Service, connecting it to your virtual network's integration subnet.
...
1.  Next, you'll create a route table to ensure that traffic to and from each subnet goes through the firewall. You'll need the private IP address of the firewall you created in the previous step.
...
1.  At this point, you can confirm that your bot is only accessible from a channel in Teams, and that all traffic to and from the bot App Service goes through your firewall.
...
- [Bots Secured Behind a Firewall & Teams \| Bot Framework Blog](https://blog.botframework.com/2020/11/23/bots-secured-behind-a-firewall-teams)
