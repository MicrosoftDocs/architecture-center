...
- [Azure ExpressRoute](/azure/well-architected/service-guides/azure-expressroute) extends on-premises networks into the Azure cloud over a private connection that a connectivity provider facilitates. In this architecture, Azure ExpressRoute provides secure, high-performance connectivity between on-premises Enterprise Server users and Azure resources.

- [Azure Application Gateway](/azure/well-architected/service-guides/azure-application-gateway) is a scalable and highly available web front end that acts as a reverse-proxy service and provides a Layer-7 internet load balancer and Web Application Firewall (WAF). In this architecture, Azure Application Gateway provides secure web services front-end access and CICS API access from the web.

- [Azure Bastion](/azure/bastion/bastion-overview) provides secure and seamless RDP and SSH access to your VMs by using SSL, without exposing public IP addresses. In this architecture, Azure Bastion enables secure management access to Enterprise Server VMs without exposing them to the internet.

- [Azure Load Balancer](/azure/well-architected/service-guides/azure-load-balancer/reliability) distributes incoming network traffic across backend resources or servers according to configured load-balancing rules and health probes. In this architecture, Azure Load Balancer distributes traffic among the Enterprise Server instances in the Performance Availability Cluster.

- [Azure Virtual Machines](/azure/well-architected/service-guides/virtual-machines) offers on-demand, scalable computing resources in Azure. Azure Virtual Machines gives you the flexibility of virtualization without having to buy and maintain the physical hardware that runs it. In this architecture, Azure Virtual Machines host the Micro Focus Enterprise Server instances in a Performance Availability Cluster configuration.
...
- [Azure Cache for Redis](/azure/well-architected/service-guides/azure-cache-redis/reliability) improves performance and scalability for applications that use backend data stores heavily. Azure Cache for Redis keeps frequently accessed data, like session state and SOR, in server memory for fast access and throughput. In this architecture, Azure Cache for Redis serves as the Scale-Out Repository (SOR) data store for sharing synchronized user and system data across Enterprise Server instances in the Performance Availability Cluster.
...
- [Azure Site Recovery](/azure/site-recovery/site-recovery-overview) keeps applications and workloads running during outages by replicating VMs from a primary site to a secondary location. In this architecture, Azure Site Recovery replicates Enterprise Server VMs to the secondary region for disaster recovery, ensuring business continuity.
