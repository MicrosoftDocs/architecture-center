...
1. The MDI platform is used to send information that can be stored on files to Azure. Examples include System Management Facilities (SMF) data, virtual storage access method (VSAM) files, sequential files, and generation data groups (GDGs). MDI hardware that's installed in the datacenter includes Channel Gateway X (CGX) controllers and MDI servers.

1. MVT CloudTAPE provides tape archival and backup. MVT hardware that's installed in the datacenter includes CGX controllers and CloudTAPE servers.

1. MDI and MVT use FICON-based CGX controller devices to connect directly to the mainframe. No System z Integrated Information Processor (zIIP) specialty engines are needed for data transfer. There's no Luminex agent on the mainframe, and no TCP/IP ports need to be open for communication between the mainframe and Luminex devices.
...
- This solution uses ExpressRoute to transfer data from the datacenter to Azure. We recommend this approach, but you can also use the internet.
...
- If you already have ExpressRoute infrastructure in place, you can use it for this solution.
- Using Azure and Luminex for backup and recovery helps to eliminate some costs that are associated with physical tape infrastructure. Examples include media and shipping expenses and off-site storage for vaulting.
...
- If you have a high volume of data, you can cluster CGX controllers. Typically, one CGX device offers a data-transfer speed of up to 800 MB/s. CGX controllers are available with up to four fiber channel ports or 1 GbE, 10 GbE, or 25 GbE. These controllers also offer up to four ports for connectivity to attached storage systems.
...
- [Migrate mainframe data tier to Azure with mLogica LIBER*IRIS](./mainframe-data-replication-azure-data-platform.yml)
