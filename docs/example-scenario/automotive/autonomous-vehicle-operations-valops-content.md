...
- When dealing with large data sets, such as images or video files, or when working with smaller objects that require high I/O performance, it is not recommended to use Azure Files. Azure Files can slow down machine learning training or other workloads that require consistently low storage latency. It is recommended to use object storage with Azure Blob Storage or Azure Data Lake Storage for highest level of performance while maintaining cost efficiency.
