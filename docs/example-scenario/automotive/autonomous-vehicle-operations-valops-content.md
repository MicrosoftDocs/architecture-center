...
### Workflow
- A GitHub action triggers the metadata and orchestration services that run the deployment [campaign](https://github.com/eclipse-symphony/symphony/blob/e82ec70cb799dc4ac2979d41db2c8a2798345e02/docs/symphony-book/concepts/unified-object-model/campaign.md) within Azure Deployment Environments.
...
- [Azure Firewall](https://learn.microsoft.com/azure/firewall/overview) protects the network from traffic surges and attacks.
- [Azure HPC Cache](https://learn.microsoft.com/azure/hpc-cache/hpc-cache-overview) speeds access to your data for high-performance computing (HPC) tasks such as resimulation, simulation, or model training.
...
- [Azure Batch](https://azure.microsoft.com/products/batch) runs efficient large-scale parallel and high-performance computing (HPC) batch jobs in Azure. This solution uses Batch to run large-scale applications for tasks, such as resimulation jobs or closed-loop testing.
...
- [Azure Deployment Environments](https://learn.microsoft.com/azure/deployment-environments/overview-what-is-azure-deployment-environments) lets you quickly and easily spin up an infrastructure based on templates. Software Defined Vehicle (SDV) toolchain utilizes Azure Deployment Environments to spin up testing infrastructure consistently and securely.
...
- [Eclipse Orchestrator](https://projects.eclipse.org/projects/iot.symphony) enables end-to-end orchestration and creates a consistent workflow across different systems and toolchains. The SDV toolchain utilizes Eclipse Symphony as the main orchestrator workflow.
- [Microsoft Fabric](https://learn.microsoft.com/fabric/get-started/microsoft-fabric-overview) is an all-in-one analytics solution that incorporates real-time analytics and business intelligence. With Microsoft Fabric Validation engineers can quickly generate reports, analysis and business reports on validation operations for multiple projects, variants, and products.
...
- [Azure virtual networks](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-networks-overview) provides a private network for your Azure components to communicate with each other.
...
:::image type="content" source="./images/example-resim-results-visualized.png" alt-text="Architecture diagram that shows resim results ingested into Microsoft Fabric." border="false" lightbox="./images/example-resim-results-ingestion.png":::
...
- [Reserved instances](https://azure.microsoft.com/pricing/reserved-vm-instances/?msockid=3338580647b1602718c8499943b1663a) – Keep for 3 to 5 years reservation mainly for batch type and long running jobs such as simulation and open/closed loop testing
- [Spot instances](https://learn.microsoft.com/azure/virtual-machines/spot-vms) – Spot instances could be useful for jobs that don't have a strict SLA such as for dev/tests jobs. A great example that an organization could think about is where Researchers need to validate an experimental model against set of scenarios and no time-sensitivy or strict SLA is associated with the workload.
...
- Utilize CI/CD pipelines for building and releasing automotive software stacks
...
Consider an infrastructure you enable through [Azure Arc](https://learn.microsoft.com/azure/azure-arc/overview) to simplify governance and management across Azure cloud and on-premises environments, HIL testing, and validation farms.
...
- Refer to scalability and performance targets for [Azure Storage](https://learn.microsoft.com/en-us/azure/storage/common/scalability-targets-standard-account)
Based on the simulation requirements, [Azure Batch](https://learn.microsoft.com/azure/batch/) can provision permanently required containers or virtual machines based on the SLA. 
...
- Use of [Private or Service Endpoints](https://learn.microsoft.com/azure/private-link/private-endpoint-overview) to avoid public network and can significantly enhance the security and compliance of your Azure resources while simplifying your network management.
