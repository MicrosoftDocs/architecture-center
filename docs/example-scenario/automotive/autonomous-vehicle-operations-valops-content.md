...
Examples of open loop testing are:
...
- *Automate your deployment and maintain consistency* with infrastructure as code (IaC). You can use Bicep, Resource Manager templates (ARM templates), Terraform, or another approach.
...
-  To avoid cross-region latencey, ensure that the storage location used for ValOps data is in the same region as compute.  
- When dealing with large data sets, such as images or video files, or when working with smaller objects that require high I/O performance, it is not recommended to use Azure Files. Azure Files can slow down machine learning training or other workloads that require consistently low storage latency. Instead, using Azure Blob Storage is more efficient for these types of data, as it provides better performance and scalability.
- Performance for storage is essential in an HPC application such as ValOps. It's recommended that you use [Premium Azure Blob storage](https://learn.microsoft.com/azure/storage/blobs/storage-blob-block-blob-premium), except in cases where there are many small files (KB magnitude) that can't be processed into "fewer and larger" blobs. For small files, [Standard Azure Blob](https://learn.microsoft.com/azure/storage/common/storage-account-overview) can suffice. For more information, see [Blob storage performance and scalability checklist](https://learn.microsoft.com/azure/storage/blobs/storage-performance-checklist).
