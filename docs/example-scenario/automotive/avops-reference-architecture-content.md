...
The AVOps Reference Architecture consists of four main building blocks (which are described in detail in the upcoming chapters):
...
1. Measurement data is recorded by loggers in the vehicle which can include different data streams for different sensors (camera, radar, ultrasound, lidar..) and vehicle telemetry, stored on a data box in the vehicle and then uploaded to the landing data lake. This could be via [Azure Data Box](https://learn.microsoft.com/azure/databox/), [Azure Stack Edge](https://learn.microsoft.com/azure/databox-online/) or through a dedicated connection such as [Azure ExpressRoute](https://learn.microsoft.com/azure/expressroute/).  
    ...
    2. Ingested measurements are validated and data quality checks (e. g. checksum) are performed to remote low quality data.
...
1. At this stage accuracy and reliability of the data is improved by adding additional information, insights, and context to the data collected.
1. Extracted measurement data is labeled and stored in a separate data lake account. The labeling is done either by people through [Azure Data Share](https://learn.microsoft.com/azure/data-share/)  and, or through automated labeling by 3rd party partners```
1. Labeled data sets are provided to further [MLOps](#mlops) processes, mainly to create a perception and sensor fusion model used by autonomous vehicles to detect scenes (i.e. lane change, blocked roads, pedestrian, traffic lights, and traffic signs)
...
### MLOps
MLOps is utilized in several places of the AVOps reference architecture where machine learning comes into the picture:
...
- Data sets sourced from test fleet vehicles that collect raw sensor data (E.g., camera, lidar, radar, ultrasonic)
...
* [Azure Data Lake Gen2](https://learn.microsoft.com/azure/storage/blobs/data-lake-storage-introduction) used to data per stages (e.g. raw, extracted)
...
* [Azure Batch](https://learn.microsoft.com/azure/batch/) runs large-scale applications batch inference for open-loop validation within a re-sim framework
...
1. Utilize auto-scaling: Auto-scaling enables organizations to automatically adjust their cloud infrastructure based on demand, reducing the need for manual intervention and helping to keep costs in check.
