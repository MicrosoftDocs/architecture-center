In recent years, many healthcare companies have moved to the cloud, opting to provide online services via Telehealth solutions. This change has led to an increase in the amount of healthcare-centric audio data available to companies providing those services. While manual analysis of this data can yield useful insights, like treatments prescribed, the scale of this data makes this a time-consuming task. However, it's possible to automate the analysis of sensitive healthcare data using Azure-based tools. Specifically, the solution described in this article enables the automated transcription of audio data, healthcare-specific analysis of that data (including medical terminology linkage), and serving of that data to end-users.
...
7. The Synapse Pipeline also calls the Azure OpenAI API (via an Azure Function) to use GPT to generate a human readable summary of the call content. Depending on the type of data & analysis needed, content extracted by the Text Analytics for Health service (Step 6) can also be passed to the Open AI service to include additional relevant medical information in the Open AI context to improve the summary. Lastly, if the call content needs to be used for machine learning (ML), we can use GPT to extract a ML-friendly representation of the data using the <i>embeddings</i> API at this point.
...
* **[Azure Text Analytics for Health](https://learn.microsoft.com/azure/cognitive-services/language-service/text-analytics-for-health/overview?tabs=ner)**: Text Analytics for Health is the Azure-native healthcare understanding service. Using Text Analytics for Health users can extract medical entities, medical entity metadata like SNOWMED code, and medical entity relationships from complex, medical natural language, like medical notes.
...
* **[Azure Synapse Analytics](https://learn.microsoft.com/azure/synapse-analytics/overview-what-is)**: Azure Synapse Analytics is a full feature enterprise analytics platform providing data ingestion, orchestration, processing, and serving capabilities at scale.
...
In this scenario, based on a real customer use case, users first upload their audio data to an Azure Storage account for analysis. An automated trigger kicks off an Azure Function to transcribe the data in the Azure Speech Service, using keys stored securely in Azure Key Vault. Once the data has been transcribed, an Azure Synapse Analytics Pipeline-based analysis pipeline uses Text Analytics for Health and Azure Open AI to extract key health information like diagnosis and patient medications, and summarize the overall content. Finally, this data is aggregated and transformed as needed for end-user consumption via PowerBI dashboards using Azure Synapse Analytics.

The above scenario makes several assumptions. As this is highly sensitive data, we assume all storage accounts and services have been securely deployed following widely available best practices for working with sensitive data in the cloud (for example, all data at rest is encrypted and account keys are securely stored). We also assume appropriate information security personnel have been consulted for enterprise security best practices. 
