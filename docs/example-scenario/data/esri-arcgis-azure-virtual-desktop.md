...
There are many ways to scale this reference architecture. They include scaling in or out and up and down the VMs for the backend or the desktops (both CPU and GPUs). Also, Azure Virtual Desktop can be deployed on individual VMs or in multi-session VMs. Azure Virtual Desktop can scale from hundreds to thousands of VMs. See [here](/mem/intune/fundamentals/azure-virtual-desktop-multi-session).
