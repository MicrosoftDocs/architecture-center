...
- [Azure IoT Hub](https://azure.microsoft.com/services/iot-hub) is in the ingestion layer. IoT Hub also support bi-directional communication back to devices, allowing Actions to be sent from the cloud or [Azure IoT Edge](https://azure.microsoft.com/en-us/services/iot-edge/) to the device.
...
- In the stream ingestion layer, we could use [Event Hubs](https://azure.microsoft.com/services/event-hubs) or [Apache Kafka on HDInsight](/azure/hdinsight/kafka/apache-kafka-introduction) to ingest data. **It is important to note these services only offer uni-directional communication, no device management, no edge capabilities and are recommended for non-IoT scenarios such as streaming data pipelines.** Refer to [Connecting IoT Devices to Azure: IoT Hub and Event Hubs](https://docs.microsoft.com/en-us/azure/iot-hub/iot-hub-compare-event-hubs) for a more detailed comparison.
- In the transformation and analytics layer, we could use [HDInsight Storm](/azure/hdinsight/storm/apache-storm-overview), [HDInsight Spark](/azure/hdinsight/spark/apache-spark-overview) or [Azure Stream Analytics](https://azure.microsoft.com/services/stream-analytics) to perform streaming analytics, and [Azure Functions](https://azure.microsoft.com/en-us/services/functions/) to perform payload transformations.
- In the service storage layer, we could use [Analysis Services](/azure/analysis-services) to provide a semantic model for your data. It can also increase system performance when analyzing your data. You can build the model based on Azure DW data. [Azure Data Explorer](https://azure.microsoft.com/en-us/services/data-explorer/) and [Time Series Insights](https://azure.microsoft.com/en-us/services/time-series-insights/) could be used for storing and analyzing time series data.
...
- [Azure IoT Hub](https://azure.microsoft.com/en-us/pricing/details/iot-hub/) is a fully managed cloud service that makes it easy, fast, and cost-effective to ingest massive amounts of IoT data
...
- Review the [Azure IoT reference architecture](https://docs.microsoft.com/en-us/azure/architecture/reference-architectures/iot/) that shows a recommended architecture for IoT applications on Azure using PaaS (platform-as-a-service) components.
