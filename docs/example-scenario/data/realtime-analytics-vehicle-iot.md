...
- In the stream ingestion layer, we could use [Event Hubs](https://azure.microsoft.com/services/event-hubs) or [Apache Kafka on HDInsight](/azure/hdinsight/kafka/apache-kafka-introduction) to ingest data. **It is important to note these services only offer uni-directional communication, no device management, no edge capabilities and are recommended for non-IoT scenarios such as streaming data pipelines.** Refer to [Connecting IoT Devices to Azure: IoT Hub and Event Hubs](/azure/iot-hub/iot-hub-compare-event-hubs) for a more detailed comparison.
