...
:::image type="complex" source="media/virtual-desktop-gis-azure-network.png" alt-text="Diagram that shows an architecture for deploying ArcGIS components on Azure." lightbox="media/virtual-desktop-gis-azure-network.png":::
   Diagram that shows a single-region architecture deployed within one Azure subscription and one virtual network. The virtual network contains four subnets, each corresponding to a resource group: storage, GIS desktop, GIS back end, and Azure Virtual Desktop. On the left, GIS analysts, administrators, and editors connect via RDP to the Azure Virtual Desktop control plane, which handles web access, gateway, broker, and diagnostics. The control plane directs users to session host VMs in the GIS desktop subnet, where ArcGIS Pro runs on GPU-enabled VMs. Web GIS users connect through Azure Application Gateway, which routes traffic to ArcGIS Server roles in the GIS back-end subnet. The GIS back-end subnet contains ArcGIS Enterprise components, including Portal for ArcGIS and ArcGIS Server. Azure SQL Managed Instance hosts the enterprise geodatabase, which ArcGIS Pro users access from their RDP sessions. The storage subnet contains Azure NetApp Files, which stores ArcGIS Server configuration data, raster images, lidar datasets, and Portal for ArcGIS items. FSLogix profile containers are also stored in Azure NetApp Files or Azure Files to provide persistent user profiles. Microsoft Entra ID and Active Directory Domain Services provide identity and authentication across the environment. Azure Monitor and Azure Policy provide observability and governance for the deployed resources.
:::image-end:::
...
- GIS analysts, administrators, and editors connect to Azure Virtual Desktop via a Remote Desktop Protocol (RDP) session. From there, ArcGIS Pro is accessed and takes advantage of the GPUs for light, medium, and heavy workflows. *Light* refers to a 2D workflow, *medium* refers to a more demanding 2D workflow, and *heavy* refers to a 2D or 3D workflow that requires GPUs. GIS administrators can also use ArcGIS Pro to publish services and administer the enterprise geodatabase. Finally, GIS editors can maintain the vector and raster layers.
  ...
  Administrators can also use semantic versioning to make it possible to publish new versions of ArcGIS Pro. For example, as new versions of ArcGIS Pro are available, like ArcGIS Pro 3.0, the new version can be published in the Remote Desktop tool. This means users can pick that new version when they're ready to upgrade without having to perform the upgrade themselves. The GPU drivers can be included in the creation of the images that the deployments are based on, or use the Azure VM extension to install the GPU driver.

  :::image type="content" border="false" source="media/arcgis-rdp.jpg" alt-text="Screenshot that shows ArcGIS and VMs in Remote Desktop.":::

- Web GIS users can also use this solution by accessing ArcGIS Enterprise administrative interfaces either in the browser in the Azure Virtual Desktop RDP session or via their local browser, if ArcGIS is published as public facing. The Azure application gateway routes the traffic to the correct endpoint for the ArcGIS server roles. As with ArcGIS Pro, the latency between the browsers and the back end are minimized.
...
Although GIS has been implemented in Azure for many years, it has typically included only the back-end components. That implementation introduces latency between the client and server components. Organizations are able to deploy desktop GIS on Azure VMs from the [Microsoft Marketplace](https://marketplace.microsoft.com/marketplace/apps?search=ArcGIS), but that deployment is for thick clients and isn't very scalable. This architecture addresses both challenges.
