...
:::image type="complex" source="media/virtual-desktop-gis-azure-network.svg" alt-text="Diagram that shows an architecture for deploying ArcGIS components on Azure." border="false" lightbox="media/virtual-desktop-gis-azure-network.svg":::
   Diagram that shows a single-region architecture deployed within one Azure subscription and one virtual network. The virtual network contains four subnets, each corresponding to a resource group: storage, geographic information system (GIS) desktop, GIS back end, and Azure Virtual Desktop. On the left, GIS analysts, administrators, and editors connect via Remote Desktop Protocol (RDP) to the Azure Virtual Desktop control plane, which handles web access, gateway, broker, and diagnostics. The control plane directs users to session host virtual machines (VMs) in the GIS desktop subnet, where ArcGIS Pro runs on GPU-enabled VMs. Web GIS users connect through Azure Application Gateway, which routes traffic to ArcGIS Server roles in the GIS back-end subnet. The GIS back-end subnet contains ArcGIS Enterprise components, including Portal for ArcGIS and ArcGIS Server. Azure SQL Managed Instance hosts the enterprise geodatabase, which ArcGIS Pro users access from their RDP sessions. The storage subnet contains Azure NetApp Files, which stores ArcGIS Server configuration data, raster images, lidar datasets, and Portal for ArcGIS items. FSLogix profile containers are also stored in Azure NetApp Files or Azure Files to provide persistent user profiles. Microsoft Entra ID and Microsoft Entra Domain Services provide identity and authentication across the environment. Azure Monitor and Azure Policy provide observability and governance for the deployed resources.
:::image-end:::
...
- [Azure NetApp Files](/azure/well-architected/service-guides/azure-netapp-files) is an enterprise-class, high-performance, metered file network-attached storage service. In this architecture, Azure NetApp Files stores ArcGIS Server configuration data, raster images, lidar datasets, and other geospatial files.
...
- [Domain Services](/windows-server/identity/ad-ds/get-started/virtual-dc/active-directory-domain-services-overview) is a directory service that provides traditional domain-based identity services like group policies and Kerberos authentication. Domain Services stores directory data and makes that data available to network users and administrators. Domain Services stores information about user accounts, like names, passwords, and phone numbers, and enables other authorized users on the same network to access that information.
