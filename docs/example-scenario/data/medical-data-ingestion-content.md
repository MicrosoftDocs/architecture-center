...
- [Azure Data Explorer](/products/data-explorer/#overview) is largely a managed-data analytics service that performs real-time analysis of large data volumes. In this use case, Azure Data Explorer stores and queries large datasets that require a high data retrieval speed and a large index set.
- [Azure Files](/azure/storage/files/storage-files-introduction) provides fully managed file shares in the cloud. Medical image files that are to be ingested go in the file share. Use AzCopy with [Azure Automation runbook](/azure/automation/overview) to handle large volumes of files, up to 30 terabytes (TB), for better performance.
