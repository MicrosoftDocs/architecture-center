...
- [Azure Data Factory](/azure/data-factory/introduction) is a cloud-based data integration service. In this architecture, it orchestrates the transformation of staged data into a common structure in Azure Synapse Analytics. Azure Data Factory [uses PolyBase when it loads data into Azure Synapse Analytics](/azure/data-factory/connector-azure-sql-data-warehouse#use-polybase-to-load-data-into-azure-synapse-analytics) to maximize throughput.
...
- If you have large-scale datasets, consider using [Data Lake Storage](/azure/storage/blobs/data-lake-storage-introduction), which provides limitless storage for analytics data.
...
- The [massively parallel processing architecture](/azure/synapse-analytics/sql-data-warehouse/massively-parallel-processing-mpp-architecture) of Azure Synapse provides scalability and high performance.
- Azure Synapse has [SLAs](https://www.microsoft.com/licensing/docs/view/Service-Level-Agreements-SLA-for-Online-Services) and [recommended practices for achieving high availability](/azure/synapse-analytics/sql/best-practices-dedicated-sql-pool).
- When analysis activity is low, the company can [scale Azure Synapse on demand](/azure/synapse-analytics/sql-data-warehouse/sql-data-warehouse-manage-compute-overview), reducing or even pausing compute to lower costs.
- Azure Analysis Services can be [scaled out](/analysis-services/azure-analysis-services/analysis-services-scale-out) to reduce response times during high query workloads. You can also separate processing from the query pool, so that client queries aren't slowed down by processing operations.
- Azure Analysis Services also has [SLAs](https://www.microsoft.com/licensing/docs/view/Service-Level-Agreements-SLA-for-Online-Services) and [recommended practices for achieving high availability](/analysis-services/azure-analysis-services/analysis-services-bcdr).
- The [Azure Synapse security model](/azure/synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-manage-security) provides connection security, [authentication and authorization](/azure/synapse-analytics/sql-data-warehouse/sql-data-warehouse-authentication) via Microsoft Entra ID or SQL Server authentication, and encryption. [Azure Analysis Services](/analysis-services/azure-analysis-services/analysis-services-manage-users) uses Microsoft Entra ID for identity management and user authentication.
...
- Learn more about the services used in this scenario:
   ...
   - [What is dedicated SQL pool in Azure Synapse Analytics?](/azure/synapse-analytics/sql-data-warehouse/sql-data-warehouse-overview-what-is)
   - [Azure Analysis Services documentation](/analysis-services/azure-analysis-services/)
