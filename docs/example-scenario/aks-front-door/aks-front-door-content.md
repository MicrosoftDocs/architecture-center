...
1. A platform engineer specifies the necessary information in the *main.bicepparams* Bicep parameters file and deploys the Bicep modules to create the Azure resources. The necessary information includes:
   ...
   - The name and resource group of the DNS zone that's used for resolving the Azure Front Door custom domain.
...
1. An Azure Front Door [secret](/azure/templates/microsoft.cdn/profiles/secrets) resource is used to manage and store the TLS certificate that's in the Azure key vault. This certificate is used by the [custom domain](/azure/templates/microsoft.cdn/profiles/customdomains) that's associated with the Azure Front Door endpoint.
