...
Connections to Azure SQL can use either a "redirect" or a "proxy" [connection policy](/azure/azure-sql/database/connectivity-architecture#connection-policy). The "redirect" option is recommended for reduced latency and improved throughput. However, as the documentation calls out, when you are accessing the database over a private endpoint, it will *always* use the "proxy" approach. This has performance implications, because in proxy mode the client connects through the Azure SQL Database gateways rather than being redirected directly to the node hosting the database.

> [!NOTE]
> As a general indication of the impact on performance, we've seen a 5% increase in end-to-end latency by performing a load test between an App Service web app (single instance P3V2 tier) and an Azure SQL Database (General Purpose Gen 5 with 2 cores). The tests were run for 5 minutes by simulating 5 users all requesting a single web page which performed a single query towards the database (the app did nothing else, to measure the connection overhead only). The load test client, web app and database were all deployed to the same Azure region. The average response time as seen by the load test client was seen to increase from 8.51 ms to 8.97 ms when using a private endpoint, resulting in a 5% increase in latency.

The impact of using private endpoints depends greatly on your specific workload and how your app communicates with the database. To assess whether you can still meet your non-functional requirements with this additional latency - and therefore whether you can use private endpoints for connectivity between your app and the database - you should validate your end-to-end scenario using a realistic performance test.
