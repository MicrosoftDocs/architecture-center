...
Service Bus has two delivery modes, *pull* or *proxied push*:
...
- In the proxied push model, Service Bus sends an event through Event Grid when there are new messages. The receiver subscribes to the event. When the event is triggered, the receiver pulls the next batch of messages from Service Bus. This model allow systems to receive messages close to real time, without using resources to poll continuosly for new messages. On the other hand, it adds extra components to the architecture, which will need to be deployed, managed and secured.

When you create a logic app to consume Service Bus messages, we recommend using the Service Bus built-in connector for Logic Apps Standard, which abstracts most of the configuration of the pull model from the customer, without adding extra cost. This will provide the right balance between cost, surface area management and security, as the connector will continuously loop within the Logic Apps runtime engine. For more information, see [Logic Apps Built-in Service Bus Connector](/azure/connectors/connectors-create-api-servicebus.md#service-bus-built-in-connector-triggers).
