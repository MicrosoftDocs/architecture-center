This architecture builds on the [basic enterprise integration][basic-enterprise-integration] architecture. It extends that architecture to show how to integrate enterprise back-end systems by using message brokers and events to decouple services for greater scalability and reliability. Ensure that you're familiar with that design and the components in the basic integration architecture. It provides foundational information about the core components of this architecture.
...
:::image type="content" source="./media/enterprise-integration-message-broker-events.png" alt-text="Reference architecture for enterprise integration using queues and events" border="false" lightbox="./media/enterprise-integration-message-broker-events.png":::

*Download a [Visio file](https://arch-center.azureedge.net/queues-events-content.vsdx) of this architecture*.

### Workflow

The preceding architecture builds on the simpler [basic enterprise integration][basic-enterprise-integration] architecture. That architecture uses [Azure Logic Apps][logic-apps] to orchestrate workflows directly with back-end systems and [Azure API Management][apim] to create catalogs of APIs.
...
Asynchronous communication via a message broker provides the following advantages compared to making direct, synchronous calls to back-end services:
...
- [A code-centric tutorial that uses messaging for integrating non-Microsoft systems using NServiceBus](https://docs.particular.net/tutorials/nservicebus-sagas/3-integration/)
