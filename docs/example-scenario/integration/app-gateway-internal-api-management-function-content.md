...
1. An on-premises application requires access to an internal API that's served through API Management.

1. API Management connects to the backend APIs that are hosted on Azure Functions. This connection is through a private endpoint, which is available through the Azure Functions Premium plan.
...
1. Application Gateway acts as the web application firewall, which requires PFX certificates for SSL termination.

1. API Management connects to the backend APIs, which are hosted on Azure Functions, through a private endpoint. This endpoint is available through the Azure Functions Premium plan.
...
- **[Azure Defender for APIs](/azure/defender-for-cloud/defender-for-apis-deploy)** provides protection, detection, and response for your APIs throughout their lifecycle. It helps you monitor your APIs, improve security, and respond quickly to threats by offering visibility, vulnerability management, and real-time threat detection.
...
- Azure [Defender for APIs](/azure/defender-for-cloud/defender-for-apis-deploy) offers full lifecycle protection, detection, and response coverage for APIs that are published in API Management. One of the main capabilities is the ability to detect exploits of the Open Web Application Security Project (OWASP) API Top 10 vulnerabilities through runtime observations of anomalies using machine learning-based and rule-based detections.
...
- Use [Application Gateway for external access of an internal API Management instance](/azure/api-management/api-management-howto-integrate-internal-vnet-appgateway) to protect API Management instance, provide protection against common web application exploits and vulnerabilities using WAF, and to enable hybrid connectivity.
...
- This deployment utilizes the [Premium plan](https://azure.microsoft.com/pricing/details/functions/) to support availability zone and virtual network support, but if you do not want dedicated instances you can also utilize [Flex Consumption](/azure/azure-functions/flex-consumption-plan) which also supports network access and availability zone. Here is a [price calculator](https://azure.com/e/802ca3c87e13413580e49bab4a9f67ea) of the deployment.
...
- Cloud Adoption Framework guidance for [API Management in Azure landing zones](/azure/cloud-adoption-framework/scenarios/app-platform/api-management/landing-zone-accelerator)
