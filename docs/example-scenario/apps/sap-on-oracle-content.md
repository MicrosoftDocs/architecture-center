...
Since all other VMs supporting SAP  allow the choice of either Gen2 only or Gen1+2 selectively, it is recommended to deploy all VMs as Gen2 only, even if the memory requirements are very low. Even the smallest VMs once deployed as Gen2 can be scaled up to the largest available with a simple deallocate and resize. Gen1 VMs can only scale to VM families allowed to run Gen1 VMs.
...
> *****  See the supported scenarios with [Azure Site Recovery](/azure/site-recovery/azure-to-azure-support-matrix).
...
- Metrics and Logs from the Azure Monitor can be cascaded to Azure log Analytics workspace and correlated with other KPIs for different use cases.
