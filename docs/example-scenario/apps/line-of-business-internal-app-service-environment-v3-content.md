...
1. Operators or administrators will want to perform administration tasks on the continuous integration/continuous deployment (CI/CD) server, or on the Kudu endpoint for the App Service Environment. First, they'll need to connect to the Azure Bastion host.
2. By using the Azure Bastion host, the operator or administrator can then use Remote Desk Protocol (RDP) to access the jumpbox server.
3. From the jumpbox server, the operator or administrator can RDP into the CI/CD server and perform the required tasks, such as agent upgrades, OS upgrades, and so on. The operator or administrator can also connect from the jumpbox server to the Kudu endpoint of the App Service Environment instance, to perform administrative tasks or to perform advanced troubleshooting.
...
2. The artifacts will then be deployed to the app service by connecting to the App Service Environment over the VNet peering.
...
- Consider your requirements for zone redundancy in this reference implementation, as well as the zone redundancy capabilities of any other Azure Services in your solution. App Service Environment v3 supports zone redundancy by spreading instances to all three zones in the target region. This configuration can only be set at the time of the App Service Environment creation, and it might not be available in all regions. See more information, see [Availability zone support for App Service Environment](/azure/app-service/environment/overview-zone-redundancy). This reference implementation implements zone redundancy, but you can change it by cloning this repo and setting the `zoneRedundant` property to `false`.
...
- Since this reference implementation deploys an App Service Environment into a virtual network (referred to as an internal App Service Environment), all applications deployed to the App Service Environment are inherently network-isolated, at the scope of the virtual network.
...
- Although there's no stamp fee for an App Service Environment v3 instance, there's a charge that's levied when no app service Plans are configured within the App Service Environment v3 instance. This charge is levied at the same rate as one instance of a Windows I1v2 instance, for the region in which the App Service Environment v3 instance is deployed.
...
- Consider the CI/CD service that you'll use to deploy the reference implementation. As this reference implementation is an internal App Service Environment, you'll need a self-hosted agent to execute the deployment pipelines. You have the choice to use either a DevOps agent or a GitHub runner. Refer to the [user guide](https://github.com/Azure/appservice-landing-zone-accelerator/tree/main/docs) on the specific configuration values that are required.
- Consider the regions to which you intend to deploy this reference implementation. Consult the [ASEv3 Regions list](/azure/app-service/environment/overview#regions) to ensure the selected regions are enabled for deployment.
