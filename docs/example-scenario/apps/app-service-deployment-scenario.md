...
* Isolated App Service Plan Pricing Tier
...
## Considerations
The real trick for me here was generating a cert that chained up to a trusted root without requiring a Certificate Signing Request generated by the server on which the cert will be eventually placed. In other words, with IIS for example the first step is to generate a CSR from your IIS server and then send it to the SSL cert issuing authority. 

You cannot issue a CSR from the Internal Load Balancer (ILB) of an ASE. The way I handled it was to use this procedure:
...
Based on that, it issues a cert that is signed up to a trusted root, which you can then upload to your ILB. You don’t need to do anything with the individual cert stores on the Web Apps because you have a trusted root SSL cert at the ILB.

Make self-signed or internally issued SSL cert work if we want to make secure calls between services running in ILB ASE
solution on how to make ILB ASE work with internally issued SSL certificate and how to load the internal CA to the trusted root store.
https://www.patrickob.com/2018/11/10/adding-ca-certs-to-the-trusted-root-store-for-web-apps-hosted-in-an-ase/ 
