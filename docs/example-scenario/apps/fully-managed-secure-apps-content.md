...
Make self-signed or internally issued SSL cert work if we want to make secure calls between services running in an ILB App Service Environment. Another [solution to consider][ase-and-internally-issued-cert] on how to make ILB App Service Environment work with internally issued SSL certificate and how to load the internal CA to the trusted root store.
