...
- [Azure API Management](/azure/well-architected/service-guides/api-management/reliability) acts as a [façade](/azure/architecture/patterns/strangler-fig) over both the existing legacy APIs and the new APIs. The new client application consumes a single consistent interface, and the team can modernize legacy backends incrementally behind that façade with minimal impact on front-end development.
- [Azure App Service](/azure/well-architected/service-guides/app-service-web-apps) is a turn-key Platform as a Service (PaaS) service for web hosting that provides out of the box features such as security, load balancing, autoscaling, and automated management. Azure App Service is a great choice for the new APIs being developed for this solution because it provides flexible turn-key hosting, enabling the DevOps team to focus on delivering features.
...
- If the organization decides to keep the existing endpoints private and not expose them publicly, the organization's API Management instance can be linked to an [Azure Virtual Network](/azure/virtual-network/virtual-networks-overview):
  ...
  - In the on-premises scenario, the API Management instance can reach back to the internal service privately via an [Azure VPN Gateway and site-to-site Internet Protocol Security (IPsec) VPN connection](/azure/vpn-gateway/vpn-gateway-howto-site-to-site-resource-manager-portal) or [Azure ExpressRoute](/azure/expressroute/expressroute-introduction). This scenario then becomes a [hybrid of Azure and on-premises](../../reference-architectures/hybrid-networking/index.yml).
...
- The organization might decide to host its APIs on-premises. One reason for this change might be that the organization can't move downstream database dependencies that are in scope for this project to the cloud. If that's the case, the organization can still take advantage of API Management locally by using a [self-hosted gateway](/azure/api-management/self-hosted-gateway-overview).
