...
- [Flannel overlay networking.][] Flannel creates a virtual network layer that overlays the host network. Overlay networking uses encapsulation of the network packets over the existing physical network. Flannel simplifies IP Address Management (IPAM), enables IP re-use across applications and namespaces, and lets Kubernetes nodes use an underlay network that's separate from container networks. Network isolation is achieved by using Virtual eXtensible Local Area Network (VXLAN), an encapsulation protocol that provides data center connectivity using tunneling to stretch Layer 2 connections over an underlying Layer 3 network. Flannel is supported both by Linux containers that use a *DaemonSet* and by Windows containers that use the Flannel CNI plugin.
